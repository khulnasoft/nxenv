...
A collection of common issues and FAQs when setting up push notifications with the `nxenv-notifications` library and Nxenv's push notification service.

## Nxenv's push notification service

### Cost of the push notification service

There is no cost associated with sending notifications through Nxenv's classic push notification service.
...
For best results, we recommend you add throttling (which is handled automatically in the [`nxenv-server-sdk-node`](https://github.com/nxenv/nxenv-server-sdk-node)) and retry logic to your server.

### Using Nxenv's push notification service is not mandatory

You can use any push notification service for Nxenv projects. The [`getDevicePushTokenAsync` method from `nxenv-notifications`](/versions/latest/sdk/notifications/#getdevicepushtokenasync-devicepushtoken) allows you to get the native device push token, which you can then use with other services, or even [send your notifications directly through FCM and APNs](/push-notifications/sending-notifications-custom).

### Connections to notification service are encrypted

Nxenv's connections to Apple and Google are encrypted and use HTTPS.

### Contents of the notification are not stored

Nxenv doesn't store the contents of push notifications any longer than it takes to deliver the notifications to the push notification services operated by Apple, Google, and so on. Notifications are stored only in memory and in message queues and not stored in databases.

### Contents of the notifications may be seen by Nxenv staff

If the Nxenv team is actively debugging the push notifications service, we may see notification contents (for example, at a breakpoint) but Nxenv cannot see push notification contents otherwise.
...
### When and why does the `NxenvPushToken` change

The `NxenvPushToken` will remain the same across app upgrades. On iOS, it will also remain the same even after uninstalling the app and reinstalling it. On Android, this results in the push token changing.

It will also change if you change your [`applicationId`](/versions/latest/sdk/application.mdx#applicationapplicationid) or `experienceId` (usually `@nxenvUsername/projectSlug`).

The `NxenvPushToken` will never expire. However, if one of your users uninstalls the app, you'll receive a `DeviceNotRegistered` error back from Nxenv's servers. This means you should stop sending notifications to this app.

### Notifications work in development, but not after building the app

This indicates that you have either misconfigured your credentials or didn't configure them at all in your production app. Nxenv Go uses Nxenv's credentials, which allows working on notifications in development.
...
`getDevicePushTokenAsync` and `getNxenvPushTokenAsync` can sometimes take a long time to resolve on iOS. This is outside of `nxenv-notifications`'s control, as stated in Apple's [Troubleshooting Push Notifications](https://developer.apple.com/library/archive/technotes/tn2265/_index.html) technical note:
...
- Try again in a few days as suggested by [this GitHub comment](https://github.com/nxenv/nxenv/issues/10369#issuecomment-717872956).
...
If you are not using [Nxenv's push notification service](/push-notifications/sending-notifications) and instead, would like to communicate with Google and Apple directly, see [Send notifications with FCM and APNs](/push-notifications/sending-notifications-custom), paying **special attention to the payload formats**, since providing different formats can result in unexpected behavior on both platforms.

### Notification icon on Android is a grey or white square

This indicates an issue with the image asset you're providing. The image should be all white with a transparent background (this is required and enforced by Google, not Nxenv). For more information, see [this article](https://clevertap.com/blog/fixing-notification-icon-for-android-lollipop-and-above/).

### Play a custom sound when I send a notification

The Nxenv push notification service currently doesn't support custom sounds. You will need to use FCM and APNs directly with the native device tokens received from `nxenv-notifications` in standalone apps.

### Migrate from Nxenv's `LegacyNotifications` library

For more information on how to migrate from Nxenv's `LegacyNotifications` library to use `nxenv-notifications`, see [the migration guide](https://nxenv.fyi/legacy-notifications-to-nxenv-notifications).
