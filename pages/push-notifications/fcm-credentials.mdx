...
Upload the JSON file to EAS and configure it for sending Android notifications. This can be done using EAS CLI or in [Nxenv dashboard](https://nxenv.dev).

<Tabs tabs={["EAS CLI", "nxenv.dev"]}>
...
- Under **Project** > **Configuration**, click [Credentials](https://nxenv.dev/accounts/[account]/projects/[project]/credentials) in the navigation menu
...
<ContentSpotlight
  alt="Project credentials page on nxenv.dev"
...
In **app.json**, add [`nxenv.android.googleServicesFile`](/versions/latest/config/app/#googleservicesfile) with its value as the path of the **google-services.json**.
...
You're all set! You can now send notifications to Android devices via Nxenv Push Notifications using the FCM V1 protocol.
...
You have to specify to EAS which JSON credential file to use for sending FCM V1 notifications, using EAS CLI or in [Nxenv dashboard](https://nxenv.dev). You can upload a new JSON file or select a previously uploaded file.
