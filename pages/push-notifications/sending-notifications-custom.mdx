...
You may need finer-grained control over your notifications, so communicating directly with FCM and APNs may be necessary. The Nxenv platform does not lock you into using Nxenv's application services, and the `nxenv-notifications` API is push-service agnostic.
  ...
  const messageBody = {
    message: {
      ...
      data: {
        ...
        scopeKey: '@yourNxenvUsername/yourProjectSlug',
        experienceId: '@yourNxenvUsername/yourProjectSlug',
      },
    },
  };
...
Your FCM server key can be found by making sure you've followed the [configuration steps](/push-notifications/push-notifications-setup/#android), and instead of uploading your FCM key to Nxenv, you would use that key directly in your server (as the `FCM-SERVER-KEY` in the previous example).
...
request.write(
  JSON.stringify({
    ...
    experienceId: '@yourNxenvUsername/yourProjectSlug', // Required when testing in the Nxenv Go app
    scopeKey: '@yourNxenvUsername/yourProjectSlug', // Required when testing in the Nxenv Go app
  })
);
request.end();
```

> This example is minimal and includes no error handling and connection pooling. For testing purposes, you can refer to [`sentNotificationToAPNS`](https://github.com/nxenv/nxenv/blob/main/docs/public/static/examples/sendNotificationToAPNS.js) example code.
...
The examples in the previous section provide the bare minimum notification requests. You may have to send category identifiers, custom sounds, icons, custom key-value pairs, and so on. [`nxenv-notifications` documents](/versions/latest/sdk/notifications/#manage-notification-categories-interactive-notifications) all the fields it supports, and below are the example payloads Nxenv sends in its notifications service:
...
```json
{
  ...
  "data": {
    ...
    "priority": AndroidNotificationPriority, // https://docs.nxenv.dev/versions/latest/sdk/notifications/#androidnotificationpriority
...
1. **Notification** messages are only handled (and displayed) by the Firebase library. They don't necessarily wake the app, and `nxenv-notifications` will not be made aware that your app has received any notification.

2. **Data** messages are not handled by the Firebase library. They are immediately handed off to your app for processing. That's where `nxenv-notifications` interprets the data payload and takes action based on that data. **In almost all cases, this is the type of notification you have to send.**
...
> Using notification-type messages can be beneficial when you need a configuration option that is not yet nxenvsed by `nxenv-notifications`. Generally, it may lead to less predictable situations than using data-type messages. However, you may need to report any issue you encounter directly to Google.
...
// it will not trigger nxenv-notifications and may not work as expected.
admin.messaging().sendToDevice(
  devicePushToken,
  {
    notification: {
      title: "This is a notification-type message",
      body: "`nxenv-notifications` will never see this 😢",
    },
    data: {
      photoId: 42,
    },
...
// so the message is a "data" message, thus triggering nxenv-notifications.
admin.messaging().sendToDevice(
  devicePushToken,
  {
    data: {
      title: "This is a data-type message",
      message: "`nxenv-notifications` events will be triggered 🤗",
      ...
      // https://docs.nxenv.dev/versions/latest/sdk/notifications/#android-push-notification-payload-specification
