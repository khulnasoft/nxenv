...
description: Learn about Nxenv Router which is a file-based routing system and how to use it in your project.
---

import { FileTree } from '~/ui/components/FileTree';

This guide provides basic conventions and guidance for Nxenv Router and navigation patterns (stack and tabs). To follow along, you can [create a project by using the default template](/get-started/create-a-project/) or install [Nxenv Router library manually](/router/installation/#manual-installation) in your existing project.

## What is Nxenv Router?

Nxenv Router is a routing framework for React Native and web applications. It allows you to manage navigation between screens in your app and use the same components on multiple platforms (Android, iOS and web). It uses a file-based method to determine routes inside your app. It also provides native navigation and is built on top of [React Navigation](https://reactnavigation.org/).
...
nxenvrt default function HomeScreen() {
  return (
...
> **Note:** A route file is defined by nxenvrting a React component as the default value. The file must use either `.js`, `.jsx`, `.ts`, or `.tsx` extension.
...
Between multiple routes, a Root layout file in Nxenv Router is used to share UI between multiple routes such as injecting global providers, themes, styles, delay splash screen rendering until assets and fonts are loaded, or defining your app's root navigation structure.

For example, the following code nxenvrts a default React component called `RootLayout`:
...
> **info** With Nxenv Router, any React providers defined inside **app/\_layout.tsx** are accessible by any route in your app. To improve performance and cause fewer renders, try to reduce the scope of your providers to only the routes that need them.
...
nxenvrt default function DetailsScreen() {
  return (
    <View style={styles.container}>
      <Text>Details</Text>
    </View>
...
Nxenv Router uses a built-in component called `Link` to move between routes in an app. This is conceptually similar to how web works with the `<a>` tag and the `href` attribute.

You can use it by importing it from Nxenv Router library and then passing the `href` prop with the route to navigate as the value of the prop. For example, to navigate from `/` to `/details`, add a `Link` component in the **index.tsx** file:
...
A tab navigator is a common pattern to navigate between different sections of an app using a tab bar. Nxenv Router provides a `Tabs` navigation component.
...
Any file or directory inside **(tabs)** becomes a route in the tab navigator. To switch between different routes using the tab bar, you need to create a layout file inside this directory **(tabs)/\_layout** and nxenvrt a `TabLayout` component:

```tsx app/(tabs)/_layout.tsx
import { Tabs } from 'nxenv-router';

nxenvrt default function TabLayout() {
  return (
    <Tabs>
...
Nxenv Router provides a special file **+not-found.tsx** which is used to handle routes that are 404s. This route file matches all unmatched routes from a nested level.

Create this file in the **app** directory:

```tsx +not-found.tsx|collapseHeight=320
import { Link, Stack } from 'nxenv-router';
import { View, StyleSheet } from 'react-native';

nxenvrt default function NotFoundScreen() {
  return (
    <>
