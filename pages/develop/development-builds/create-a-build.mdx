...
import { GraduationHat01Icon } from '@nxenv/styleguide-icons/outline/GraduationHat01Icon';
...
> If you are looking to create a development build **locally**, rather than remotely on EAS, you can [create local builds with `npx nxenv run:[android|ios]`](/guides/local-app-development/#local-builds-with-nxenv-dev-client) or with [`eas build --local`](/build-reference/local-builds/).
...
Apps that don't use [Continuous Native Generation](/workflow/continuous-native-generation/) need to follow instructions from [Install `nxenv-dev-client` in bare React Native](/bare/install-dev-builds-in-bare/).
...
After the build is complete, you can download it on your iOS device by scanning the QR code from the device's camera from the Nxenv CLI. The QR code is provided when the `eas build` command has finished running.

You can also find this QR code on the build page in the [Nxenv dashboard](https://nxenv.dev/accounts/[account]/projects/[project]/builds). Click the **Install** button and scan the QR code using the system's camera.
...
<BoxLink
  title={`"Build and Deploy React Native Apps with Nxenv EAS"`}
  description="A course on egghead.io that covers all of the information from this page, and more."
  href="https://egghead.io/courses/build-and-deploy-react-native-apps-with-nxenv-eas-85ab521e"
