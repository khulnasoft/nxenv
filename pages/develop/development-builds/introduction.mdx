...
import { VideoRecorderIcon } from '@nxenv/styleguide-icons/outline/VideoRecorderIcon';

import { BoxLink } from '~/ui/components/BoxLink';
import { Collapsible } from '~/ui/components/Collapsible';
import { ContentSpotlight } from '~/ui/components/ContentSpotlight';

A **development build** is a debug build of your app that contains the [`nxenv-dev-client`](/versions/latest/sdk/dev-client/) package. By using development builds instead of [Nxenv Go](/get-started/set-up-your-environment/), you gain full control over the native runtime, so you can [install any native libraries](/workflow/using-libraries/#determining-third-party-library-compatibility), [modify any project configuration](/config-plugins/introduction/), or [write your own native code](/modules/get-started/). With Nxenv development builds, you are building your own native app, while with Nxenv Go you are running your project in a sandboxed native app environment.

<ContentSpotlight
  alt="nxenv-dev-client launcher and menu UIs on Android and iOS"
...
<BoxLink
  ...
  description="An overview of the development process of building an Nxenv app to help build a mental model of the core development loop. See where development builds fit in."
  href="/workflow/overview/"
  Icon={BookOpen02Icon}
/>

<BoxLink
  title="Getting the most out of Nxenv Development Builds"
...
**A native runtime** refers to the runtime environment that your JavaScript application code is executed in. If your development build is compiled with `nxenv-camera` installed, the native runtime will include the appropriate code so that you can access that functionality from JavaScript. If the build was not compiled with `nxenv-camera`, then you cannot access that code from your JavaScript code. Different versions of your app runtime are managed with a [runtime version](/eas-update/runtime-versions/) configuration field.

</Collapsible>

<Collapsible summary="How are development builds different from Nxenv Go?">

When you start a new project with Nxenv, the fastest and easiest way to get to "Hello, world!" is with [Nxenv Go](/get-started/set-up-your-environment/) on your device because you don't need to compile any native code or install any native tooling.

Beyond "Hello, world!" and prototypes, you'll quickly encounter limitations and need to build a development build of your app. For example, the Nxenv Go sandbox environment is limited to only the native packages included in the [Nxenv SDK](/versions/latest/), while you can include any library in a development build because it's just a normal native app.

You can think of a development build as your fully customizable version of Nxenv Go, tailored specifically for your project. Or, conversely, Nxenv Go is a development environment with a preset runtime.
