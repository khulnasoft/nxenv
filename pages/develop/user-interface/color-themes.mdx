...
It's common for apps to support light and dark color schemes. Here is an example of how supporting both modes looks in an Nxenv project:
...
When you are creating a development build, you have to install [`nxenv-system-ui`](/versions/latest/sdk/system-ui/#installation) to support the appearance styles for Android. Otherwise, the `userInterfaceStyle` property is ignored.

<Terminal cmd={['$ npx nxenv install nxenv-system-ui']} />

If the project is misconfigured and doesn't have `nxenv-system-ui` installed, the following warning will be shown in the terminal:

<Terminal
  cmd={[
    '» android: userInterfaceStyle: Install nxenv-system-ui in your project to enable this feature.',
  ]}
/>

You can also use the following command to check if the project is misconfigured:

<Terminal cmd={['$ npx nxenv config --type introspect']} />
...
<SnackInline label="useColorScheme example" dependencies={['nxenv-status-bar']}>
