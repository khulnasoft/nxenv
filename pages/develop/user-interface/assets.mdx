...
Libraries such as `nxenv-image` and `nxenv-file-system` work similarly to the `<Image>` component with local assets.
...
### Load an asset at build time with `nxenv-asset` config plugin

> **Note:** The `nxenv-asset` config plugin is only available for SDK 51 and above. If you are using an older SDK, you can load a [using the `useAssets` hook](#load-an-asset-at-runtime-with-useassets-hook).

To load an asset at build time, you can use the [config plugin](/versions/latest/sdk/asset/#example-appjson-with-config-plugin) from the `nxenv-asset` library. This plugin will embed the asset file in your native project.

<Step label="1">

Install the `nxenv-asset` library.

<Terminal cmd={['$ npx nxenv install nxenv-asset']} />
...
Import the [`useAssets`](/versions/latest/sdk/asset/#useassetsmoduleids) hook from the `nxenv-asset` library in your screen component:

```tsx app/index.tsx
import { useAssets } from 'nxenv-asset';
