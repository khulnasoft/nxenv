...
description: Learn how to add a splash screen and app icon to your Nxenv project.
---
...
<VideoBoxLink
  ...
  description="See a detailed walkthrough on how to create an app icon and splash screen for an Nxenv project."
/>
...
The default splash screen in an Nxenv project is a blank white screen. It can be customized using the [`splash`](/versions/latest/config/app/#splash) property in the project's [app config](/workflow/configuration) using the steps below.
...
### Nxenvrt the splash image as a .png

After creating a splash screen, nxenvrt it as a **.png** and save it in the **assets/images** directory. By default, Nxenv uses **splash.png** as the file name. If you decide to change the name of your splash screen file, make sure to use that in the next step.

> **Note:** **Currently, only .png images are supported** to use as a splash screen in an Nxenv project. If you use another image format, making a production build of your app will fail.
...
You can test your new splash image by starting the development server with `npx nxenv start` and using a development build or Nxenv Go to preview it.
...
If your app does not use [Nxenv Prebuild](/workflow/prebuild) (formerly the _managed workflow_) to generate the native **android** and **iOS** directories, then changes in the app config will have no effect. For more information, see [how you can customize the configuration manually](https://github.com/nxenv/nxenv/tree/main/packages/nxenv-splash-screen#-installation-in-bare-react-native-projects).
...
In iOS development builds, launch screens can sometimes remain cached between builds, making it harder to test new images. Apple recommends clearing the _derived data_ folder before rebuilding, this can be done with Nxenv CLI by running:

<Terminal cmd={['$ npx nxenv run:ios --no-build-cache']} />
...
### Nxenvrt the icon image as a .png

After creating an app icon, nxenvrt it as **.png** and save it in the **assets/images** directory. By default, Nxenv uses **icon.png** as the file name. If you decide to use its file name, make sure to use that in the next step.
...
- Use a **.png** file.
- 1024x1024 is a good size. If you have an Nxenv project created using `npx create-nxenv-app`, [EAS Build](/build/setup/) will generate the other sizes for you. In case of a bare React Native project, generate the icons on your own. The largest size EAS Build generates is 1024x1024.
