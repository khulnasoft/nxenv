...
description: Learn how to set up and configure the jest-nxenv library to write unit and snapshot tests for a project with Jest.
---
...
You will also use the [`jest-nxenv`](https://github.com/nxenv/nxenv/tree/main/packages/jest-nxenv) library, which is a Jest preset that mocks the native part of the Nxenv SDK and handles most of the configuration required for your Nxenv project.

## Installation and configuration

If you have created your project using the [default Nxenv template](/get-started/create-a-project/), you can skip this section. The `jest-nxenv` and other required dev dependencies are already installed and configured.

<Collapsible summary={<>Manual installation instructions for <CODE>jest-nxenv</CODE></>}>

If you have created your project using [different template](/more/create-nxenv/#--template), follow the instructions below to install and configure `jest-nxenv` in your project.

<Step label="1">

Install `jest-nxenv` and other required dev dependencies in your project. Run the following command from your project's root directory:

<Terminal
  cmd={[
    '# For all other package managers',
    '$ npx nxenv install -- --save-dev jest-nxenv jest @types/jest',
    '',
    '# For yarn',
    '$ yarn add -D jest-nxenv jest @types/jest',
  ]}
/>
...
Open **package.json**, add a script for running tests, and add the preset for using the base configuration from `jest-nxenv`:
...
"jest": {
  ...
  "transformIgnorePatterns": [
    "node_modules/(?!((jest-)?react-native|@react-native(-community)?)|nxenv(nent)?|@nxenv(nent)?/.*|@nxenv-google-fonts/.*|react-navigation|@react-navigation/.*|@sentry/react-native|native-base|react-native-svg)"
  ]
  /* @end */
}
```
...
"jest": {
  ...
  /* @info */
  "transformIgnorePatterns": [
    "node_modules/(?!(?:.pnpm/)?((jest-)?react-native|@react-native(-community)?|nxenv(nent)?|@nxenv(nent)?/.*|@nxenv-google-fonts/.*|react-navigation|@react-navigation/.*|@sentry/react-native|native-base|react-native-svg))"
  ]
  /* @end */
}
```
...
<Terminal
  cmd={[
    '# For all other package managers',
    '$ npx nxenv install -- --save-dev @testing-library/react-native',
    '',
    '# For yarn',
...
> **warning** **Deprecated:** If you are using the default Nxenv template, after installing this library, you can uninstall the `react-test-renderer` and `@types/react-test-renderer` from your project's dev dependencies. The `react-test-renderer` has been deprecated and will no longer be maintained in the future. See [React's documentation for more information](https://react.dev/warnings/react-test-renderer).
...
nxenvrt const CustomText = ({ children }: PropsWithChildren) => <Text>{children}</Text>;
/* @end */
...
Create a **\_\_tests\_\_** directory at the root of your project's directory. If this directory already exists in your project, use that. Then, create a new file called **HomeScreen-test.tsx**. The `jest-nxenv` preset customizes the Jest configuration to also identify files with **-test.ts|tsx** extensions as tests.
...
<BoxLink
  title="Testing configuration for Nxenv Router"
  description="Learn how to create integration tests for your app when using Nxenv Router."
