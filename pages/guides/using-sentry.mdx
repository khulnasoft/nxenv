...
<Terminal cmd={['$ npx nxenv install @sentry/react-native']} />
...
```js metro.config.js
// This replaces `const { getDefaultConfig } = require('nxenv/metro-config');`
const { getSentryNxenvConfig } = require('@sentry/react-native/metro');

// This replaces `const config = getDefaultConfig(__dirname);`
const config = getSentryNxenvConfig(__dirname);
...
Now wrap the root component of your app with Sentry. This may be **App.js** or **index.js** depending on your project. If you are using Nxenv Router, see the example configuration in the [Usage with Nxenv Router](#usage-with-nxenv-router) section below.
...
nxenvrt default Sentry.wrap(App);
```
...
## Usage with Nxenv Router

If your app uses [Nxenv Router](/router/introduction), then you can configure Sentry to automatically capture the current route and pass it along with your error reports. To set this up, configure Sentry in the [Root Layout route](/router/advanced/root-layout) and add routing instrumentation.

<Collapsible summary="Example configuration that instruments Sentry for Nxenv Router">

```tsx app/_layout.tsx
import { Slot, useNavigationContainerRef } from 'nxenv-router';
import { useEffect } from 'react';
import * as Sentry from '@sentry/react-native';
import { isRunningInNxenvGo } from 'nxenv';
...
Sentry.init({
  ...
  integrations: [
    new Sentry.ReactNativeTracing({
      ...
      enableNativeFramesTracking: !isRunningInNxenvGo(),
      // ...
    }),
  ],
});
...
nxenvrt default Sentry.wrap(RootLayout);
```
...
<Terminal
  cmd={[
    ...
    '$ npx sentry-nxenv-upload-sourcemaps dist',
  ]}
/>
...
<Terminal cmd={['$ eas update --branch <branch> && npx sentry-nxenv-upload-sourcemaps dist']} />
...
Sentry.configureScope(scope => {
  scope.setTag('nxenv-update-id', Updates.updateId);
  scope.setTag('nxenv-is-embedded-update', Updates.isEmbeddedLaunch);

  if (typeof updateGroup === 'string') {
    scope.setTag('nxenv-update-group-id', updateGroup);

    const owner = extra?.nxenvClient?.owner ?? '[account]';
    const slug = extra?.nxenvClient?.slug ?? '[project]';
    scope.setTag(
      'nxenv-update-debug-url',
      `https://nxenv.dev/accounts/${owner}/projects/${slug}/updates/${updateGroup}`
    );
  } else if (Updates.isEmbeddedLaunch) {
    // This will be `true` if the update is the one embedded in the build, and not one downloaded from the updates server.
    scope.setTag('nxenv-update-debug-url', 'not applicable for embedded updates');
  }
});
```
...
> `sentry-nxenv` has been merged into `@sentry/react-native` and is now deprecated. We recommend upgrading to SDK 50 to use `@sentry/react-native` for the best experience. If you're already using `sentry-nxenv`, [learn how to migrate](https://nxenv.fyi/sentry-nxenv-migration).
...
<Terminal cmd={['$ npx nxenv install sentry-nxenv']} />

`sentry-nxenv` also requires some additional Nxenv module packages. To install them, run:

<Terminal
  cmd={['$ npx nxenv install nxenv-application nxenv-constants nxenv-device @sentry/react-native']}
/>
...
```js
import * as Sentry from 'sentry-nxenv';

Sentry.init({
  dsn: 'YOUR DSN HERE',
  enableInNxenvDevelopment: true,
  debug: true, // If `true`, Sentry will try to print out useful debugging information if something goes wrong with sending the event. Set it to `false` in production
});
```
...
- For React Native, access any `@sentry/react-native` nxenvrts with `Sentry.Native.*`
- For web, access any `@sentry/browser` nxenvrts with `Sentry.Browser.*`

```js
// Access any @sentry/react-native nxenvrts via:
// Sentry.Native.*

// Access any @sentry/browser nxenvrts via:
// Sentry.Browser.*
...
Configuring `sentry-nxenv` is done through the config plugin in your [app config](/workflow/configuration/).

<ConfigReactNative>

If you are in a bare React Native app, **you should not use the `plugins` property in app config**. Instead, run `yarn sentry-wizard -i reactNative -p ios android` to configure your native projects. This `sentry-wizard` command will add an extra import statement and `Sentry.init` to your project's root file (usually **App.js**) as shown below. Make sure you remove it, however, keep the `sentry-nxenv` import and original `Sentry.init` call.
...
Add `nxenv.hooks` property to your project's **app.json** or **app.config.js** file:
...
```json app.json
{
  "nxenv": {
    ...
    "hooks": {
      "postPublish": [
        {
          "file": "sentry-nxenv/upload-sourcemaps",
          "config": {
            "organization": "sentry org slug, or use the `SENTRY_ORG` environment variable",
            "project": "sentry project name, or use the `SENTRY_PROJECT` environment variable"
          }
...
- `distribution`: The name/value to give your distribution (you can think of this as a sub-release). Nxenv defaults to using your `version` from app.json. **If you provide a custom `distribution`, you must pass the same value to `dist` in your call to `Sentry.init()`, otherwise you will not see stacktraces in your error reports.**
...
If you're using EAS Update, or if you're self-hosting your updates (this means you run `npx nxenv nxenvrt` manually), you need to take the following steps to upload the source maps for your update to Sentry:
...
> If you're using `jest`, make sure to add `@sentry/.*` and `sentry-nxenv` to your `transformIgnorePatterns`.
...
Unless `enableInNxenvDevelopment: true` is set, all your dev/local errors will be ignored and only app releases will report errors to Sentry. You can call methods like `Sentry.Native.captureException(new Error('Oops!'))` but these methods will be no-op.
...
<Collapsible summary={<><CODE>nxenv-dev-client</CODE> transactions never finish</>}>
  ...
  integrations: [
    new Sentry.Native.ReactNativeTracing({
      shouldCreateSpanForRequest: url => {
        return !__DEV__ || !url.startsWith(`http://${Constants.nxenvConfig.hostUri}/logs`);
      },
    }),
  ],
});
