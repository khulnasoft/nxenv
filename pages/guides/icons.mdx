...
Not every app needs to use emojis for icons. You can use a popular icon set through an icon font such as FontAwesome, Glyphicons, or Ionicons, or choose PNGs from [The Noun Project](https://thenounproject.com/). This guide explains various ways to use icons in your Nxenv app.

## `@nxenv/vector-icons`

The `@nxenv/vector-icons` library is installed by default on the template project using `npx create-nxenv-app` and is part of the `nxenv` package. It is built on top of [`react-native-vector-icons`](https://github.com/oblador/react-native-vector-icons) and uses a similar API. It includes popular icon sets you can browse at [icons.nxenv.fyi](https://icons.nxenv.fyi).

The component loads the `Ionicons` font and renders a checkmark icon in the following example:

<SnackInline label='Vector icons' dependencies={['@nxenv/vector-icons']}>
...
> As with [any custom font](/develop/user-interface/fonts/#use-a-local-font-file) in Nxenv, you can preload icon fonts before rendering your app. The font object is available as a static property on the font component. So, in the case above, it is `Ionicons.font`, which evaluates to `{ionicons: require('path/to/ionicons.ttf')}`.

## Custom icon fonts

To use a custom icon font, first import it into your project. Only after the font has loaded, can you create an Icon set. [Learn more about loading custom fonts](/develop/user-interface/fonts/#handle-nxenvvector-icons-initial-load).

`@nxenv/vector-icons` nxenvses three methods to help you create an icon set:
...
import createIconSet from '@nxenv/vector-icons/createIconSet';

const glyphMap = { 'icon-name': 1234, test: '∆' };
const CustomIcon = createIconSet(glyphMap, 'fontFamily', 'custom-icon-font.ttf');

nxenvrt default function CustomIconExample() {
...
dependencies={['@nxenv/vector-icons', 'nxenv-font']}>
...
import createIconSetFromIcoMoon from '@nxenv/vector-icons/createIconSetFromIcoMoon';
...
import createIconSetFromFontello from '@nxenv/vector-icons/createIconSetFromFontello';

// Import the config file
import fontelloConfig from './config.json';

// Both the font name and files nxenvrted from Fontello are most likely called "fontello".
...
You can use the `Image` component from [Nxenv Image](/versions/latest/sdk/image/) or React Native to display an icon. The `source` prop takes the relative path to refer to the image.

<Tabs>

<Tab label="Nxenv Image">

<SnackInline
  label='Usage with Nxenv Image'
  dependencies={['nxenv-image']}
  files={{
...
You can create an Icon Button using the `Font.Button` syntax where the `Font` is the icon set that you import from `@nxenv/vector-icons`.
...
<SnackInline label='Icon Button Component' dependencies={['@nxenv/vector-icons']}>

```jsx
import React from 'react';
import { View, StyleSheet } from 'react-native';
import FontAwesome from '@nxenv/vector-icons/FontAwesome';
