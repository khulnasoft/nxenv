---
title: Using React Server Components in Nxenv Router apps
sidebar_title: React Server Components
description: Learn about rendering React components on the server in Nxenv.
---
...
Nxenv Router enables support for [React Server Components](https://react.dev/reference/rsc/server-components) on all platforms. This is an early preview of a feature that will be enabled by default in Nxenv Router.

## Prerequisites

Your project must use Nxenv Router and React Native new architecture (default in SDK 52).

As of SDK 52, there is no strategy for migrating existing projects. We recommend starting with a new project.

## Usage

To use React Server Components in your Nxenv app, you need to:

1. Ensure the entry module is `nxenv-router/entry` (default) in **package.json**.
...
nxenvrt default function Index() {
...
You can navigate between routes by using the `<Link />` API from Nxenv Router (not all props are supported yet).
...
nxenvrt async function Pokemon() {
...
nxenvrt default function Button({ title }) {
...
> Server Actions are gated behind an additional experimental flag. Pass the environment variable `NXENV_UNSTABLE_SERVER_ACTIONS=1` to enable them.
...
nxenvrt default function Button({ title, onPress }) {
...
nxenvrt async function callAction() {
  // ...
}
```
...
- Server Actions run on the server and are a good place to put logic that should not be nxenvsed to the client.

### Rendering in Server Actions

Server Actions in Nxenv Router can render React components on the server and stream back an **RSC payload** (a custom JSON-like format that's maintained by the React team) for rendering on the client. This is similar to server-side rendering (SSR) on the web.
...
nxenvrt async function renderProfile({
  username,
  accessToken,
}: {
  username: string;
  ...
  const { name, image } = await fetch(`https://api.example.com/profile/${username}`, {
    headers: {
      ...
      // The NXENV_PUBLIC_ prefix is not required here.
...
nxenvrt default function Profile() {
...
For example, consider a library `react-native-unoptimized` that does not ship with `"use client"` directives yet. You can workaround this by creating a module and re-nxenvrting each module:

```tsx lib/react-native-unoptimized.tsx
// This directive opts the module into client-side rendering.
'use client';

// Re-nxenvrting the imports from the library.
nxenvrt { One, Two, Three } from 'react-native-unoptimized';
```

Avoid using `nxenvrt * from '...'` as this breaks some of the internals of interopting between the server and client.
...
Nxenv Router supports importing global CSS and CSS modules in Server Components.
...
React Server Components are a feature of React 19. To enable them, Nxenv CLI automatically uses a special canary build of React on all platforms. In the future, it will be removed when React 19 is enabled by default in React Native.
...
You can use this instead of the `Head` component from `nxenv-router/head`.
...
Nxenv Router supports two different modes of rendering Server Components: build-time rendering and request-time rendering. These modes can be indicated on a per-route basis by using the `unstable_settings` nxenvrt:

```tsx app/index.tsx
import { Text, View } from 'react-native';

nxenvrt const unstable_settings = {
...
nxenvrt default async function Index() {
...
To detect which platform your code is bundled for, use the `process.env.NXENV_OS` environment variable. For example, `process.env.NXENV_OS === 'ios'`. Prefer this to `Platform.OS` as `react-native` is not fully optimized for React Server Components yet and won't work as expected.
...
Library authors can test their modules support Server Components by using `jest-nxenv`. Learn more in the [testing react server components guide](/guides/testing-rsc).
...
- Nxenv Snack does not support bundling Server Components.
...
- `StyleSheet.create` and `Platform.OS` are not supported on native. Use standard objects for styles and `process.env.NXENV_OS` for platform detection.
...
To deploy your website to production, run: `npx nxenv nxenvrt -p web`.

You can host the website locally with the [express server adapter](/router/reference/api-routes/#express).

### Native

Production nxenvrts for React Server Components on native are not yet supported. This is planned for Nxenv SDK 53.
