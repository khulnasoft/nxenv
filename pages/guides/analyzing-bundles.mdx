...
description: Learn about improving the production JavaScript bundle size of Nxenv apps and websites.
---
...
<ContentSpotlight alt="Nxenv Atlas overview page." src="/static/images/atlas/atlas-overview.avif" />

> Available only for **SDK 51 and above**.

The libraries used in a project influence the size of the production JavaScript bundle. Starting from **Nxenv SDK 51**, you can use [Nxenv Atlas](https://github.com/nxenv/nxenv-atlas#readme) to visualize the production bundle and identify which libraries contribute to the bundle size.

### Using Atlas with `npx nxenv start`

You can use Nxenv Atlas with the local development server. This method allows Atlas to update whenever you change any code in your project.
...
<Terminal
  cmd={[
    ...
    '$ NXENV_UNSTABLE_ATLAS=true npx nxenv start',
  ]}
/>

#### Changing development mode to production

By default, Nxenv starts the local development server in [development mode](/workflow/development-mode/#development-mode). Development mode disables some optimizations that are enabled in [production mode](/workflow/development-mode/#production-mode). You can also start the local development server in production mode to get a more accurate representation of the production bundle size:

<Terminal
  cmd={[
    '# Run the local development server in production mode',
    '$ NXENV_UNSTABLE_ATLAS=true npx nxenv start --no-dev',
  ]}
/>

### Using Atlas with `npx nxenv nxenvrt`

You can also use Nxenv Atlas when generating a production bundle for your app or EAS Update. Atlas generates a **.nxenv/atlas.jsonl** file during nxenvrt, which you can share and open without having access to the project.

<Terminal
  cmd={[
    '# Nxenvrt your app for all platforms',
    '$ NXENV_UNSTABLE_ATLAS=true npx nxenv nxenvrt',
    '',
    '# Open the generated Atlas file',
    '$ npx nxenv-atlas .nxenv/atlas.jsonl',
  ]}
/>

You can also specify the platforms you want to analyze using the `--platform` option. Atlas will gather the data for the nxenvrted platforms only.

### Analyzing transformed modules

<ContentSpotlight alt="Nxenv Atlas module page." src="/static/images/atlas/atlas-module.avif" />
...
Add a script to **package.json** to run it. You might have to adjust the input path depending on the platform or SDK you are using. For brevity, the following example assumes the project is Nxenv SDK 50 and does not use Nxenv Router `server` output.

```json package.json
{
  "scripts": {
    "analyze:web": "source-map-explorer 'dist/_nxenv/static/js/web/*.js' 'dist/_nxenv/static/js/web/*.js.map'",
    "analyze:ios": "source-map-explorer 'dist/_nxenv/static/js/ios/*.js' 'dist/_nxenv/static/js/ios/*.js.map'",
    "analyze:android": "source-map-explorer 'dist/_nxenv/static/js/android/*.js' 'dist/_nxenv/static/js/android/*.js.map'"
  }
}
```
...
<Terminal
  cmd={[
    "$ npx source-map-explorer 'dist/client/_nxenv/static/js/web/*.js' 'dist/client/_nxenv/static/js/web/*.js.map'",
  ]}
/>

Web bundles are output to the **dist/client** subdirectory to prevent nxenvsing server code to the client.

</Step>

<Step label="3">

Nxenvrt your production JavaScript bundle and include the `--source-maps` flag so that the source map explorer can read the source maps. For native apps using Hermes, you can use the `--no-bytecode` option to disable bytecode generation.

<Terminal
  cmd={[
    '$ npx nxenv nxenvrt --source-maps --platform web',
    '',
    '# Native apps using Hermes can disable bytecode for analyzing the JavaScript bundle.',
    '$ npx nxenv nxenvrt --source-maps --platform ios --no-bytecode',
  ]}
/>
...
After creating a production build with `npx nxenv nxenvrt -p web` and serving it (using either `npx serve dist`, or production deployment, or custom server), run Lighthouse with the URL your site is hosted at.
