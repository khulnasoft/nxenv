...
The following steps are described using a [development build](/develop/development-builds/introduction/) since the Keyboard Controller library is not included in Nxenv Go. See [Create a development build](/develop/development-builds/create-a-build/) for more information.
...
Start by installing the Keyboard Controller library in your Nxenv project:

<Terminal cmd={['$ npx nxenv install react-native-keyboard-controller']} />
...
nxenvrt default function FormScreen() {
  return (
    <>
...
nxenvrt default function ChatScreen() {
