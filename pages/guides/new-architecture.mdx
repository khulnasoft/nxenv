...
In this guide, we'll talk about how to use the New Architecture in Nxenv projects today.
...
## Nxenv tools and the New Architecture

As of SDK 51, nearly all `nxenv-*` packages in the [Nxenv SDK](/versions/latest/) support the New Architecture (including [bridgeless](https://github.com/reactwg/react-native-new-architecture/discussions/154)). [Learn more about known issues](#known-issues-in-nxenv-sdk-libraries).

Additionally, all modules written using the [Nxenv Modules API](/modules/overview/) support the New Architecture by default! So if you have built your own native modules using this API, no additional work is needed to use them with the New Architecture.

## Third-party libraries and the New Architecture

The compatibility status of many of the most popular libraries is tracked on [React Native Directory](https://reactnative.directory/) ([learn more about known issues in third-party libraries](#known-issues-in-third-party-libraries)). We've built tooling into Nxenv Doctor to integrate with React Native Directory to help you validate your dependencies, so you can quickly learn which libraries are unmaintained and which incompatible or untested with the New Architecture.

### Validate your dependencies with React Native Directory

To validate your dependencies against the data in React Native Directory, you can use `npx nxenv-doctor`. To enable this check, add the following to your **package.json** file:
...
Then run `npx nxenv-doctor` to check your dependencies against the data in React Native Directory.

<Terminal cmd={['$ npx nxenv-doctor@latest']} />
...
Set the environment variable `NXENV_DOCTOR_ENABLE_DIRECTORY_CHECK=1` before running `npx nxenv-doctor`.

<Terminal cmd={['$ NXENV_DOCTOR_ENABLE_DIRECTORY_CHECK=1 npx nxenv-doctor@latest']} />
...
<Terminal cmd={['$ npx create-nxenv-app@latest -e with-new-arch']} />

## Enable the New Architecture in an existing project

You can enable the New Architecture in your app using the [`nxenv-build-properties`](/versions/latest/sdk/build-properties/) config plugin:

<Step label="1">

Install `nxenv-build-properties`:

<Terminal cmd={['$ npx nxenv install nxenv-build-properties']} />
...
<Tabs tabs={['Android', 'iOS']}>
  <Tab>
    <Terminal
      cmd={['$ npx nxenv prebuild --clean', '$ npx nxenv run:android', '$ eas build -p android']}
    />
  </Tab>
  <Tab>
    <Terminal cmd={['$ npx nxenv prebuild --clean', '$ npx nxenv run:ios', '$ eas build -p ios']} />
  </Tab>
</Tabs>
...
<Collapsible summary="Known issues in Nxenv SDK libraries">

While there are likely incompatibilities we are not aware of, the following are the known issues with the New Architecture in the Nxenv SDK:

- **nxenv-dev-client**: we are currently investigating issues with stability on iOS, so you may encounter an increased crash rate in your development builds.
- **nxenv-notifications**: local notifications work, but remote notifications not yet fully supported.
- **nxenv-task-manager** and **nxenv-background-fetch**: not yet supported.
- **nxenv-location**: background location is not yet supported.
...
Refer to [React Native Directory](https://reactnative.directory/) a more complete list of libraries and their compatibility with the New Architecture. The following libraries were found to popular among Nxenv apps and are known to be incompatible:
...
- **react-native-fs**: use `nxenv-file-system` or [a fork of react-native-fs](https://github.com/birdofpreyru/react-native-fs) instead.
- **react-native-geolocation-service**: use `nxenv-location` instead.
