...
description: A guide on configuring Hermes for both Android and iOS in an Nxenv project.
---
...
The Hermes engine is the default JavaScript engine used by Nxenv and it is fully supported across all Nxenv tooling.
...
- If your app is built by `eas build`, `npx nxenv run:android` or `npx nxenv run:ios`, make sure it is a debug build.
- Internally, the app will establish a WebSocket connection, make sure your app is connected to the development server.

  - Try to reload the app by pressing <kbd>r</kbd> in the Nxenv CLI Terminal UI.
  - Test debugging availability by running the command: `curl http://127.0.0.1:8081/json/list` (adjust the `127.0.0.1:8081` to match your dev server URL). The HTTP response should be an array, as shown below. If it is an empty response, add either the `--localhost` or `--tunnel` flag to the `npx nxenv start` command.

  ```json
  [
    {
      "id": "0-2",
      "description": "host.exp.Nxenvnent",
...
Hermes supports [Chrome DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/v8/) to debug JavaScript in place by connecting to the engine running on the device, as opposed to remote debugging, which executes JavaScript within a desktop Chrome tab. Hermes apps use this debugging technique automatically when you open the debugger in Nxenv Go or a development build.
