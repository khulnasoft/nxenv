---
title: Localization
description: Learn about getting started and configuring localization in an Nxenv project using nxenv-localization.
---
...
This guide uses the `nxenv-localization` library for accessing user language settings and adding support for multiple languages. It uses `i18n-js` as an example to add multi-language support.

## Getting the user's language

Use the [`nxenv-localization`](/versions/latest/sdk/localization/) library to get the user's current language. Install the package by running the following command:

<Terminal cmd={['$ npx nxenv install nxenv-localization']} />

Then, you will be able to access localization methods and data in your app:

```tsx
import { getLocales } from 'nxenv-localization';
...
- Other preferences if there's no API to get the default value on platforms you want to support (check [`nxenv-localization`](/versions/latest/sdk/localization) API documentation for details)
...
<Terminal cmd={['$ npx nxenv install i18n-js']} />
...
<SnackInline label="Localization" dependencies={['nxenv-localization', 'i18n-js']}>
...
To enable RTL support, use the [`nxenv-localization`](/versions/latest/sdk/localization/#installation) config plugin and enable `extra.supportsRTL` property in app config:
...
This enables RTL when your app is loaded in Nxenv Go, in Nxenv dev Client, and in applications built using EAS Build or `npx nxenv prebuild`.

When an application starts, Nxenv checks if the current device locale should render in RTL layout to look correctly. For example, an app marked to support RTL in the app config file will render in RTL mode in Hebrew or Arabic locale.
...
This does not work in Nxenv Go, as Nxenv Go resets RTL preferences when opening the launcher or individual projects.

<SnackInline
label="Overriding RTL settings"
dependencies={['nxenv-updates', 'nxenv-constants']}
>
...
Nxenv uses `react-native-web` for running Nxenv projects in the browser. To make `react-native-web` automatically adapt to locale direction, add a `dir` property to your root `<View>` component.
...
Nxenv provides the `nxenv-localization` library to allow you to read the user's locale and other preferences. You can use synchronous `getLocales()` and `getCalendars()` methods to get the current locale settings of the user device:
...
There are a few limitations to keep in mind when relying on auto-detected locale preferences from `nxenv-localization`.
...
If you pass `default` as the locale string, the `Intl` API will use the device's locale, so you don't need to rely on `nxenv-localization` to get the current locale (such as `"en-US"`).
...
>
> For this you need to use `nxenv-localization`, JS code on the web, or third-party or custom native code on Android and iOS.
