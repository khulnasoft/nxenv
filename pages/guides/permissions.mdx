...
Permissions in standalone and [development builds](/develop/development-builds/introduction/) require native build-time configuration before they can be requested using runtime JavaScript code. This is not required when testing projects in the [Nxenv Go](https://nxenv.dev/go) app.
...
Many of these properties are also directly configurable using the [config plugin](/config-plugins/introduction/) properties associated with the library that adds them. For example, with [`nxenv-media-library`](/versions/latest/sdk/media-library) you can configure photo permission messages like this:
...
When testing in [Nxenv Go](https://nxenv.dev/go), you can delete the app and reinstall it by running `npx nxenv start` and pressing <kbd>i</kbd> or <kbd>a</kbd> in the [Nxenv CLI](/more/nxenv-cli/) Terminal UI.
