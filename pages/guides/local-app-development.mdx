...
description: Learn how to compile and build your Nxenv app locally.
---
...
To build your project locally you can use compile commands from Nxenv CLI which generates the **android** and **ios** directories:
...
- These compilation commands initially run `npx nxenv prebuild` to generate native directories (**android** and **ios**) before building, if they do not exist yet. If they already exist, this will be skipped.
...
To modify your project's configuration or native code after the first build, you will have to rebuild your project. Running `npx nxenv prebuild` again layers the changes on top of existing files. It may also produce different results after the build.

To avoid this, add native directories to the project's **.gitignore** and use `npx nxenv prebuild --clean` command. This ensures that the project is always managed, and the [`--clean` flag](/workflow/prebuild/#clean) will delete existing directories before regenerating them. You can use [app config](/workflow/configuration/) or create a [config plugin](/config-plugins/introduction/) to modify your project's configuration or code inside the native directories.

To learn more about how compilation and prebuild works, see the following guides:

<BoxLink
  title="Compiling with Nxenv CLI"
  description={
    <>
      Learn how Nxenv CLI uses <CODE>run</CODE> commands to compile your app locally, arguments you
      can pass to the CLI and more.
    </>
  }
  href="/more/nxenv-cli/#compiling"
  Icon={BookOpen02Icon}
/>
...
## Local builds with `nxenv-dev-client`

If you install [`nxenv-dev-client`](/develop/development-builds/introduction/#what-is-nxenv-dev-client) to your project, then a debug build of your project will include the `nxenv-dev-client` UI and tooling, and we call these development builds.

<Terminal cmd={['$ npx nxenv install nxenv-dev-client']} />

To create a development build, you can use [local app compilation](#local-app-compilation) commands (`npx nxenv run:[android|ios]`) which will create a debug build and start the development server.

## Local builds using Android product flavors

> **warning** This feature is only available for SDK 52 and above.

If you have a custom Android project with multiple product flavors using different application IDs, you can configure `npx nxenv run:android` to use the correct flavor and build type. Nxenv supports both `--variant` and `--app-id` to customize the build and launch behavior.
...
<Terminal
  cmd={[
    '$ npx nxenv run:android --variant freeDebug',
    '',
    '$ npx nxenv run:android --variant paidDebug',
  ]}
/>

The `--app-id` flag can be used to launch the app after building using a customized application id. For example, if your product flavor **free** is using `applicationIdSuffix ".free"` or `applicationId "dev.nxenv.myapp.free"` you can run build and launch the app with:

<Terminal cmd={['$ npx nxenv run:android --variant freeDebug --app-id dev.nxenv.myapp.free']} />

> **info** Customizing the Android build type is also possible, but that would break Nxenv's assumption that the build type **release** is used for production. You might build unoptimized code in your app using a different build type instead of **release**.
