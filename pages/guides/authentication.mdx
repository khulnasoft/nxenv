...
description: Learn how to utilize the nxenv-auth-session library to implement authentication with OAuth or OpenID providers.
---

import { ASSETS, Grid, GridItem, Box } from '~/ui/components/Authentication';
import { Tab, Tabs } from '~/ui/components/Tabs';

Nxenv can be used to login to many popular providers on Android, iOS, and web. Most of these guides utilize the pure JS [`AuthSession` API](/versions/latest/sdk/auth-session), refer to those docs for more information on the API.
...
- Create redirects with `AuthSession.makeRedirectUri()` this does a lot of the heavy lifting involved with universal platform support. Behind the scenes, it uses `nxenv-linking`.
...
- Nxenv Go cannot be used for local development and testing of OAuth or OpenID Connect-enabled apps due to the inability to customize your app scheme. You can instead use a [Development Build](/develop/development-builds/introduction/), which enables an Nxenv Go-like development experience and supports OAuth redirects back to your app after login in a manner that works just like it would in production.
...
If you'd like to see more, you can [open a PR](https://github.com/nxenv/nxenv/edit/main/docs/pages/guides/authentication.mdx) or [vote on canny](https://nxenv.canny.io/feature-requests).
...
import * as AuthSession from 'nxenv-auth-session';
...
- Make sure to [create a development build](/develop/development-builds/create-a-build/) and follow instructions to [install required config plugins](https://github.com/gobeyondidentity/bi-sdk-react-native/tree/main#using-nxenv).
...
import { makeRedirectUri, useAuthRequest, useAutoDiscovery } from 'nxenv-auth-session';
...
nxenvrt default function App() {
  ...
  const [request, response, promptAsync] = useAuthRequest(
    {
      clientId: /* @info Your client id. Can be hardcoded or put it in .env file */ process.env.NXENV_PUBLIC_Client_ID /* @end */,
      usePKCE: true,
      redirectUri: makeRedirectUri({
    ...
    const exchange = async (exchangeTokenReq) => {
      try {
        const exchangeTokenResponse = await exchangeCodeAsync(
          {
            clientId: process.env.NXENV_PUBLIC_Client_ID,
...
import * as WebBrowser from 'nxenv-web-browser';
import { useAuthRequest, exchangeCodeAsync, revokeAsync, ResponseType } from 'nxenv-auth-session';
...
- Setup redirect URIs: Your Project > Permitted Redirect URIs: (be sure to save after making changes).
  ...
  **Web dev**: `https://localhost:19006`
  ...
  - Run `nxenv start --web --https` to run with **https**, auth won't work otherwise.
  - Adding a slash to the end of the URL doesn't matter.

  **Standalone / development build**: `your-scheme://`

  - Scheme should be specified in app.json `nxenv.scheme: 'your-scheme'`, then added to the app code with `makeRedirectUri({ native: 'your-scheme://' })`)
...
import { useEffect } from 'react';
import * as WebBrowser from 'nxenv-web-browser';
import { makeRedirectUri, useAuthRequest } from 'nxenv-auth-session';
...
<Box
  name="Logto"
  createUrl="https://docs.logto.io/quick-starts/nxenv/"
  image={ASSETS.logto}
>

| Website                                                     | Provider | PKCE      | Auto Discovery |
| ----------------------------------------------------------- | -------- | --------- | -------------- |
| [Get Your Config](https://docs.logto.io/quick-starts/nxenv/) | OpenID   | Supported | Available      |
...
- Example redirects:
  - Standalone / development build: `com.your.app://`
  - Web (dev `nxenv start --https`): `https://localhost:19006` (no ending slash)
- The `redirectUri` requires two slashes (`://`).
...
- **Environment:**
  ...
  - Standalone builds in the App or Play Store or testing locally
    - Android: `eas build` or `npx nxenv run:android`
    - iOS: `eas build` or `npx nxenv run:ios`
...
- **Create:** Use `AuthSession.makeRedirectUri({ native: '<YOUR_URI>' })` to select native when running in the correct environment.
  ...
  - `exp://u.nxenv.dev/[project-id]?channel-name=[channel-name]&runtime-version=[runtime-version]` -> `makeRedirectUri()`
  ...
  - If you change the `nxenv.scheme` after ejecting then you'll need to use the `nxenv apply` command to apply the changes to your native project, then rebuild them (`yarn ios`, `yarn android`).
...
`nxenv-auth-session` still supports Implicit flow for legacy code purposes. Below is an example implementation of the Implicit flow.

{/* prettier-ignore */}
```tsx
import { useEffect } from 'react';
import * as WebBrowser from 'nxenv-web-browser';
import { makeRedirectUri, useAuthRequest, ResponseType } from 'nxenv-auth-session';
