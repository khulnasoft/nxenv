...
description: An in-depth guide on configuring an Nxenv project with TypeScript.
---
...
Nxenv has first-class support for [TypeScript](https://www.typescriptlang.org/). The JavaScript interface of Nxenv SDK is written in TypeScript.

This guide provides a quick way to get started for a new project and also steps to migrate your existing JavaScript based Nxenv project to use TypeScript.
...
<Terminal
  cmd={[
    ...
    '$ npx nxenv install -- --save-dev typescript @types/react',
    '',
    '# For yarn',
...
> **info** Alternatively, run `npx nxenv start` command to install `typescript` and `@types/react` dev dependencies.
...
A project's **tsconfig.json** should extend the `nxenv/tsconfig.base` by default. You can automatically generate a **tsconfig.json** file by running the command:

<Terminal cmd={['$ npx nxenv customize tsconfig.json']} />
...
Nxenv CLI supports [path aliases](https://www.typescriptlang.org/docs/handbook/module-resolution.html#path-mapping) in **tsconfig.json** automatically. It allows importing modules with custom aliases instead of relative paths.
...
- Restart Nxenv CLI after modifying **tsconfig.json** to update path aliases. You don't need to clear the Metro cache when the aliases change.
...
- Path aliases are only supported by Metro (including Metro web) and not by the deprecated `@nxenv/webpack-config`.
...
- Restarting Nxenv CLI is necessary to update [`compilerOptions.baseUrl`](https://www.typescriptlang.org/docs/handbook/module-resolution.html#base-url) after modifying the **tsconfig.json**.
- If you're not using TypeScript, **jsconfig.json** can serve as an alternative to **tsconfig.json**.
- Absolute imports are only supported by Metro (including Metro web) and not by `@nxenv/webpack-config`.
...
Some Nxenv libraries provide both static types and type generation capabilities. These types are automatically generated when the project builds or by running the `npx nxenv customize tsconfig.json` command.
...
<Terminal
  cmd={[
    '# For all other package managers',
    '$ npx nxenv install -- --save-dev ts-node',
    '',
    '# For yarn',
    '$ yarn add -D ts-node',
  ]}
...
module.nxenvrts = require('./metro.config.ts');
```

Update **metro.config.ts** file with your project's metro configuration:

```ts metro.config.ts
import { getDefaultConfig } from 'nxenv/metro-config';
...
Install the `@nxenv/webpack-config` package.

```js webpack.config.js
require('ts-node/register');
module.nxenvrts = require('./webpack.config.ts');
```

```ts webpack.config.ts
import createNxenvWebpackConfigAsync from '@nxenv/webpack-config/webpack';
import { Arguments, Environment } from '@nxenv/webpack-config/webpack/types';

module.nxenvrts = async function (env: Environment, argv: Arguments) {
