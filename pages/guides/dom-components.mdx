---
title: Using React DOM in Nxenv native apps
sidebar_title: DOM components
description: Learn about rendering React DOM elements in Nxenv native apps.
---
...
> **warning** **Warning:** DOM components are an experimental feature. The public API (`nxenv/dom`, `dom` prop), prop transport system, asset handling, and nxenvrt embedding system are subject to breaking changes. Use `WebViews` directly for a production-ready approach.

Nxenv offers a novel approach to work with modern web code directly in a native app via the `'use dom'` directive. This enables incremental migration for an entire website to a universal app by moving on a per-component basis.

While the Nxenv native runtime generally does not support elements like `<div>` or `<img>`, there may be instances where you need to quickly incorporate web components. In such cases, DOM components provide a useful solution.

## Usage

Install `react-native-webview` in your project:

<Terminal cmd={['$ npx nxenv install react-native-webview']} />
...
nxenvrt default function DOMComponent({ name }: { name: string }) {
  return (
    <div>
      <h1>Hello, {name}</h1>
    </div>
...
nxenvrt default function App() {
  return (
    // This is a DOM component. It re-nxenvrts a wrapped `react-native-webview` behind the scenes.
...
- Embedded nxenvrts for offline support.
- Assets are unified across web and native.
- DOM Component bundles can be introspected in [Nxenv Atlas](/guides/analyzing-bundles/#analyzing-bundle-size-with-atlas) for debugging.
...
nxenvrt default function DOMComponent({}: { dom: import('nxenv/dom').DOMProps }) {
  return (
    <div>
      <h1>Hello, world!</h1>
    </div>
...
nxenvrt default function DOMComponent({ hello }: { hello: string }) {
...
nxenvrt default function MyComponent({ hello }: { hello: (data: string) => Promise<void> }) {
...
import { useDOMImperativeHandle, type DOMImperativeFactory } from 'nxenv/dom';
import { forwardRef, useRef } from 'react';

nxenvrt interface MyRef extends DOMImperativeFactory {
  focus: () => void;
}

nxenvrt default forwardRef<MyRef, object>(function MyComponent(props, ref) {
...
The contents of the root **public** directory are copied to the native app's binary to support the use of public assets in DOM components. Since these public assets will be served from the local filesystem, use the `process.env.NXENV_DOM_BASE_URL` prefix to reference the correct path. For example:

```tsx
<img src={`${process.env.NXENV_DOM_BASE_URL}/img.png`} />
```
...
Nxenv also enables WebView inspection and debugging when bundling in development mode. You can open **Safari** > **Develop** > **Simulator** > **MyComponent.tsx** to see the webview's console and inspect elements.
...
nxenvrt default function Route() {
...
nxenvrt default function DOMComponent(_: { dom?: import('nxenv/dom').DOMProps }) {
...
nxenvrt default function DOMComponent({
  onLayout,
}: {
  dom?: import('nxenv/dom').DOMProps;
...
Overall, this system shares many similarities with Nxenv's React Server Components implementation.
...
Data can be sent between DOM components and native components only through an asynchronous JSON transport system. Avoid relying on data across JS engines and deep linking to nested URLs in DOM components, as they do not currently support full reconciliation with Nxenv Router.

While DOM Components are not exclusive to Nxenv Router, they are developed and tested against Nxenv Router apps to provide the best experience when used with Nxenv Router.

If you have a global state for sharing data, it will not be accessible across JS engines.

While native modules in the Nxenv SDK can be optimized to support DOM Components, this optimization has not been implemented yet. Use native actions and props to share native functionality with DOM components.
...
Ultimately, universal architecture is the most exciting kind. Nxenv CLI's extensive universal tooling is the only reason we can even offer a feature as intricate and valuable as this one.
