...
You can include an iOS privacy manifest by using the `privacyManifests` field under `nxenv.ios` in your app config.
...
> Available with SDK 50 and above. Make sure you have updated your Nxenv SDK libraries to the latest versions for your SDK version using `npx nxenv install --fix`. For SDK versions below 50, use the [`nxenv-privacy-manifest-polyfill-plugin`](https://www.npmjs.com/package/nxenv-privacy-manifest-polyfill-plugin).

<ConfigReactNative>

You can include an iOS privacy manifest in a bare Nxenv app by creating a **PrivacyInfo.xcprivacy** file using Xcode and adding it to your iOS app target.
...
### Including required reasons for Nxenv SDK packages and other third-party libraries

As of now, Apple does not correctly parse all the **PrivacyInfo** files included by static CocoaPods dependencies (such as Nxenv SDK packages and other ecosystem libraries). You may need to include the required reasons for the APIs used by those dependencies in your app's **PrivacyInfo.xcprivacy** file or the configuration in the **app.json**.

All Nxenv SDK packages that use "required reason" APIs file have a **PrivacyInfo** file included in the package directory. Here's [an example file](https://github.com/nxenv/nxenv/blob/main/packages/nxenv-application/ios/PrivacyInfo.xcprivacy) included with the `nxenv-application` library.
