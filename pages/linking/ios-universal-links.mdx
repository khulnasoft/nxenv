...
description: Learn how to configure iOS Universal Links to open your Nxenv app from a standard web URL.
---
...
If you're using Nxenv Router to build your website (or any other modern React framework such as Remix, Next.js, and so on), create the AASA file at **public/.well-known/apple-app-site-association**. For legacy Nxenv webpack projects, create the file at **web/.well-known/apple-app-site-association**.
...
- Any links to `https://www.myapp.io/records/*` (with wildcard matching for the record ID) should be opened directly by the app with a matching bundle identifier on an iOS device. It is a combination of the [Apple Team ID](https://nxenv.fyi/apple-team) and the bundle identifier.
...
For example, if an associated website is `https://nxenv.dev/`, the `applinks` is:

```json app.json
{
  "nxenv": {
    "ios": {
      "associatedDomains": ["applinks:nxenv.dev"]
    }
  }
}
```
...
If you're not using EAS or [Continuous Native Generation](/workflow/continuous-native-generation) (`npx nxenv prebuild`), you have to [manually configure](/build-reference/ios-capabilities#manual-setup) the **Associated Domains** capability for your bundle identifier.
...
<array>
  <string>applinks:nxenv.dev</string>
</array>
```
...
If you're building a [statically rendered website with Nxenv Router](/router/reference/static-rendering), then add the HTML tag to the `<head>` component in your [**app/+html.js** file](/router/reference/static-rendering#root-html).
...
Nxenv CLI enables you to test iOS Universal Links without deploying a website. Utilizing the [`--tunnel`](/more/nxenv-cli/#tunneling) functionality, you can forward your dev server to a publicly available HTTPS URL.

<Step label="1">

Set the environment variable `NXENV_TUNNEL_SUBDOMAIN=my-custom-domain` where `my-custom-domain` is a unique string that you use during development. This ensures that your tunnel URL is consistent across dev server restarts.
