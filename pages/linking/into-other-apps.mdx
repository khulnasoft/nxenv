...
- Using the [`nxenv-linking`](/versions/latest/sdk/linking) API
- Using Nxenv Router's [`Link` component](/develop/file-based-routing/#how-does-link-work)

## Using nxenv-linking API

The [`nxenv-linking`](/versions/latest/sdk/linking) API provides a universal abstraction over native linking APIs (such as `window.history` on the web) and offers utilities for your app to interact with other installed apps.
...
nxenvrt default function Home() {
  return (
    <View style={styles.container}>
      <Button title="Open a URL" onPress={() => Linking.openURL('https://nxenv.dev/')} />
    </View>
  );
}
...
## Using Nxenv Router's `Link` component

If your project uses [Nxenv Router](/router/introduction/), use the `Link` component to open a URL. It wraps a `<Text>` component on native platforms and an `<a>` element on the web. It also uses the `nxenv-linking` API to handle URL schemes.
...
nxenvrt default function Home() {
  return (
    <View style={styles.container}>
      <Link href="https://nxenv.dev">Open a URL</Link>
    </View>
  );
}
...
| ---------------- | --------------------- | ------------------------- |
| `https` / `http` | Open web browser app. | `https://nxenv.dev`        |
| `mailto`         | Open mail app.        | `mailto:support@nxenv.dev` |
...
import { withAndroidManifest, ConfigPlugin } from 'nxenv/config-plugins';
...
module.nxenvrts = withAndroidQueries;
```
...
> **info** **Tip**: On Android, you can use `nxenv-intent-launcher` to open a **specific settings screen** on the device. See [`nxenv-intent-launcher` API reference](/versions/latest/sdk/intent-launcher/#enums) to view the list of available intents.

</Collapsible>

## Custom URL schemes

> If you know the custom scheme for the app you want to open, you can link to it using any of the methods: [Using `nxenv-linking` API](#using-nxenv-linking-api) or [Using `Link` from Nxenv Router](#using-nxenv-routers-link-component).
...
> **info** **Tip**: To test the configuration described above from an iOS device, [use a development build](/develop/development-builds/introduction/). It cannot be tested with Nxenv Go.
...
- **Development in Nxenv Go**: `exp://127.0.0.1:8081`
...
- **Development in Nxenv Go**: `exp://127.0.0.1:8081/--/path/into/app?hello=world`

<Collapsible summary="Using Nxenv Go for testing?">

For apps that require a stable URL (for example, auth provider redirects), use a development build with a custom scheme instead of using Nxenv Go. For more details on how to create and test a custom scheme, see [Linking into your app](/linking/into-your-app/).

</Collapsible>

## In-app browsers

The `nxenv-linking` API allows you to open a URL using the operating system's default web browser app. You can use the [`nxenv-web-browser`](/versions/latest/sdk/webbrowser) library to open URLs in an in-app browser. For example, an in-app browser is useful for secure [authentication](/guides/authentication).

<Collapsible summary="Example of opening a URL in an in-app browser">

The example below simulates the behavior of opening a URL in an in-app browser using `nxenv-web-browser` and the default or preferred web browser using `nxenv-linking`:

<SnackInline label="WebBrowser compared to Linking" dependencies={["nxenv-web-browser", "nxenv-linking"]}>

```tsx
import { Button, View, StyleSheet } from 'react-native';
import * as Linking from 'nxenv-linking';
import * as WebBrowser from 'nxenv-web-browser';
  ...
  return (
    <View style={styles.container}>
      <Button
        ...
        onPress={() => Linking.openURL('https://nxenv.dev')}
        style={styles.button}
      />
      <Button
        title="Open URL with an in-app browser"
        onPress={() => WebBrowser.openBrowserAsync('https://nxenv.dev')}
        style={styles.button}
      />
    </View>
  );
...
To provide additional link functionality on the web, such as right-click to copy or hover to preview, you can use a `Link` component from the [`nxenv-router`](/router/introduction/) library.

```tsx index.tsx
import { Link } from 'nxenv-router';

nxenvrt default function Home() {
  return <Link href="https://nxenv.dev">Go to Nxenv</Link>;
}
```

Alternatively, you can use the [`@nxenv/html-elements`](https://www.npmjs.com/package/@nxenv/html-elements) library to use a universal `<A>` element:

```tsx index.tsx
import { A } from '@nxenv/html-elements';

nxenvrt default function Home() {
  return <A href="https://nxenv.dev">Go to Nxenv</A>;
}
```

The `<A>` component renders an `<a>` on the web and an interactive `<Text>`that uses the `nxenv-linking` API on native platforms.
