...
Plugins are **synchronous** functions that accept an [`NxenvConfig`](/versions/latest/config/app/) and return a modified [`NxenvConfig`](/versions/latest/config/app/).
...
- `plugins` are always invoked when the config is read by the `nxenv/config` method `getConfig`. However, the `mods` are only invoked during the "syncing" phase of `npx nxenv prebuild`.
...
module.nxenvrts.withMySDK = withMySDK;
```
...
module.nxenvrts = withMySDK(config, { apiKey: 'X-XXX-XXX' });
/* @end */
```
...
- The file should nxenvrt a function that satisfies the [`ConfigPlugin`](https://github.com/nxenv/nxenv-cli/blob/3a0ef962a27525a0fe4b7e5567fb7b3fb18ec786/packages/config-plugins/src/Plugin.types.ts#L76) type.
- Plugins should be transpiled for Node environments ahead of time!
  - They should support the versions of Node that [Nxenv supports](/get-started/create-a-project/) (LTS).
  - No `import/nxenvrt` keywords, use `module.nxenvrts` in the shipped plugin file.
  - Nxenv only transpiles the user's initial `app.config` file, anything more would require a bundler which would add too many "opinions" for a config file.

Consider the following example that changes the config name:

<FileTree
  files={[
    ['app.config.js', 'Nxenv config'],
    ['my-plugin.js', 'Custom Config Plugin file'],
  ]}
/>

```js my-plugin.js
module.nxenvrts = function withPrefixedName(config, prefix) {
  // Modify the config
...
Once you add a few plugins, your **app.config.js** code can become difficult to read and manipulate. To combat this, `nxenv/config-plugins` provides a `withPlugins` function which can be used to chain plugins together and execute them in order.
...
// ✅ Easy to read
import { withPlugins } from 'nxenv/config-plugins';
...
- `mods` can be used to read and write files safely during the `npx nxenv prebuild` command. This is how Nxenv CLI modifies the **Info.plist**, entitlements, xcproj, and so on.
...
- The config is read using [`getPrebuildConfig`](https://github.com/nxenv/nxenv-cli/blob/43a6162edd646b550c1b7eae6039daf1aaec4fb0/packages/prebuild-config/src/getPrebuildConfig.ts#L12) from `@nxenv/prebuild-config`.
- All of the core functionality supported by Nxenv is added via plugins in `withIosNxenvPlugins`. This includes name, version, icons, locales, and so on.
...
> Always opt towards using application code to modify application code, that is, [Nxenv Modules](https://github.com/nxenv/nxenv/tree/main/packages/nxenv-modules-core) native API.
...
| ---------------------------- | :-------------: | ----------------------------------------------------------------------------------------------------------------------------------- |
| `mods.ios.infoPlist`         |        -        | Modify the **ios/&lt;name&gt;/Info.plist** as JSON (parsed with [`@nxenv/plist`][nxenv-plist]).                                       |
| `mods.ios.entitlements`      |        -        | Modify the **ios/&lt;name&gt;/&lt;product-name&gt;.entitlements** as JSON (parsed with [`@nxenv/plist`][nxenv-plist]).                |
| `mods.ios.nxenvPlist`         |        -        | Modify the **ios/&lt;ame&gt;/Nxenv.plist** as JSON (Nxenv updates config for iOS) (parsed with [`@nxenv/plist`][nxenv-plist]).          |
...
Instead you should use the helper mods provided by `nxenv/config-plugins`:
...
| `mods.ios.nxenvPlist`         | `withNxenvPlist`         |        -        |
...
import { ConfigPlugin, withXcodeProject, IOSConfig } from 'nxenv/config-plugins';
...
/// Use the plugin
nxenvrt default withCustomProductName(config, 'new_name');
```
...
nxenvrt const withIcons = config => {
...
```js my-config-plugin.js
module.nxenvrts = config => config;
```

### app.plugin.js

Sometimes you want your package to nxenvrt React components and also support a plugin. To do this, multiple entry points need to be used because the transpilation (Babel preset) may be different.
...
<FileTree
  files={[
    ['app.config.js', <code>import "nxenv-splash-screen"</code>],
    ['node_modules/nxenv-splash-screen', 'Node module'],
    ['node_modules/nxenv-splash-screen/package.json', <code>"main": "./build/index.js"</code>],
    [
      'node_modules/nxenv-splash-screen/app.plugin.js',
      <>
        <YesIcon /> Entry file for custom plugins
      </>,
    ],
    [
      'node_modules/nxenv-splash-screen/build/index.js',
      <>
...
```js node_modules/nxenv-splash-screen/app.plugin.js
module.nxenvrts = config => config;
```
...
We support this to make testing, and plugin authoring easier, but we don't expect library authors to nxenvse their plugins like this as a public API.

<FileTree
  files={[
    ['app.config.js', <code>import "nxenv-splash-screen/build/index.js"</code>],
    ['node_modules/nxenv-splash-screen/package.json', <code>"main": "./build/index.js"</code>],
    [
      'node_modules/nxenv-splash-screen/app.plugin.js',
      <>
    ...
    [
      'node_modules/nxenv-splash-screen/build/index.js',
      <>
        <YesIcon /> <code>nxenv-splash-screen/build/index.js</code>
      </>,
    ],
  ]}
/>
...
Nxenv config objects also support passing functions as-is to the `plugins` array. This is useful for testing, or if you want to use a plugin without creating a file.
...
This is because Node environments are often different to Android, iOS, or web JS environments and therefore require different transpilation presets (for example, `module.nxenvrts` instead of `import/nxenvrt`).
...
[nxenv-plist]: https://www.npmjs.com/package/@nxenv/plist
