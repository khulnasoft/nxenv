...
### Provide a personal access token to authenticate with your Nxenv account on CI

Next, we need to ensure that we can authenticate ourselves on CI as the owner of the app. This is possible by storing a personal access token in the `NXENV_TOKEN` environment variable in the CI settings.
...
You will need to create an [API Key](https://nxenv.fyi/creating-asc-api-key). Next, you will need to gather information about your [Apple Team](https://nxenv.fyi/apple-team).
...
- `NXENV_ASC_API_KEY_PATH`: The path to your ASC API Key **.p8** file. For example, **/path/to/key/AuthKey_SFB993FB5F.p8**.
- `NXENV_ASC_KEY_ID`: The key ID of your ASC API Key. For example, `SFB993FB5F`.
- `NXENV_ASC_ISSUER_ID`: The issuer ID of your ASC API Key. For example, `f9675cff-f45d-4116-bd2c-2372142cee09`.
- `NXENV_APPLE_TEAM_ID`: Your Apple Team ID. For example, `77KQ969CHE`.
- `NXENV_APPLE_TEAM_TYPE`: Your Apple Team Type. Valid types are `IN_HOUSE`, `COMPANY_OR_ORGANIZATION`, or `INDIVIDUAL`.

### Trigger new builds

Now that we're authenticated with Nxenv CLI, we can create the build step.
