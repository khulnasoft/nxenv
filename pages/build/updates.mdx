...
EAS Build includes some special benefits for [`nxenv-updates`](/versions/latest/sdk/updates/) library. In particular, you can configure the [`channel`](/eas-update/how-it-works/#distributing-builds) property in **eas.json** and EAS Build will take care of updating it in your native project at build time.

This document covers concerns specific to using `nxenv-updates` library with EAS Build. For more general information about configuring the library with EAS Update, see [Getting started with EAS Update ](/eas-update/getting-started/).
...
Updates published with the `runtimeVersion` field can't be loaded in Nxenv Go. Instead, you should use [`nxenv-dev-client`](/versions/latest/sdk/dev-client/) to create a development build.
