...
Your project will always run in either **development** or **production** mode. By default, running your project locally with `npx nxenv start` runs it in development mode, whereas a published project (with `eas update`), or any standalone app, will run in production mode.
...
<Terminal cmd={['$ npx nxenv start --no-dev --minify']} />
...
To completely compile your app for production see [Compiling Android](/more/nxenv-cli/#compiling-android) and [Compiling iOS](/more/nxenv-cli/#compiling-ios).
