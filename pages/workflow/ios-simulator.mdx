...
Developing your app directly on a computer can be more convenient than constantly interacting with an iPhone or iPad, especially when network conditions are slow or when a [tunnel connection](/more/nxenv-cli/#tunneling) is required due to LAN limitations.
...
Run your app with `npx nxenv start` and press <kbd>i</kbd> from the command line.
...
You can also press <kbd>shift</kbd> + <kbd>i</kbd> in the Nxenv CLI to interactively select a simulator to open.

<ContentSpotlight
  alt="List of iOS Simulators in the Nxenv CLI UI."
...
You can use the Nxenv Orbit app which allows launching builds and simulator management with one click from the menu bar on macOS.

<BoxLink
  title="Use Nxenv Orbit"
  description="Learn more about how to use Nxenv Orbit."
  href="/build/orbit/"
  Icon={BookOpen02Icon}
/>
...
Although the iOS Simulator is great for rapid development, it does come with a few limitations. We'll list out a few of the main differences that affect Nxenv APIs below. However, see [Apple's documentation](https://help.apple.com/simulator/mac/current/#/devb0244142d) for more details.
...
You can use this menu to open any version of the simulator. You can also open multiple simulators at the same time, however, Nxenv CLI will always target the most recently opened simulator.

### Simulator opened but the Nxenv Go app isn't opening inside of it

The first time you install the app in the simulator, iOS will ask if you'd like to open the Nxenv Go app. You may need to interact with the simulator (click around, drag something) for this prompt to show up, then press **OK**.

### How do I force an update to the latest version?

Create a project with the desired SDK version and open it in a simulator to install a particular version of Nxenv Go.

<Terminal
  cmd={[
    '# Bootstrap an SDK 51 project',
    '$ npx create-nxenv-app --template blank@51',
    '',
    '# Open the app on a simulator to install the required Nxenv Go app',
    '$ npx nxenv start --ios',
  ]}
/>

### Nxenv CLI is printing an error message about `xcrun`, what do I do?

For miscellaneous errors, try the following:

- Manually uninstall Nxenv Go on your simulator and reinstall by pressing <kbd>shift</kbd> + <kbd>i</kbd> in the Nxenv CLI Terminal UI and selecting the desired simulator.
