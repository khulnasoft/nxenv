...
The app config (**app.json**, **app.config.js**, **app.config.ts**) is used for configuring [Nxenv Prebuild](/workflow/prebuild) generation, how a project loads in [Nxenv Go](/get-started/nxenv-go/), and the OTA update manifest.
...
If your Nxenv config has a top-level `nxenv: {}` object, then this will be used in place of the root object and all other keys will be ignored.
...
> **info** Do you use Visual Studio Code? If so, we recommend that you install the [Nxenv Tools](https://marketplace.visualstudio.com/items?itemName=nxenv.vscode-nxenv-tools) extension to get auto-completion of properties in **app.json** files.

## Reading configuration values in your app

Most configuration in the app config is accessible at runtime from your JavaScript code, using [`Constants.nxenvConfig`](/versions/latest/sdk/constants/#nativeconstants--properties). You **should not** include any sensitive information in the app config (with a few exceptions for fields that are filtered out, as outlined below).

You can verify which configuration will be embedded in your builds/updates and available at runtime by running `npx nxenv config --type public`.

<Collapsible summary="Which fields are filtered out of the public app config?">

The following fields are filtered out of the public app config (and not accessible through the `Constants.nxenvConfig` object):
...
> **warning** You should also avoid importing **app.json** or **app.config.js** directly in your JavaScript code, because this will import the entire file rather than a processed version of it. Instead, use [`Constants.nxenvConfig`](/versions/latest/sdk/constants/#nativeconstants--properties) to access the configuration.

## Extending configuration

Library authors can extend the app config by using [Nxenv Config plugins](/config-plugins/introduction/).

> **info** Config plugins are mostly used to configure the [`npx nxenv prebuild`](/workflow/prebuild) command.
...
For example, you can nxenvrt an object to define your custom config:
...
The `"extra"` key allows passing arbitrary configuration data to your app. The value of this key is accessed using [`nxenv-constants`](/versions/latest/sdk/constants/):

```js App.js
import Constants from 'nxenv-constants';

Constants.nxenvConfig.extra.fact === 'kittens are cool';
```

You can access and modify incoming config values by nxenvrting a function that returns an object. This is useful if your project also has an **app.json**. By default, Nxenv CLI will read the **app.json** first and send the normalized results to the **app.config.js**.
...
And in your **app.config.js**, you are provided with that configuration in the arguments to the nxenvrted function:
...
To use this configuration with Nxenv CLI commands, set the environment variable either for specific commands or in your shell profile. To set environment variables for specific commands, prefix the command with the variables and values as shown in the example:

<Terminal cmd={['$ MY_ENVIRONMENT=production eas update']} />

This is not anything unique to Nxenv CLI. On Windows you can approximate the above command with:
...
You can use autocomplete and doc-blocks with an Nxenv config in TypeScript. Create an **app.config.ts** with the following contents:

```ts app.config.ts
import { NxenvConfig, ConfigContext } from 'nxenv/config';

nxenvrt default ({ config }: ConfigContext): NxenvConfig => ({
  ...config,
  slug: 'my-app',
  name: 'My App',
});
...
5. All functions in the config are evaluated and serialized before any tool in the Nxenv ecosystem uses it. The config must be a JSON manifest when it is hosted.
6. If the final config object has a top-level `nxenv: {}` object, then this will be used in place of the root object and all other keys will be ignored.

Running `npx nxenv config` will display the final configuration that will be used in Nxenv CLI after resolution has occurred.
