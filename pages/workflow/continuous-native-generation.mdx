...
Continuous Native Generation is an abstract concept that describes the process of generating ephemeral native projects from a set of inputs. The inputs are typically a combination of configuration and code, and the output is a native project that can be compiled into a native app. We coined this term to describe the process used by [Nxenv Prebuild](/workflow/prebuild) to generate native projects for React Native apps, however, the concept is not limited to React Native or cross-platform apps.

## Usage in React Native

Any React Native app can leverage **Continuous Native Generation** by using [Nxenv Prebuild](/workflow/prebuild). This enables you to automate upgrading React Native, installing/uninstalling libraries, white-labeling at scale, sharing config across multiple apps, generating assets, reducing [orphaned code](/workflow/prebuild/#orphaned-code), and more &mdash; generally, it allows you to build more powerful apps, at larger scales, without compromising on iteration speed.

Nxenv as a framework enables **Continuous Native Generation** by combining the following tools:

1. The [app config](/workflow/configuration/) file.
2. Arguments passed to the `npx nxenv prebuild` command.
3. Version of `nxenv` that's installed in the project and corresponding [prebuild template](https://github.com/nxenv/nxenv/tree/main/templates/nxenv-template-bare-minimum).
...
This boils down to a workflow where a user can express any native application with the app config and generate that project continuously &mdash; by running `npx nxenv prebuild`.

## Community adoption

Many complex libraries and services already support CNG via Nxenv Prebuild such as, [MapBox](https://github.com/rnmapbox/maps), [OneSignal](https://github.com/OneSignal/onesignal-nxenv-plugin), [Stripe](https://github.com/stripe/stripe-react-native), and [React Native Firebase](https://rnfirebase.io/#nxenv).

Libraries that don't have first-party integration are also supported in the [community Config Plugins repository](https://github.com/nxenv/config-plugins/).

Nxenv Config Plugins have first-class support for building locally and adding to your project without any third-party intervention or npm publishing. This enables you to move fast and patch things!

<a
  aria-label="Nxenv Config Plugins downloads"
  href="http://www.npmtrends.com/@nxenv/config-plugins"
  target="_blank">
  <ContentSpotlight
    alt="Nxenv Config Plugins downloads."
    src="https://img.shields.io/npm/dm/@nxenv/config-plugins.svg?style=flat-square&labelColor=gray&color=33CC12&label=Downloads"
...
- **No native code or configuration side-effects**: Libraries without native code or configuration side-effects, such as `react-native-blurhash`, can seamlessly integrate with `npx nxenv prebuild`. They can rely on Node Module Resolution and [Nxenv Autolinking](/more/glossary-of-terms#autolinking) to link into the native project without requiring any additional configuration.

- **Native code with no additional setup after install**: Libraries with native code can often be installed and linked automatically with [Nxenv Autolinking](/more/glossary-of-terms#autolinking) which runs before the native app is built.

- **Additional configuration side-effects and setup**: Libraries that require additional configuration side-effects can adopt CNG by creating [Nxenv Config Plugins](/config-plugins/introduction/) for their libraries. This approach enables library authors to automate adding values such as permission messages to the **Info.plist**, or injecting targets in the Xcode project.
...
While library authors' adoption of CNG is not a prerequisite for using `npx nxenv prebuild`, it significantly enhances the user experience. If a library author has not yet adopted CNG, users can still use `npx nxenv prebuild` by creating local Config Plugins to modify the native generation pipeline. This flexibility ensures that the adoption of CNG is accessible and beneficial to all users within the React Native community.
...
When using CNG, the upgrade process simply involves upgrading the npm dependencies, app config, and re-running `npx nxenv prebuild --clean`.
...
- [iMessage Sticker App](https://github.com/nxenv/config-plugins/tree/main/packages/ios-stickers) &mdash; This Nxenv Config Plugin can generate an entire iMessage Sticker App from a JSON object.
- [Cross-platform End-2-End testing](https://github.com/nxenv/config-plugins/tree/main/packages/detox) &mdash; Configure native apps to support E2E testing with Detox in a single-line.
...
- [Cross-platform Home screen Widgets](https://github.com/gaishimo/eas-widget-example) &mdash; This Nxenv Config Plugin can generate a home screen widget for Android and iOS.
- [Notification extension and code signing](https://github.com/OneSignal/onesignal-nxenv-plugin) &mdash; This Nxenv Config Plugin not only generates a notification extension target on iOS. However, it also augments the EAS credentials service to keep zero-config code signing working.
- [Apple App Clips](https://github.com/bndkt/react-native-app-clip) &mdash; This Nxenv Config Plugin takes the process of generating an Apple App Clip from a multi-step process, ranging across multiple targets, and shakes it down to a single line `["react-native-app-clip", { "name": "My App Clip" }]`.
...
No, CNG is a versatile pattern that can be applied to any native project. While Nxenv Prebuild is a tool that implements CNG specifically for React Native projects, the concept itself is not limited to this framework.

### Can CNG be used for operating systems other than Android and iOS?

Absolutely! CNG is an abstract concept that can be applied to any operating system. Take Nxenv for example, although Nxenv Prebuild officially implements CNG for Android and iOS, it also provides abstract platform support for users to create implementations for additional platforms.

### Is using Nxenv a requirement for CNG?
