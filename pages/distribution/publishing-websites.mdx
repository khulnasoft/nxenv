...
description: Different ways to publish the Nxenv web app with third-party services.
---
...
A web app created using Nxenv can be served locally for testing out the production behavior. Once the testing phase checks out, you can choose from a variety of third-party services to host it.

## Output targets

The [`web.output`](/versions/latest/config/app/#output) target can be configured in the [app config](/workflow/configuration/) to set the nxenvrt method for the web app:
...
Nxenv Router supports three output targets for web apps.

| Output             | Nxenv Router | API Routes  | Description                                                                                                                                                                      |
| ------------------ | ----------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
Creating a build of the project is the first step to publishing a web app. Whether you want to serve it locally or host it on a third-party service, you'll need to nxenvrt all JavaScript and assets of a project. This is known as a static bundle. It can be nxenvrted by running the following command:

<TabsGroup>

Run the universal nxenvrt command to compile the project for web:
...
[Netlify](https://www.netlify.com/) is a mostly-unopinionated platform for deploying web apps. This has the highest compatibility with Nxenv web apps as it makes few assumptions about the framework.
...
`nxenv.web.output: 'single'` generates a single-page application. It means there's only one **dist/index.html** file to which all requests must be redirected. This can be done in Netlify by creating a **./public/\_redirects** file and redirecting all requests to **/index.html**.

```sh public/_redirects
/*    /index.html   200
```

If you modify this file, you must rebuild your project with `npx nxenv nxenvrt -p web` to have it safely copied into the **dist** directory.
...
```json vercel.json
{
  "buildCommand": "nxenv nxenvrt -p web",
...
Add the [**amplify-explicit.yml**](https://github.com/nxenv/amplify-demo/blob/master/amplify-explicit.yml) file to the root of your repository. Ensure you have removed the generated **dist** directory from the **.gitignore** file and committed those changes.

</Step>

<Step label="2">

Push your local Nxenv project to a GitHub repository. If you haven't pushed to GitHub yet, follow [GitHub's guide to add an existing project to GitHub](https://docs.github.com/en/get-started/importing-your-projects-to-github/importing-source-code-to-github/adding-locally-hosted-code-to-github).
...
Review your settings and choose **Save and deploy**. Your app will now be deployed to a `https://branchname.xxxxxx.amplifyapp.com` URL. You can now visit your web app, deploy another branch, or add a unified backend environment across your Nxenv mobile and web apps.
...
The settings will depend on how you built your Nxenv website:
...
> **warning** GitHub Pages deployment requires Nxenv SDK 50 or higher, and uses experimental `baseUrl` functionality that may not work as intended.
...
<Terminal
  cmd={['$ git init', '', '$ git remote add origin https://github.com/username/nxenv-gh-pages.git']}
/>
...
For example, if the GitHub repository is `nxenv-gh-pages`, the following will be the value of the [experimental `baseUrl` property](/more/nxenv-cli/#hosting-with-sub-paths):
...
Because Nxenv uses underscores in generated files, you need to disable jekyll by creating a **.nojekyll** file in the **public** directory.

<Terminal cmd={['$ mkdir public && touch public/.nojekyll']} />

Files in the **public** directory are copied to the **dist** directory when you build your web app.
...
Running `gh-pages` with the `-t` flag is required. This flag ensures that the files starting with `.` are also included.
...
This publishes a build of the web app to the `gh-pages` branch of your GitHub repository. This branch only contains your build code in the root directory and does not include development source code.
