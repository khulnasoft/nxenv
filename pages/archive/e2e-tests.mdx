...
This guide explains how to run E2E tests with Detox in a bare workflow project. You can use [`@config-plugins/detox`](https://github.com/nxenv/config-plugins/tree/main/packages/detox) for a managed project, but you may need to adjust some of the instructions in this guide to do so.
...
Let's start by initializing a new Nxenv project, installing and configuring `@config-plugins/detox`, and running `npx nxenv prebuild` to generate the native projects.
...
<Terminal
  cmd={[
    ...
    '$ npx create-nxenv-app eas-tests-example',
...
<BoxLink
  ...
  href="https://github.com/nxenv/eas-tests-example"
  Icon={GithubIcon}
/>
...
Development builds typically display an onboarding welcome screen when an app is launched for the first time, which intends to provide context about the `nxenv-dev-client` UI for developers. However, it can interfere with your E2E tests (which expect to interact with your app and not an onboarding screen). To skip the onboarding screen in a test environment, the query parameter `disableOnboarding=1` can be appended to the project URL (an EAS Update URL or a local development server URL).

An example of such a Detox test is shown below. Full example code is available on the [`eas-tests-example`](https://github.com/nxenv/eas-tests-example) repository.
...
module.nxenvrts.openApp = async function openApp() {
...
async function openAppForDebugBuild(platform) {
  const deepLinkUrl = process.env.NXENV_USE_UPDATES
...
const getDeepLinkUrl = url =>
  `eastestsexample://nxenv-development-client/?url=${encodeURIComponent(url)}`;

const getDevLauncherPackagerUrl = platform =>
  `http://localhost:8081/index.bundle?platform=${platform}&dev=true&minify=false&disableOnboarding=1`;

const getLatestUpdateUrl = () =>
  `https://u.nxenv.dev/${getAppId()}?channel-name=test_debug&disableOnboarding=1`;

const getAppId = () => appConfig?.nxenv?.extra?.eas?.projectId ?? '';
