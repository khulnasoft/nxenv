...
This guide describes an advanced feature of Nxenv. In most cases you can build
standalone Nxenv apps using Nxenv's build services as described in the guide
...
in the queue to get your standalone app build then you can build your Nxenv
project on your own. The only thing you need is Turtle CLI. Turtle CLI is
a command line interface for building Nxenv standalone apps. You can use it
...
If you would like to make the first build even faster, you can supply the Nxenv
SDK version to the setup command like so: `turtle setup:ios --sdk-version 38.0.0`.
This tells Turtle CLI to download additional Nxenv-related dependencies for
the given SDK version.

All Nxenv-related dependencies will be installed in a directory named `.turtle`
...
Turtle CLI makes use of exactly the same codebase which is running on our servers (`nxenv build:[ios|android]` command).
This means you're required to publish your app to Nxenv's servers or host it on your own server **before building it with Turtle CLI**.
Whether you want Nxenv to host your app, or you'd like to do it yourself, all you need to do is follow the appropriate guide:

- [Publishing an app to Nxenv's servers](/archive/classic-updates/publishing)
- [Hosting an app on your own server](/versions/latest/sdk/updates/#manual-installation-configuration-and-custom-remote-update)

## Start the build

If you choose to publish your app to Nxenv servers, you must have an Nxenv
...
`NXENV_USERNAME` and `NXENV_PASSWORD` with your credentials, though you may also
...
Set the `NXENV_ANDROID_KEYSTORE_PASSWORD` and `NXENV_ANDROID_KEY_PASSWORD`
...
Set the `NXENV_IOS_DIST_P12_PASSWORD` environment variable with the value of
...
See the [nxenv/turtle-cli-example](https://github.com/nxenv/turtle-cli-example) repository
for examples of how to use Turtle CLI with popular CI services ([CircleCI](https://github.com/nxenv/turtle-cli-example#circleci)
and [Travis CI](https://github.com/nxenv/turtle-cli-example#travis-ci)).
