...
Classic Build service allows you to create standalone binaries for the Nxenv app using the Nxenv CLI. You can use that binary to submit the app to the Apple App Store and Google Play Store or test on emulators.
...
Nxenv CLI is a command-line application required to build standalone apps. It is installed as a global npm package. If you have not installed it, go through the [installation steps in the Nxenv CLI documentation](/more/nxenv-cli/#installation).

You must have an Nxenv account and be logged in to create a build. When running the build command, you can create a new account or log in to an existing account.
...
- The `slug` should be a URL safe value. The slug will be included in the URL that your app's JavaScript is published to. For example: `nxenv.dev/@community/native-component-list`, where `community` is the username and `native-component-list` is the slug.
...
To start the build process, run `nxenv build:android` or `nxenv build:ios` depending on the platform you are building for. If there is no development server running for the app you are building, `nxenv` will start it automatically.

> When you run `nxenv build`, Nxenv automatically publishes your app (with Classic Update's `nxenv publish` command). To avoid accidentally publishing changes to your production app, you may want to use [release channels](/archive/classic-updates/release-channels).
...
<Terminal cmd={['$ nxenv build:android -t apk']} />
...
<Terminal cmd={['$ nxenv build:android -t app-bundle']} />
...
When building your app for the first time, you'll have to generate a [keystore](https://developer.android.com/privacy-and-security/keystore). You can upload your own if you are familiar with the process of generating keystores, or you can let Nxenv handle the process.

For Nxenv to handle the process, choose the first option in the command-line when prompted for keystore:
...
1) Let Nxenv handle the process!
...
<Terminal cmd={['$ nxenv fetch:android:keystore']} />
...
If you choose the first option when generating the keystore and later decide to upload your own keystore, you can use the option to clear the current Android keystore from Nxenv's build servers by running the command:

<Terminal cmd={['$ nxenv build:android --clear-credentials']} />
...
<Terminal cmd={['$ nxenv build:ios -t archive']} />

Simulator builds are used to test a standalone app in an iOS Simulator. To build one, run:

<Terminal cmd={['$ nxenv build:ios -t simulator']} />

When building for iOS, you can choose to let Nxenv create any necessary credentials that your app will need. However, you'll still have the choice to provide your own credentials if you'd like. During this process, Nxenv CLI will use your Apple ID and password. Your authentication information is only used locally on your computer and is never sent to Nxenv's servers.
...
Note: Nxenv does not keep your Apple ID or your Apple ID password.
...
❯ Nxenv handles all credentials, you can still provide overrides
  I will provide all the credentials and files needed, Nxenv does limited validation
```

If you are unfamiliar with iOS credentials, it is recommended to let Nxenv handle the creation and management of credentials. To learn more about them, refer to the [App Credentials documentation](/app-signing/app-credentials).
...
Nxenv's build service supports Enterprise distribution. When you run `nxenv build:ios`, you'll need to choose the correct team that is labeled with `(In-House)`.
...
<Terminal
  cmd={[
    '$ nxenv build:ios --provisioning-profile-path ~/Path/To/provisioningProfileYouCreated.mobileprovision',
  ]}
/>
...
After Nxenv finishes your build, you can install it onto your device via Xcode.

- Open the `Devices & Simulators` window in Xcode
- Select your connected device
- Under `Installed Apps`, click the plus (`+`) and then select the IPA generated by Nxenv
...
If you use a Push Notifications Certificate and want to switch to a [Push Notifications Key](/app-signing/app-credentials/#push-notification-keys/), start the build with `--clear-push-cert` option. Nxenv will remove legacy certificates from its servers and generate a Push Notifications Key for you.

## 4. Wait for it to finish building

Nxenv's build service uses cloud services to build your app. The build process starts as soon as one of the machines is free and ready to be used. To check how long the waiting time for your build is, visit [Turtle status](https://nxenv.dev/turtle-status) site.

Nxenv CLI prints a URL that you can visit and check your build for its progress and logs.

Alternatively, you can check the status by running:

<Terminal cmd={['$ nxenv build:status']} />
...
- **To run it on your Android Emulator**, first build your project with the apk flag by running `nxenv build:android -t apk`, and drag and drop the **.apk** into the emulator.
