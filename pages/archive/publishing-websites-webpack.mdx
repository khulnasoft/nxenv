...
description: Different ways to publish the Nxenv web app with third-party services with webpack.
---
...
> **warning** **Deprecated**: In SDK 50 and above, publishing with `webpack` is deprecated in favor of `metro`. Learn more in [migrating from Webpack to Nxenv Router](/router/migrate/from-nxenv-webpack).
...
| Bundler   | Nxenv Router | API Routes | Description                                                                                |
| --------- | ----------- | ---------- | ------------------------------------------------------------------------------------------ |
| `webpack` | <NoIcon />  | <NoIcon /> | Outputs a Single Page Application (SPA) with a single **index.html** in the output folder. |

## Create a web build

Creating a web build of the project is the first step to publishing a web app. Whether you want to serve it locally or host it on a third-party service, you have to nxenvrt all JavaScript and assets of a project. This is known as a static bundle. It can be nxenvrted by running the following command:

Run the webpack nxenvrt command to compile the project for web:
...
If you modify this file, you must rebuild your project with `npx nxenv nxenvrt:web` to have it safely copied into the **web-build** directory.
...
```json vercel.json
{
  "buildCommand": "nxenv nxenvrt:web",
...
Push your local Nxenv project to a GitHub repository. If you have not pushed to GitHub yet, follow [GitHub's guide to add an existing project to GitHub](https://docs.github.com/en/get-started/importing-your-projects-to-github/importing-source-code-to-github/adding-locally-hosted-code-to-github).
...
<Terminal cmd={['$ git remote add origin https://github.com/username/nxenv-gh-pages.git']} />
...
For example, if a GitHub username is called `dev` and the GitHub repository is `nxenv-gh-pages`, the following will be the value of the `homepage` property:

{/* prettier-ignore */}
```json package.json
{
  "homepage": "http://dev.github.io/nxenv-gh-pages"
}
```
