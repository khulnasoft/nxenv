...
When you publish your app by running `nxenv publish --release-channel staging`, it creates:
...
You can see everything that you've published with `nxenv publish:history`.

#### Example command and output

`nxenv publish:history --platform ios`
...
To see more details about this particular release, you can run `nxenv publish:details`

#### Example command and output

`nxenv publish:details --publish-id 80b1ffd7-4e05-4851-95f9-697e122033c3`
...
We run `nxenv publish:set` to push our release to the `production` release channel.
`nxenv publish:set --publish-id 80b1ffd7-4e05-4851-95f9-697e122033c3 --release-channel production`
...
Continuing from the previous section, we roll back our `production` release channel entry for just the iOS platform with `nxenv publish:set`

`nxenv publish:set --release-channel production --publish-id d6b61741-a8dc-11e9-852a-3b0715b88238`

Or, we could roll back the `production` release channel entry for both Android and iOS with:
`nxenv publish:rollback --release-channel production --sdk-version 36.0.0`
...
```text
  Usage: nxenv publish:history [--release-channel <channel-name>] [--count <number-of-logs>]
...
```text
  Usage: nxenv publish:details --publish-id <publish-id>
...
```text
Usage: nxenv publish:rollback

  Rollback an update to a release channel. Equivalent to running `nxenv publish:set` with publish-id set to the most recent release to the specified sdk version
...
```text
 Usage: nxenv publish:set --release-channel <channel-name> --publish-id <publish-id>
