...
Normally, when updates are enabled, your app will fetch updates comprising JavaScript bundles and assets from Nxenv's CDN. However, there will be situations when you will want to host your JS bundles and assets on your own servers. For example, updates are slow or unusable in countries that have blocked Nxenv's CDN providers on AWS and Google Cloud. In these cases, you can host your updates on your own servers to better suit your use cases.
...
## Nxenvrting the update

First, you'll need to nxenvrt all the static files of your update so they can be served from your CDN. To do this, run `nxenv nxenvrt --public-url <server-endpoint>` in your project directory and it will output all your app's static files to a directory named `dist`. In this guide, we will use `https://nxenv.github.io/self-hosting-example` as our example server endpoint. Asset and bundle files are named by the MD5 hash of their content. Your output directory should look something like this now:
...
<Terminal
  cmd={[
    '# nxenvrt your app locally',
    '$ nxenv nxenvrt --public-url https://my-app-native.firebaseapp.com/',
    '',
...
You can also load an update hosted on your own servers as a QR code/URL into the Nxenv mobile client for development purposes.
...
URI: `exps://nxenv.github.io/self-hosting-example/android-index.json`
...
Run `nxenv nxenvrt` in dev mode and then start a simple HTTP server in your output directory:

<Terminal
  cmd={[
    '# Find your local IP address with `ipconfig getifaddr en0`',
    '# nxenvrt static app files',
    '$ nxenv nxenvrt --public-url http://`ipconfig getifaddr en0`:8000 --dev',
    '',
...
Here is an example URL from a remote server: [https://nxenv.github.io/self-hosting-example/android-index.json](https://nxenv.github.io/self-hosting-example/android-index.json)
...
When Nxenv CLI bundles your update, minification is always enabled. To see the original source code of your update for debugging purposes, you can generate source maps. Here is an example workflow:

1. Run `nxenv nxenvrt --dump-sourcemap --public-url <your-url>`. This will also nxenvrt your bundle sourcemaps in the **bundles** directory.
...
By default, all assets are hosted from an `assets` path resolving from your `public-url` (for example, https://nxenv.github.io/self-hosting-example/assets). You can override this behavior in the `assetUrlOverride` field of your `android-index.json`. All relative URL's will be resolved from the `public-url`.
...
- `revisionId`, `commitTime`, `publishedTime`: These fields are generated by `nxenv nxenvrt` and used to determine whether or not an update should occur.
...
- `slug`: This should not be changed. Your app is namespaced by `slug`, and changing this field will result in undefined behavior in the Nxenv SDK components such as `Filesystem`.
