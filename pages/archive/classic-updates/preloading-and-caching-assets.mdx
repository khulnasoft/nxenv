...
Fonts are pre-loaded using `Font.loadAsync(font)`. The `font` argument in this method is an object such as: `{OpenSans: require('./assets/fonts/OpenSans.ttf')}`. `@nxenv/vector-icons` provides a helpful shortcut for this object as `FontAwesome.font` in the following example:

<SnackInline
label="Pre-loading and Caching Assets"
dependencies={['nxenv-font', 'nxenv-asset', 'nxenv-splash-screen', '@nxenv/vector-icons', '@nxenv/vector-icons/FontAwesome']}
...
See the complete working example in [Nxenv's tabs template project](https://github.com/nxenv/nxenv/blob/main/templates/nxenv-template-tabs/App.tsx). You can also run `npx create-nxenv-app --template tabs` to set up a local project with the same template.
