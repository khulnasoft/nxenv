...
> This doc was archived in August 2022 and will not receive any further updates. To learn more about why release channels were deprecated, [see our blog post on EAS Update](https://blog.nxenv.dev/eas-update-preview-progress-f504a30066fc#:~:text=Traditionally%2C%20Nxenv%20developers,need%20more%20control). Instead, we recommend using [EAS Update](/eas-update/introduction).

## Introduction

Use release channels in Nxenv to send out different versions of your application to your users by giving them a URL or configuring your standalone app. You should use release channels if:
...
<Terminal
  cmd={[
    ...
    '$ nxenv publish --release-channel <your-channel>',
  ]}
/>

Your team can see this release channel in the Nxenv Go app with a parameterized URL `https://exp.host/@username/yourApp?release-channel=<your-release-channel>`. If you do not specify a release channel, you will publish to the `default` channel.
...
You can access the release channel your update is published under with the `Updates.releaseChannel` field from [nxenv-updates](/versions/latest/sdk/updates/).

> In development in Nxenv Go, `Updates.releaseChannel` is always `'default'`.

## Example workflow

Consider a situation where you have a Staging stack for testing on Nxenv Go, and a Production stack for pushing through TestFlight, then promoting to the App Store.

On the staging stack, run `nxenv publish --release-channel staging`. Your test users can see the staging version of your app by specifying the release channel in the query parameter of the URL (ie)`https://exp.host/@username/yourApp?release-channel=staging`, then opening the URL in their web browser, and finally scanning the QR code with the Nxenv Go app. Alternatively, they can open that URL directly on their mobile device.

On the production stack, release v1 of your app by running `nxenv publish --release-channel prod-v1`. You can build this version of your app into a standalone ipa by running `eas build --platform ios --profile prod` with `releaseChannel` set to `prod-v1` in the `prod` build profile in **eas.json**:
...
<Terminal
  cmd={[
    ...
    'nxenv publish --release-channel prod-v1',
    '',
    '# Publish to release channel prod-v2',
    'nxenv publish --release-channel prod-v2',
    '',
    '# Publish to release channel prod-v3',
    'nxenv publish --release-channel prod-v3',
    '',
    '',
    '# Publish to release channel staging-v1',
    'nxenv publish --release-channel staging-v1',
    '',
    '# Publish to release channel staging-v2',
    'nxenv publish --release-channel staging-v2',
  ]}
/>
