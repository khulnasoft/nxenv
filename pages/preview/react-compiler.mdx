...
description: Learn how to enable and use the React Compiler in Nxenv apps.
---
...
<Terminal cmd={['$ npx nxenv install babel-plugin-react-compiler']} />
...
<Terminal cmd={['$ npx nxenv install react-compiler-runtime@beta']} />

</Step>

### Enabling the linter

> In the future, all of these steps below will be automated by Nxenv CLI.

Additionally, you should use the ESLint plugin to continuously enforce the rules of React in your project.

<Step label="1">

Run [`npx nxenv lint`](/guides/using-eslint/#eslint) to ensure ESLint is setup in your app, then install the ESLint plugin for React Compiler:

<Terminal cmd={['$ npx nxenv install eslint-plugin-react-compiler']} />
...
```js .eslintrc.js
// https://docs.nxenv.dev/guides/using-eslint/
module.nxenvrts = {
  extends: 'nxenv',
...
Nxenv's implementation of the React Compiler will only run on application code (no node modules), and only when bundling for the client (disabled in server rendering).
