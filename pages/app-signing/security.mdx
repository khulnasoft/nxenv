...
Before you enter outside credentials or provide other sensitive data to third-party software you should ask yourself whether you trust the software to use it responsibly and protect it. Due to the nature of what goes into building an app binary for distribution on app stores, the Nxenv standalone app build service requires various pieces of information with varying degrees of sensitivity. This document explains what those are, how we store them, and what could go wrong if they were to be compromised.

Most data stored by Nxenv servers (credentials or otherwise) is encrypted at rest by our cloud provider, Google Cloud. Credentials are additionally encrypted using [KMS](https://cloud.google.com/security/products/security-key-management). Credentials are only unencrypted for as long as we need them in memory in the standalone app builders or push notification services. Credentials are always encrypted in our databases, message queues, and other less transient parts of the system.

All of the data related to the information explained below can be downloaded and removed from Nxenv servers (if it is stored there at all in the first place), and some of it may be available through other locations such as the Apple Developer Portal.

## Android Push Notification credentials

Android uses Firebase Cloud Messaging (FCM) for push notifications. If you build a standalone app with Nxenv we store your FCM server key for you.
...
You can create and delete server keys through the Firebase console. When you delete a key, notifications using that key will stop working. When you create a new one and upload it to Nxenv, notifications will resume working.
...
Nxenv tools never ask you to provide your Google account credentials.

## Android submit credentials

### Google Service Account Key

Google Service Account Key is the authentication method used to submit an Android app to the Google Play Store with EAS Submit. This key is stored on Nxenv servers and encrypted using [KMS](https://cloud.google.com/security/products/security-key-management) when at rest. The key will remain on Nxenv servers to be re-used for subsequent submissions, and it can be removed at any time by a user with the requisite permissions.
...
Auth keys are revocable from the Apple Developer Center. If you revoke them, notifications will stop working. If you provision a new auth key and upload it to Nxenv then notifications will resume working. Device tokens are not invalidated when auth keys are revoked.
...
When creating a standalone app build, or uploading to the App Store you will be prompted for your Apple Developer account credentials. We do not store these on our servers &mdash; EAS CLI only uses them locally. Your computer alone provisions distribution certificates and auth keys that are sent to Nxenv servers; your developer credentials are not sent to Nxenv servers. An additional layer of security is enforced by Apple, as they require two-factor authentication for all Apple Developer accounts.
...
Disable Keychain support with the environment variable `NXENV_NO_KEYCHAIN=1`. You can also use this to change the saved password.

### Changing Apple ID password in Keychain

To delete the locally stored password, open the "Keychain Access" app, switch to "All Items", and search for "deliver. [Your Apple ID]" (example: `deliver.bacon@nxenv.dev`). Select the item you wish to modify and delete it. Next time running an Nxenv command you'll be prompted for a new password.
...
Apple app-specific password is another authentication method that can be used to submit an iOS app to Apple's App Store using the EAS Submit. Unlike other credentials, the app-specific password is not stored in the Nxenv servers between submissions, it must be provided each time it is to be used.

The password is encrypted using [KMS](https://cloud.google.com/security/products/security-key-management) and stored only for the period required to submit the app to the App Store plus 24 hours, to allow for retries during that time. Once this period is over, the password is deleted from the Nxenv servers.

This authentication method is **not recommended**. We recommend using the Apple Store Connect (ASC) API key for submitting your apps to the App Store instead. Nxenv won't use the Apple app-specific password in any way other than to submit your app to the App Store.
...
On top of the platform-specific credentials, a device token is necessary to send a push notification. Nxenv manages this for you and provides an abstraction on top of it with the Nxenv Push Token. The device token identifies the recipient, that is, the device to whom the notification is being sent. The device tokens are encrypted at rest and periodically cycled automatically by Android and iOS.
