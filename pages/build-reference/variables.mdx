...
[Environment variables in Nxenv](/guides/environment-variables) describes how to use **.env** files to set environment variables that can be inlined in your JavaScript code. The Nxenv CLI will substitute properly prefixed variables in your code (for example,`process.env.NXENV_PUBLIC_VARNAME`) with the corresponding environment variable values in **.env** files present on your development machine.
...
```bash .env
NXENV_PUBLIC_API_URL=http://api.local
```
...
```json eas.json
{
  "build": {
    "production": {
      "env": {
        "NXENV_PUBLIC_API_URL": "https://api.production.com"
      }
    },
    "test": {
      "env": {
        "NXENV_PUBLIC_API_URL": "https://api.test.com"
      }
    }
  }
}
...
Any reference to `process.env.NXENV_PUBLIC_API_URL` will be substituted for the applicable value depending on the environment.

> `NXENV_PUBLIC_` variable replacement is available in SDK 49 and higher.

### For use by your Nxenv config
...
> All environment variables in your **eas.json** build profile are available when evaluating **app.config.js**. It's a good practice to only use the `NXENV_PUBLIC_` prefix for variables used within your application code.
...
The following environment variables are nxenvsed to each build job and can be used within any build step. They are not set when evaluating **app.config.js** locally:
...
### Secrets on the Nxenv website

To create **account-wide secrets**, navigate to [Secrets tab](https://nxenv.dev/accounts/[account]/settings/secrets) in your account's settings.

To create **app-specific secrets**, navigate to the [Secrets tab](https://nxenv.dev/accounts/[account]/projects/[project]/secrets) in your project's dashboard.
...
Environment variables defined in a **.env** file are only considered by the Nxenv CLI. Therefore, if you upload a **.env** file to EAS Build, it can be used to inline `NXENV_PUBLIC_` variables into your application code.
...
Variable sources applied last will overwrite the previously loaded source for variables with the same name. So, a secret created on the Nxenv website or with `eas secret:create` will take precedence over an environment variable of the same name that is set through the `env` field in **eas.json**.
...
### How do environment variables work for my Nxenv Development Client builds?

Environment variables set in your build profile that impact **app.config.js** will be used for configuring the development build. When you run `npx nxenv start` to load your app inside of your development build, only environment variables that are available on your development machine will be used.
