...
You have created a project using Nxenv tooling, and now you want to create a development and a production build. Your project's **app.json** may have the following configuration:
...
To have multiple variants of the app installed on the same device, rename the **app.json** to **app.config.js** and nxenvrt the configuration as shown below:
...
- If you're using [development builds](/develop/development-builds/introduction/), you can configure the `nxenv-dev-client` plugin to disable the app scheme used by Nxenv CLI and EAS Update QR codes in non-development builds. This ensures that those URLs will always launch the development build, regardless of your device's defaults:
...
When you start your development server, you'll need to run `APP_VARIANT=development npx nxenv start` (or the platform equivalent if you use Windows).
...
{
  "scripts": {
    "dev": "APP_VARIANT=development npx nxenv start"
  }
}
```
