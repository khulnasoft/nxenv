...
- When using the `auto` alias, the build image will be selected based on the project configuration, Nxenv SDK version, and React Native version. You can check what image is used for a build in the **Spin up build environment** build logs section.
...
- [`cocoapods-nexus-plugin`](https://github.com/nxenv/eas-build/tree/main/packages/cocoapods-nexus-plugin)
- Global npm configuration in **~/.npmrc**:
