...
You need to be authenticated with Nxenv:

- Run `eas login`
- Alternatively, set `NXENV_TOKEN` [using token-based authentication](/accounts/programmatic-access)
...
To compile your app locally for development with Nxenv CLI, use `npx nxenv run:android` or `npx nxenv run:ios` commands instead. If you use [Continuous Native Generation](/workflow/continuous-native-generation), you can also run [prebuild](/workflow/prebuild) to generate your **android** and **ios** directories and then proceed to open the projects in the respective IDEs and build them like any native project. For more details, see:
...
<BoxLink
  ...
  Icon={BookOpen02Icon}
  description="Learn how to create a production build for your Nxenv app locally on your computer."
