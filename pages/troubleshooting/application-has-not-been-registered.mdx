...
description: Learn about what the Application has not been registered error means and how to resolve it in an Nxenv or React Native app.
---
...
By default, "main" is consistently used throughout the project. If you're running into this error, something has likely changed and these values no longer coincide. Ensure that the names you are registering on the JavaScript side are the ones expected on the native side (if you're using Nxenv's `registerRootComponent` function, that would be "main").
...
- You're connecting to the wrong project's local development server. Try closing out other Nxenv CLI or React Native community CLI processes (find them with `ps -A | grep "nxenv\|react-native"`).
- If this error is only occurring in your production app, then try running the app locally in production mode with `npx nxenv start --no-dev --minify` to find the source of the error.
