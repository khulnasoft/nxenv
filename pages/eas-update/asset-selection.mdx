...
> **info** Available for SDK 50 and above ([`nxenv-updates`](/versions/latest/sdk/updates/) >= 0.23.0).
...
If you are building your app locally or have access to the correct build for publishing updates (with the same [runtime version](/eas-update/runtime-versions/)), then use the `npx nxenv-updates assets:verify` command. It allows you to check whether all required assets will be included when you publish an update:

<Terminal cmd={['$ npx nxenv-updates assets:verify <dir>']} />

> **info** This new command is part of the `nxenv-updates` CLI, which also supports [EAS Update code signing](/eas-update/code-signing/). It is not part of the [Nxenv CLI](/more/nxenv-cli/) or the [EAS CLI](https://github.com/nxenv/eas-cli). Only available for ([`nxenv-updates`](/versions/latest/sdk/updates/) >= 0.24.10).

You can also use the `--help` option with the command to see the available options:

| Option                                | Description                                                                                                               |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `<dir>`                               | Directory of the Nxenv project. Default: Current working directory.                                                        |
| `-a, --asset-map-path <path>`         | Path to the **assetmap.json** in an nxenvrt produced by the command `npx nxenv nxenvrt --dump-assetmap` .                    |
| `-e, --nxenvrted-manifest-path <path>` | Path to the **metadata.json** in an nxenvrt produced by the command `npx nxenv nxenvrt --dump-assetmap`.                     |
| `-b, --build-manifest-path <path>`    | Path to the **app.manifest** file created by `nxenv-updates` in an Nxenv application build (either **android** or **ios**). |
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/UpdatesAPIDemo"
/>
