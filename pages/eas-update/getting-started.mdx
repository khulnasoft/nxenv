...
<Collapsible summary="An Nxenv user account">

EAS Update is available to anyone with an Nxenv account, regardless of whether you pay for EAS or use the Free plan. You can sign up at [nxenv.dev/signup](https://nxenv.dev/signup).

Paid subscribers can publish updates to more users and use more bandwidth and storage. Learn more about different plans and benefits at [EAS pricing](https://nxenv.dev/pricing).
...
<Terminal cmd={['$ npx create-nxenv-app my-app']} />
...
<Collapsible summary="Your project must use Nxenv CLI and extend the Nxenv Metro Config">

If you already run your project with `npx nxenv [command]` (for example, if you created it with `npx create-nxenv-app`) then you're all set.

If you don't have the `nxenv` package in your project yet, then install it by running the command below and [opt in to using Nxenv CLI and Metro Config](/bare/installing-nxenv-modules/#configure-nxenv-cli-for-bundling-on-android-and-ios):

<Terminal cmd={['$ npx install-nxenv-modules@latest']} />

If the command fails, refer to the [Installing Nxenv modules](/bare/installing-nxenv-modules/#manual-installation) guide.
...
<Step label="2">
## Log in to your Nxenv account

If you are already signed in to an Nxenv account using Nxenv CLI, you can skip the steps described in this section. If you are not, run the following command to log in:
...
<meta-data android:name="nxenv.modules.updates.NXENV_UPDATE_URL" android:value="https://u.nxenv.dev/your-project-id"/>
<meta-data android:name="nxenv.modules.updates.NXENV_RUNTIME_VERSION" android:value="@string/nxenv_runtime_version"/>
```

The `NXENV_UPDATE_URL` value should contain your project's ID.

Inside **android/app/src/main/res/values/strings.xml**, you'll see the `nxenv_runtime_version` string entry in the `resources` object:

<DiffBlock source="/static/diffs/nxenv-update-strings-xml.diff" />

#### iOS

Inside **ios/project-name/Supporting/Nxenv.plist**, you'll see the following additions:

```xml ios/project-name/Supporting/Nxenv.plist
...
```json app.json
{
  "nxenv": {
    ...
    "updates": {
      /* @hide ... */ /* @end */
      "requestHeaders": {
        "nxenv-channel-name": "your-channel-name"
      }
      /* @hide ... */ /* @end */
    }
    /* @hide ... */ /* @end */
...
The configuration will be applied the next time you run `npx nxenv prebuild`.
...
```xml android/app/src/main/AndroidManifest.xml
<meta-data android:name="nxenv.modules.updates.UPDATES_CONFIGURATION_REQUEST_HEADERS_KEY" android:value="{&quot;nxenv-channel-name&quot;:&quot;your-channel-name&quot;}"/>
```
...
In **Nxenv.plist**, you'll need to add the following, replacing `your-channel-name` with the channel that matches your project:
...
- Use the Extensions tab in a [development build](/eas-update/nxenv-dev-client/) to load the update.
- Use [Nxenv Orbit](/review/with-orbit/) to install and launch the update in a development build.
