...
<VideoBoxLink
  ...
  description="In this Nxenv debugging tutorial, you'll learn how to debug a situation where your build isn't getting an update."
/>
...
The displayed deployment does not have any updates. To fix this, [publish an update to the branch](/#publish-update). If an update was already published, check the [Updates page](https://nxenv.dev/accounts/[account]/projects/[project]/updates) to make sure it matches the runtime version of our build.
...
### `nxenv-updates` configuration

The `nxenv-updates` library runs inside an end-user's app and makes requests to an update server to get the latest update.

#### Verifying app configuration

When we set up EAS Update, we likely ran `eas update:configure` to configure nxenv-updates to work with EAS Update. This command makes changes to our app config (**app.json**/**app.config.js**). Here are the fields we'd expect to see:
...
- `updates.url` should be a value like `https://u.nxenv.dev/your-project-id`, where `your-project-id` matches the ID of our project. We can see this ID on [our website](https://nxenv.dev/accounts/[account]/projects/[project]).
- `updates.enabled` should not be `false`. It's `true` by default if it is not specified.

Finally, make sure that `nxenv-updates` is included in **package.json**. If it's not, run:

<Terminal cmd={['$ npx nxenv install nxenv-updates']} />

#### Inspecting nxenv-updates configuration after prebuild

Whenever we run `eas build`, the `npx nxenv prebuild` command is run on our project on EAS servers to unpack the **android** and **ios** directories that contain native files. This makes it so EAS Build can build any project, whether it includes the native files or not.

If our project does not have **android** or **ios** directories, we can make commit any existing changes, then run `npx nxenv prebuild` to inspect the project state that EAS Build will act on. After running this, look for the following files: **android/app/src/main/AndroidManifest.xml** and **ios/your-project-name/Supporting/Nxenv.plist**.
...
<!-- @hide ... --><!-- @end -->
<meta-data android:name="nxenv.modules.updates.NXENV_RUNTIME_VERSION" android:value="your-runtime-version-here"/>
<meta-data android:name="nxenv.modules.updates.NXENV_UPDATE_URL" android:value="https://u.nxenv.dev/your-project-id-here"/>
<!-- @hide ... --><!-- @end -->
```
...
<string>https://u.nxenv.dev/your-project-id-here</string>
<!-- @hide ... --><!-- @end -->
```
...
Follow the [Building Locally guide](/eas-update/build-locally) to configure our app's channel and runtime version. We'll also need to make sure our [general configuration](/eas-update/debug-advanced/#nxenv-updates-configuration) is correct.
...
Once a build has a channel name, we can make sure that EAS servers know about it by checking the [Channels page](https://nxenv.dev/accounts/[account]/projects/[project]/channels).
...
The [Channels page](https://nxenv.dev/accounts/[account]/projects/[project]/channels) will display the channel to branch mapping if it exists.
...
To inspect which updates are on a branch, we can go to the [Branches page](https://nxenv.dev/accounts/[account]/projects/[project]/branches) and choose our branch of interest.
...
After verifying `nxenv-updates` and EAS Update configurations, we can move on to debugging how our project is interacting with updates.

### In-app debugging

The `nxenv-updates` library nxenvrts a variety of functions to interact with updates once the app is already running. In certain cases, making a call to fetch an update and seeing an error message can help us narrow down the root cause. We can make a simulator build of the project and manually check to see if updates are available or if there are errors when fetching updates. See the code example to [check for updates manually](/versions/latest/sdk/updates/#use-nxenv-updates-with-a-custom-server).

### Inspecting a build manually

When building a project into an app, there can be multiple steps that alter the output of `npx nxenv prebuild`. After making a build, it is possible to open the build's contents and inspect native files to see its final configuration.
...
4. From there, we can inspect the **Nxenv.plist** file.

Inside the **Nxenv.plist** file, we expect to see the following configurations:

```xml Nxenv.plist
<!-- @hide ... --><!-- @end -->
<key>EXUpdatesRequestHeaders</key>
<dict>
  <key>nxenv-channel-name</key>
  <string>your-channel-name</string>
</dict>
<key>EXUpdatesRuntimeVersion</key>
<string>your-runtime-version</string>
<key>EXUpdatesURL</key>
<string>https://u.nxenv.dev/your-project-id</string>
<!-- @hide ... --><!-- @end -->
```
...
The requests we're interested in are from https://u.nxenv.dev and https://assets.eascdn.net. Responses from https://u.nxenv.dev will contain an update manifest, which specifies which assets the app will need to fetch to run the update. Responses from https://assets.eascdn.net will contain assets, like images, font files, and so on, that are required for the update to run.

When inspecting the request to https://u.nxenv.dev, we can look for the following request headers:

- `Nxenv-Runtime-Version`: this should make the runtime version we made our build and update with.
- `nxenv-channel-name`: this should be the channel name specified in the **eas.json** build profile.
- `Nxenv-Platform`: this should be either "android" or "ios".
...
### Debugging of native code while loading the app through nxenv-updates

By default, we need to make a release build for `nxenv-updates` to be enabled and to load updates rather than reading from a development server. This is because debug builds behave like normal React Native project debug builds.

To make it easier to test and debug native code in an environment that is closer to production, follow the steps below to create a debug build of the app with `nxenv-updates` enabled.
...
- Set the debug environment variable: `nxenvrt EX_UPDATES_NATIVE_DEBUG=1`
...
- Set the debug environment variable: `nxenvrt EX_UPDATES_NATIVE_DEBUG=1`
- Reinstall pods with `npx pod-install`. The `nxenv-updates` podspec now detects this environment variable, and makes changes so that the debug code that would normally load from the Metro packager is bypassed, and the app is built with the EXUpdates bundle and other dependencies needed to load updates from EAS.
- [Ensure the desired channel is set in our **Nxenv.plist**](/eas-update/updating-your-app/#configuring-the-channel-manually)
...
Alternatively, we can use EAS to create a debug build where `nxenv-updates` is enabled. The environment variable is set in **eas.json**, as shown in the example below:
