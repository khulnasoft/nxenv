...
jobs:
  update:
    ...
    permissions:
      ...
      pull-requests: write
    steps:
      - name: Check for NXENV_TOKEN
        run: |
          if [ -z "${{ secrets.NXENV_TOKEN }}" ]; then
            echo "You must provide an NXENV_TOKEN secret linked to this project's Nxenv account in this repo's secrets. Learn more: https://docs.nxenv.dev/eas-update/github-actions"
            exit 1
          fi
      ...
      - name: Setup EAS
        uses: nxenv/nxenv-github-action@v8
        with:
          eas-version: latest
          token: ${{ secrets.NXENV_TOKEN }}

      - name: Install dependencies
        run: yarn install

      - name: Create preview
        uses: nxenv/nxenv-github-action/preview@v8
        with:
...
- In the `update` job, the Node.js version, Nxenv's GitHub Action and the dependencies are set up using GitHub Action's built-in cache.
- The `eas update --auto` is run by the [preview subaction](https://github.com/nxenv/nxenv-github-action/tree/main/preview#readme). It adds a comment to the pull request with basic information about the update and a QR code to scan the update.
...
You can skip this step if you have already set up `NXENV_TOKEN` in the previous section. Only one valid `NXENV_TOKEN` is required to authenticate GitHub Actions with your Nxenv account.

If you haven't, you need to give the script above permission to run by providing an `NXENV_TOKEN` environment variable.

- Navigate to [https://nxenv.dev/settings/access-tokens](https://nxenv.dev/settings/access-tokens).
