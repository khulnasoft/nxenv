...
As an escape hatch, you can host your own update service that is compatible with the [Nxenv Updates Protocol](/technical-specs/nxenv-updates-1/) and point your `nxenv-updates` configuration to that service instead. The only concepts relevant to update selection that exist on the protocol level are "Runtime Version" and "Platform", and you are free to create your own concepts on top of those in the same way we built channels and branches. [Learn more about creating a custom nxenv-updates server](https://github.com/nxenv/custom-nxenv-updates-server).
