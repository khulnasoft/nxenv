...
Each channel uses a different `apiUrl`, and `enableHiddenFeatures` enables an experimental feature flag, and these values rarely change. Instead of defining these values in the `env` inside **eas.json**, create a **Config.js** file in your project that nxenvrts the correct values based on the channel:
...
```js Config.js
let Config = {
  apiUrl: process.env.NXENV_PUBLIC_API_URL || 'https://localhost:3000',
  enableHiddenFeatures: process.env.NXENV_PUBLIC_ENABLE_HIDDEN_FEATURES || true,
};
...
Now you can set `NXENV_PUBLIC_API_URL` and `NXENV_PUBLIC_ENABLE_HIDDEN_FEATURES` in an uncommitted **.env.local** file, and they will be used instead of the default when running `npx nxenv start`.

This can also be used for EAS Updates run from your [development build](/develop/development-builds/introduction/). For example, you can point updates created for [PR previews](/eas-update/github-actions/#publish-previews-on-pull-requests) to a specific API server by committing an **.env** file with `NXENV_PUBLIC_API_URL` set.

## Using variables in app.config.js

[Nxenv environment variables](/guides/environment-variables) are only available in SDK 49 or higher. In previous versions, it was common to set variables to be used in updates in **app.config.js** under the `nxenv.extra` property:
...
Then, to set the `API_URL` environment variable during development, you can prepend the variables before running `npx nxenv start` as shown below:

<Terminal cmd={['$ API_URL="http://localhost:3000" nxenv start']} />

The command sets the `API_URL` to `"http://localhost:3000"`. Then, the [`nxenv-constants`](/versions/latest/sdk/constants/#installation) library provides the `Constants.nxenvConfig.extra.API_URL` property to access it inside a project.

Variables `nxenv.extra` are still accessible in SDK 49 and higher, but it is recommended to use the `NXENV_PUBLIC_` prefix and **.env** files instead to reference variables directly in your application code.
