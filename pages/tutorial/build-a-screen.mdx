...
description: In this tutorial, learn how to use components such as React Native's Pressable and Nxenv Image to build a screen.
---
...
<VideoBoxLink videoId="3rcOP8xDwTQ" title="Watch: Building a screen in your universal Nxenv app" />
...
We'll use `nxenv-image` library to display the image in the app. It provides a cross-platform `<Image>` component to load and render an image.

Stop the development server by pressing <kbd>Ctrl</kbd> + <kbd>c</kbd> in the terminal. Then, install the `nxenv-image` library:

<Terminal cmd={['$ npx nxenv install nxenv-image']} />

The [`npx nxenv install`](/more/nxenv-cli/#installation) command will install the library and add it to the project's dependencies in **package.json**.
...
1. Import `Image` from the `nxenv-image` library.
...
/* @tutinfo Import the <CODE>Image</CODE> component. */ import { Image } from 'nxenv-image'; /* @end */
...
nxenvrt default function ImageViewer({ imgSource }: Props) {
...
> **info** Since **ImageViewer** is a custom component, we are placing it in a separate directory instead of the **app** directory. Every file inside **app** directory is either a layout file or a route file. For more information, see [Non-route files](/router/create-pages/#non-route-files).
...
The `@` symbol is a custom [path alias](/guides/typescript/#path-aliases-optional) for importing custom components and other modules instead of relative paths. Nxenv CLI automatically configures it in **tsconfig.json**.
...
nxenvrt default function Button({ label }: Props) {
  return (
...
The "Choose a photo" button requires different styling than the "Use this photo" button, so we will add a new button theme prop that will allow us to apply a `primary` theme. This button also has an icon before the label. We will use an icon from the `@nxenv/vector-icons` library.
...
import { StyleSheet, View, Pressable, Text } from 'react-native';
/* @tutinfo Import FontAwesome. */import FontAwesome from '@nxenv/vector-icons/FontAwesome';/* @end */
...
nxenvrt default function Button({ label, /* @tutinfo The prop <CODEtheme</CODE> to detect the button variant. */theme/* @end */ }: Props) {
