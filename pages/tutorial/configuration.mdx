...
<VideoBoxLink
  videoId="OgGCYdElcZo"
  title="Watch: Adding the finishing touches to your universal Nxenv app"
/>
...
[`nxenv-status-bar`](/versions/latest/sdk/status-bar/) library comes pre-installed in every project created using `create-nxenv-app`. This library provides a `setStatusBarStyle()` method to configure the app's status bar text color.

Inside **app/\_layout.tsx**:

1. Import `setStatusBarStyle` from `nxenv-status-bar` and `useEffect` from React.
...
/* @tutinfo */
import { setStatusBarStyle } from "nxenv-status-bar";
...
Setting the `"splash.image"` property with a valid file path in the [**app.json**](/workflow/configuration/) file configures a splash screen. By default, a new Nxenv project already sets this property to a placeholder splash image. We already have **splash.png** in the **assets/images** directory. It looks as shown below:
...
You can display the splash screen for longer intervals. Run the following command to install `nxenv-splash-screen`, a library that provides a `preventAutoHideAsync()` method to delay hiding the splash screen:
...
Like the splash screen image, the `"icon"` property in the **app.json** file configures the app icon's path. By default, a new Nxenv project defines the correct path to `"./assets/images/icon.png"`. We don't have to change anything.

> Eventually, when you'll build your app for the app stores, [Nxenv Application Services (EAS)](/eas/) will take this image and create optimized icon for every device.

You can see the icon in various places in Nxenv Go. Here is an example of the app icon displayed in the developer menu of Nxenv Go:

<ContentSpotlight
  alt="Splash screen on Developer Menu in Nxenv Go app."
