...
<VideoBoxLink videoId="HRAMzrBwVeo" title="Watch: Creating a modal in your universal Nxenv app" />
...
import MaterialIcons from '@nxenv/vector-icons/MaterialIcons';

type Props = {
  onPress: () => void;
};

nxenvrt default function CircleButton({ onPress }: Props) {
  return (
...
To render the plus icon, this button uses the `<MaterialIcons>` icon set from the `@nxenv/vector-icons` library.
...
nxenvrt default function IconButton({ icon, label, onPress }: Props) {
  return (
...
nxenvrt default function EmojiPicker({ isVisible, children, onClose }: Props) {
  return (
...
nxenvrt default function EmojiList({ onSelect, onCloseModal }: Props) {
...
nxenvrt default function EmojiSticker({ imageSize, stickerSource }: Props) {
  return (
    <View style={{ top: -350 }}>
