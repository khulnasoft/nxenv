...
description: In this tutorial, learn how to capture a screenshot using a third-party library and Nxenv Media Library.
---
...
In this chapter, we'll learn how to take a screenshot using a third-party library and save it on the device's media library. We'll use [`react-native-view-shot`](https://github.com/gre/react-native-view-shot) to take a screenshot and [`nxenv-media-library`](/versions/latest/sdk/media-library/) to save an image on device's media library.
...
<VideoBoxLink videoId="Jft3_Yfr-p4" title="Watch: Taking screenshots in your universal Nxenv app" />
...
To install `react-native-view-shot` and `nxenv-media-library`, run the following commands:

<Terminal cmd={['$ npx nxenv install react-native-view-shot nxenv-media-library']} />
...
An app that requires sensitive information, such as accessing a device's media library, has to prompt permission to allow or deny access. Using `usePermissions()` hook from `nxenv-media-library`, we can use the permission `status` and `requestPermission()` method to ask for access.
...
/* @tutinfo Import <CODE>nxenv-media-library</CODE>. */import * as MediaLibrary from 'nxenv-media-library';/* @end */
...
<ProgressTracker
  ...
  summary={
    <>
        ...
        We've successfully used <CODE>react-native-view-shot</CODE> and <CODE>nxenv-media-library</CODE> to capture a
