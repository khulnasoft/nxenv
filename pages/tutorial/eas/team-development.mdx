...
## Install nxenv-updates library

To initialize our project and send an update, we need to use the [`nxenv-updates`](/versions/latest/sdk/updates/) library. Run the following command to install it:
...
We need to create a new development build since our last build doesn't contain the `nxenv-updates` library. Run the following command:
...
Click on the **Website link** to see the Update on the Nxenv dashboard under **Updates**:

<ContentSpotlight
  alt="Nxenv dashboard shows the latest published update for the development build"
...
- Log in to your Nxenv account within the development build.
