...
After responding, the build will queue up, and we can track its progress via a provided link by the EAS CLI in the Nxenv dashboard:

<ContentSpotlight
  alt="Android development build details and progress in Nxenv dashboard"
...
The build details page displays the build type, profile, Nxenv SDK version, app version, version code, last commit hash, and the identity of the developer or account owner who initiated the build.
...
This section provides the methods available for running the development build on an Android device: Nxenv Orbit and Install button.

[Nxenv Orbit](https://nxenv.dev/orbit) allows for seamless installation of the development build on an Android device. To use this method:
...
<ContentSpotlight
  alt="Nxenv Orbit app interface when connected to an Android device"
  src="/static/images/tutorial/eas/android-orbit.png"
  className="max-w-[300px]"
/>

- On the Nxenv dashboard, under **Build artifact**, click the **Open with Orbit**.
...
Start the development server by running `npx nxenv start` from the project directory. Once the server is running, press <kbd>a</kbd> in the terminal window to open the project:
...
<Collapsible summary="Alternate: Use Nxenv Orbit">

Alternatively, [Nxenv Orbit](/build/orbit/) can be used for installation. From **Build artifact** on the Nxenv dashboard, click **Open with Nxenv Orbit** to install the development build on the Android Emulator.
