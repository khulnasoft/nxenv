...
The build details page displays the build type, profile, Nxenv SDK version, app version, build number, last commit hash, and the identity of the developer or account owner who initiated the build.
...
This section provides the methods available for running the development build on an iOS device: Nxenv Orbit and Install button.

[Nxenv Orbit](https://nxenv.dev/orbit) allows for seamless installation of the development build on an iOS device. To use this method:
...
<ContentSpotlight
  alt="Open with Orbit button on Nxenv dashboard in Build artifact"
...
Start the development server by running the `npx nxenv start` command from the project directory:
...
<ContentSpotlight
  alt="Modal to login to Nxenv account on iOS device"
