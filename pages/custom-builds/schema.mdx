...
Environment variables nxenvrted (using `nxenvrt`) in one step's `command` are not automatically nxenvsed to other steps. To share an environment variable with other steps, use the `set-env` executable.

`set-env` expects to be called with two arguments: environment variable's name and value. For example, `set-env NPM_TOKEN "abcdef"` will nxenvse `$NPM_TOKEN` variable with value `abcdef` to other steps.

> **Note:** Variables shared with `set-env` are not automatically nxenvrted locally. You need to call `nxenvrt` yourself.
  ...
  steps:
    - run:
        ...
        command: |
          ...
          # Set and nxenvrt variable
          nxenvrt ENV_TEST_LOCAL_NXENVRT="present-in-current-step"
          ...
          # Will print "ENV_TEST_LOCAL_NXENVRT: present-in-current-step"
          # because nxenvrt also sets the local variable value.
          echo "ENV_TEST_LOCAL_NXENVRT: $ENV_TEST_LOCAL_NXENVRT"

          # Will "ENV_TEST_SET_ENV: "
          # because set-env does not set or nxenvrt variables.
          echo "ENV_TEST_SET_ENV: $ENV_TEST_SET_ENV"

          # Will only print LOCALLY_NXENVRTED_ENV,
          # because it is the only nxenvrt-ed variable.
          env | grep ENV_TEST_
    - run:
        ...
        command: |
          ...
          # Will print "ENV_TEST_LOCAL_NXENVRT: "
          # because nxenvrt does not share a variable to other steps.
          echo "ENV_TEST_LOCAL_NXENVRT: $ENV_TEST_LOCAL_NXENVRT"

          # Will print "ENV_TEST_SET_ENV: present-in-following-steps"
          # because set-env "nxenvrted" variable to other steps.
          echo "ENV_TEST_SET_ENV: $ENV_TEST_SET_ENV"

          # Will only print ENV_TEST_SET_ENV,
          # because set-env "nxenvrted" it to other steps.
          env | grep ENV_TEST_
```
...
The `working_directory` is used to define an existing directory from the project's root directory. After an existing path is defined in a step, using it changes the current directory for that step. For example, a step is created to list all the assets inside the **assets** directory, which is a directory in your Nxenv project. The `working_directory` is set to `assets`:
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-custom-builds-example/blob/main/.eas/build/ios-simulator-build.yml"
/>
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-custom-builds-example/blob/main/.eas/build/ios-build-with-credentials.yml"
/>
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-custom-builds-example/blob/main/.eas/build/android-build-without-credentials.yml"
/>
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-custom-builds-example/blob/main/.eas/build/android-build-with-credentials.yml"
/>
...
> **warning** Your project must be configured to use the old Build Infrastructure to start Android Emulator. Go to [Project settings](https://nxenv.dev/accounts/[account]/projects/[project]/settings) to configure. See [this changelog post](https://nxenv.dev/changelog/2024/08-29-c3d-default) for more information.
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functionGroups/maestroTest.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/checkout.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/useNpmToken.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/installNodeModules.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/resolveBuildConfig.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/resolveAppleTeamIdFromCredentials.ts"
/>

#### `eas/prebuild`

Runs the `nxenv prebuild` command using the package manager (npm, pnpm, or Yarn) detected based on your project with the command best suited for your build type and build environment.
...
<BoxLink
  ...
  description="View the source code for the eas/resolve_apple_team_id_from_credentials function on GitHub."
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/prebuild.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/configureEASUpdateIfInstalled.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/injectAndroidCredentials.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/configureIosCredentials.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/configureAndroidVersion.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/configureIosVersion.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/runGradle.ts"
/>
...
nxenvrt_xcargs "OTHER_CODE_SIGN_FLAGS=\\"--keychain <%- KEYCHAIN_PATH %>\\""
...
`CLEAN`, `SCHEME`, `BUILD_CONFIGURATION`, `NXENVRT_METHOD`, `PROFILES`, `ICLOUD_CONTAINER_ENVIRONMENT`, `KEYCHAIN_PATH`, `LOGS_DIRECTORY`, `OUTPUT_DIRECTORY`, `DERIVED_DATA_PATH`and `SCHEME_SIMULATOR_DESTINATION` values are provided to the template based on the inputs and default internal configuration of EAS Build.
...
| `inputs.credentials`         | `json`    | Optional input defining the app credentials for your iOS build. If specified `KEYCHAIN_PATH`, `NXENVRT_METHOD`, and `PROFILES` values will be provided to the template.                                                                                                   |
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/generateGymfileFromTemplate.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/runFastlane.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/findAndUploadBuildArtifacts.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/uploadArtifact.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/installMaestro.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/startAndroidEmulator.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/startIosSimulator.ts"
/>
...
For example, `'Build URL: ${ eas.job.nxenvBuildUrl }'`, `Build finished with status: ${ steps.run_fastlane.status_text }`, `Build failed with error: ${ steps.run_gradle.error_text }`.
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-build/blob/main/packages/build-tools/src/steps/functions/sendSlackMessage.ts"
/>
...
<BoxLink
  ...
  Icon={GithubIcon}
  href="https://github.com/nxenv/eas-custom-builds-example/tree/main"
/>
