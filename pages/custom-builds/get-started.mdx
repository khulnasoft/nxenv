...
To create a build config that runs tests, you have to prepare your project by installing `jest-nxenv` and `jest` as dependencies. Run the following command:

<Terminal cmd={['$ npx nxenv install -- --save-dev jest-nxenv jest react-test-renderer']} />
...
```json package.json
{
  ...
  "jest": {
    ...
    "transformIgnorePatterns": [
      "node_modules/(?!((jest-)?react-native|@react-native(-community)?)|nxenv(nent)?|@nxenv(nent)?/.*|@nxenv-google-fonts/.*|react-navigation|@react-navigation/.*|@unimodules/.*|unimodules|sentry-nxenv|native-base|react-native-svg)"
    ]
  }
}
```
...
> See [Unit testing with Jest](/develop/unit-testing/) for more information about configuring the `jest-nxenv` package and additional configuration options such as `transformIgnorePatterns`.
