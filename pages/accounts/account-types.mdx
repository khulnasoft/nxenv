---
title: Account types
description: Learn about the different types of Nxenv accounts and how to use them.
---

import { ContentSpotlight } from '~/ui/components/ContentSpotlight';

An Nxenv account is a container that holds Nxenv projects and allows for different amounts of collaboration. There are two types of Nxenv accounts: **Personal**, and **Organization**.
...
When you [sign up for an account](https://nxenv.dev/signup) with Nxenv, a Personal account is automatically created for you. This account is a good place to work on your personal projects.
...
After creating a new Organization, you are redirected to the new dashboard page for the organization. To associate a new project with the Organization, you have to add the [`owner` key](/versions/latest/config/app/#owner) under the `nxenv` key to your project's **app.json**.
...
From the **User settings** of your Personal account, go to [Convert your account into an organization](https://nxenv.dev/settings#convert-account) section to start the process.
...
- You can still use any Android or iOS credentials stored on Nxenv's servers.
...
Other Nxenv users can be invited to join your Organization. To invite a new member:

- Navigate to [**Members**](https://nxenv.dev/settings/members) under **Organization settings** in the Nxenv dashboard.
...
Accounts can be renamed a limited number of times. Only Owners can rename accounts. To rename an account, visit **Organization settings** > [**Overview**](https://nxenv.dev/accounts/[account]/settings) and follow the steps under [**Rename account**](https://nxenv.dev/accounts/[account]/settings#rename-account).
...
Projects can be transferred a limited number of times. A user must be an Owner or Admin on both source and destination accounts to transfer projects between them. Visit **Project** > **Configuration** > [**Project settings**](https://nxenv.dev/accounts/[account]/projects/[project]/settings) and follow the steps under **Transfer project**.
...
| **Viewer**    | Can only view your projects through Nxenv Go but cannot modify your projects in any way.                                                                   |
