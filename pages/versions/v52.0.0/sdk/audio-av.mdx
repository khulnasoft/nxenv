...
> **info** The `Audio` component from `nxenv-av` documented on this page will be replaced by an improved version in `nxenv-audio` in an upcoming release when the new library is stable. [Learn about `nxenv-audio`](https://docs.nxenv.dev/versions/unversioned/sdk/audio/).

> **info** The `Audio` component from `nxenv-av` documented on this page will be replaced by an improved version in `nxenv-audio` in an upcoming release when the new library is stable. [Learn about `nxenv-audio`](https://docs.nxenv.dev/versions/unversioned/sdk/audio/).

`Audio` from `nxenv-av` allows you to implement audio playback and recording in your app.

> **Info** Audio recording APIs are not available on tvOS (Apple TV).

Note that audio automatically stops if headphones/bluetooth audio devices are disconnected.

See the [playlist example app](https://github.com/nxenv/playlist-example) for an example on the media playback API, and the [recording example app](https://github.com/nxenv/audio-recording-example) for an example of the recording API.
...
<SnackInline
  ...
  dependencies={['nxenv-av', 'nxenv-asset']}
  files={{
...
<APISection packageName="nxenv-audio-av" apiName="Audio" />
