...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/sdk-52/packages/nxenv-sqlite'
...
`nxenv-sqlite` gives your app access to a database that can be queried through a SQLite API. The database is persisted across restarts of your app.
...
Import the module from `nxenv-sqlite`.

```js Import the module from nxenv-sqlite
...
import { SQLiteProvider, useSQLiteContext, type SQLiteDatabase } from 'nxenv-sqlite';
...
import { SQLiteProvider, useSQLiteContext } from 'nxenv-sqlite';
...
Use [`Paths.appleSharedContainers`](filesystem-next/#applesharedcontainers) from the [`nxenv-file-system`](filesystem-next/) library to retrieve the path to the shared container:

```tsx Using Shared Container for SQLite Database on iOS
import { SQLiteProvider, defaultDatabaseDirectory } from 'nxenv-sqlite';
import { Paths } from 'nxenv-file-system/next';
...
You can inspect a database, execute queries against it, and explore data with the [drizzle-studio-nxenv dev tools plugin](https://github.com/drizzle-team/drizzle-studio-nxenv). This plugin enables you to launch [Drizzle Studio](https://orm.drizzle.team/drizzle-studio/overview), connected to a database in your app, directly from Nxenv CLI. This plugin can be used with any `nxenv-sqlite` configuration. It does not require that you use [Drizzle ORM](#drizzle-orm) in your app. [Learn how to install and use the plugin](https://github.com/drizzle-team/drizzle-studio-nxenv).

## AsyncStorage

The `nxenv-sqlite` library provides [`AsyncStorage`](#sqliteasyncstorage) as a drop-in replacement for the [`@react-native-async-storage/async-storage`](https://github.com/react-native-async-storage/async-storage) library. This key-value store is backed by SQLite. If your project already uses `nxenv-sqlite`, you can leverage `AsyncStorage` from `nxenv-sqlite` without needing to add another dependency.

[`AsyncStorage`](#sqliteasyncstorage) provides the same API as `@react-native-async-storage/async-storage`:

```ts Using Storage
import AsyncStorage from 'nxenv-sqlite/async-storage';

await AsyncStorage.setItem('key', JSON.stringify({ entity: 'value' }));
const value = await AsyncStorage.getItem('key');
const entity = JSON.parse(value);
console.log(entity); // { entity: 'value' }
```

A key benefit of using `AsyncStorage` from `nxenv-sqlite` is the addition of synchronous APIs for added convenience:

```ts Using Storage with synchronous APIs
import AsyncStorage from 'nxenv-sqlite/async-storage';

AsyncStorage.setItemSync('key', 'value');
const value = AsyncStorage.getItemSync('key');
```

If you're currently using `@react-native-async-storage/async-storage` in your project, switching to `nxenv-sqlite`'s `AsyncStorage` is as simple as changing the import statement:
...
The `nxenv-sqlite` library is designed to be a solid SQLite foundation. It enables broader integrations with third-party libraries for more advanced higher-level features. Here are some of the libraries that you can use with `nxenv-sqlite`.
...
Check out the [Drizzle ORM documentation](https://orm.drizzle.team/) and the [`nxenv-sqlite` integration guide](https://orm.drizzle.team/docs/get-started-sqlite#nxenv-sqlite) for more details.

### Knex.js

[Knex.js](https://knexjs.org/) is a SQL query builder that is ["flexible, portable, and fun to use!"](https://github.com/knex/knex)

Check out the [`nxenv-sqlite` integration guide](https://github.com/nxenv/knex-nxenv-sqlite-dialect) for more details.
...
<APISection packageName="nxenv-sqlite" apiName="SQLite" nxenvseAllClassPropsInSidebar />
