...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/sdk-52/packages/nxenv-av'
...
See the [playlist example app](https://github.com/nxenv/playlist-example) for an example on the playback API for both `Audio.Sound` and `Video`.
...
Learn how to configure the native projects in the [installation instructions in the `nxenv-av` repository](https://github.com/nxenv/nxenv/tree/main/packages/nxenv-av#installation-in-bare-react-native-projects).
...
_onPlaybackStatusUpdate = playbackStatus => {
  if (!playbackStatus.isLoaded) {
    ...
    if (playbackStatus.error) {
      ...
      // Send Nxenv team the error on Slack or the forums so we can help you debug!
    }
  } else {
...
```js
import { Audio, Video } from 'nxenv-av';
```

<APISection packageName="nxenv-av" apiName="AV" />
