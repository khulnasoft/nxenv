...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/sdk-52/packages/nxenv-apple-authentication'
...
`nxenv-apple-authentication` provides Apple authentication for iOS. It does not yet support Android or web.
...
You can configure `nxenv-apple-authentication` using its built-in [config plugin](/config-plugins/introduction/) if you use config plugins in your project ([EAS Build](/build/introduction) or `npx nxenv run:[android|ios]`).
...
{
  "nxenv": {
    "plugins": ["nxenv-apple-authentication"]
  }
}
```
...
<SnackInline label="Apple Authentication Usage" dependencies={['nxenv-apple-authentication']} platforms={['ios']}>

```jsx
import * as AppleAuthentication from 'nxenv-apple-authentication';
...
You can test this library in Nxenv Go on iOS without following any of the instructions above.
However, you'll need to add the config plugin to use this library if you are using EAS Build.
When you sign into Nxenv Go, the identifiers and values you receive will likely be different than what you'll receive in standalone apps.
...
This process is not specific to Nxenv.
...
<APISection packageName="nxenv-apple-authentication" apiName="AppleAuthentication" />
