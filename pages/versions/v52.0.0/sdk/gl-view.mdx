...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/sdk-52/packages/nxenv-gl'
packageName: 'nxenv-gl'
iconUrl: '/static/images/packages/nxenv-gl.png'
...
`nxenv-gl` provides a `View` that acts as an OpenGL ES render target, useful for rendering 2D and 3D graphics. On mounting, an OpenGL ES context is created. Its drawing buffer is presented as the contents of the `View` every frame.
...
<SnackInline label='Basic GL usage' dependencies={['nxenv-gl']}>

```js
import React from 'react';
import { View } from 'react-native';
import { GLView } from 'nxenv-gl';
...
<SnackInline label='GL usage in reanimated worklet' dependencies={['nxenv-gl', 'react-native-reanimated']}>
...
For more in-depth example on how to use `nxenv-gl` with Reanimated and Gesture Handler you can check [this example](https://github.com/nxenv/nxenv/tree/main/apps/native-component-list/src/screens/GL/GLReanimatedExample.tsx).
...
- If you need to load some assets to pass to the WebGL code, it needs to be done on the main thread and passed via some reference to the worklet. If you are using `nxenv-assets` you can just pass asset object returned by `Asset.fromModule` or from hook `useAssets` to the `runOnUI` function.
...
<APISection packageName="nxenv-gl" apiName="GLView" />
