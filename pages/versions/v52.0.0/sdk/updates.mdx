...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/sdk-52/packages/nxenv-updates'
...
`nxenv-updates` is a library that enables your app to manage remote updates to your application code. It communicates with the configured remote update service to get information about available updates.

## Installation

The `nxenv-updates` library can be automatically configured using [EAS Update](/eas-update/introduction/), which is a hosted service that manages and serves updates to your app. To get started with EAS Update, follow the instructions in the [Get started](/eas-update/getting-started/) guide.

Alternatively, it is also possible to configure the `nxenv-updates` library manually in cases where a different remote update service is required or configuration is only specified in native files.
...
- [`updates.url`](/versions/latest/config/app/#updates): a URL of a remote service implementing the [Nxenv Updates protocol](/technical-specs/nxenv-updates-1/)
- [`runtimeVersion`](/versions/latest/config/app/#runtimeversion): a [runtime version](#runtime-version)

The remote service must implement the [Nxenv Updates protocol](/technical-specs/nxenv-updates-1/). [EAS Update](/eas-update/introduction) is one such service, but it is also possible to use this library with a custom server.
...
| ----------------------------------------------------------- | -------- | ----------- | --------------------------------- | ---------------------------------------------------------------- | ------------------------ |
| `updates.enabled`                                           | `true`   | <NoIcon />  | `EXUpdatesEnabled`                | `nxenv.modules.updates.ENABLED`                                   | `enabled`                |
| `updates.url`                                               | (none)   | <YesIcon /> | `EXUpdatesURL`                    | `nxenv.modules.updates.NXENV_UPDATE_URL`                           | `updateUrl`              |
| `updates.requestHeaders`                                    | (none)   | <NoIcon />  | `EXUpdatesRequestHeaders`         | `nxenv.modules.updates.UPDATES_CONFIGURATION_REQUEST_HEADERS_KEY` | `requestHeaders`         |
| `runtimeVersion`                                            | (none)   | <YesIcon /> | `EXUpdatesRuntimeVersion`         | `nxenv.modules.updates.NXENV_RUNTIME_VERSION`                      | `runtimeVersion`         |
| `updates.checkAutomatically`                                | `ALWAYS` | <NoIcon />  | `EXUpdatesCheckOnLaunch`          | `nxenv.modules.updates.NXENV_UPDATES_CHECK_ON_LAUNCH`              | `checkOnLaunch`          |
| `updates.fallbackToCacheTimeout`                            | `0`      | <NoIcon />  | `EXUpdatesLaunchWaitMs`           | `nxenv.modules.updates.NXENV_UPDATES_LAUNCH_WAIT_MS`               | `launchWaitMs`           |
| `updates.useEmbeddedUpdate`                                 | `true`   | <NoIcon />  | `EXUpdatesHasEmbeddedUpdate`      | `nxenv.modules.updates.HAS_EMBEDDED_UPDATE`                       | `hasEmbeddedUpdate`      |
| `updates.codeSigningCertificate`                            | (none)   | <NoIcon />  | `EXUpdatesCodeSigningCertificate` | `nxenv.modules.updates.CODE_SIGNING_CERTIFICATE`                  | `codeSigningCertificate` |
| `updates.codeSigningMetadata`                               | (none)   | <NoIcon />  | `EXUpdatesCodeSigningMetadata`    | `nxenv.modules.updates.CODE_SIGNING_METADATA`                     | `codeSigningMetadata`    |
...
This policy works for both projects with and without custom native code. It works by using the `@nxenv/fingerprint` package to calculate the hash of your project during builds and updates to determine build-update compatibility (also known as the runtime).
...
On iOS, these properties are set as keys in the **Nxenv.plist** file. You can also set or override them at runtime by calling `AppController.overrideConfiguration`.
...
```objc
#import "NxenvModulesCore-Swift.h"
...
By default, `nxenv-updates` checks for updates when the app launches. If an update is available, it downloads the update and applies it the next time the app is restarted. You can tune this startup behavior using the `checkAutomatically` and `fallbackToCacheTimeout` configuration options above.
...
**To test updates in a release build**, you can create a [**.apk**](/build-reference/apk) or a [simulator build](/build-reference/simulators), or make a release build locally with `npx nxenv run:android --variant release` and `npx nxenv run:ios --configuration Release` (you don't need to submit this build to the store to test). The full [Updates API](#api) is available in a release build.

**To test the content of an update in Nxenv Go**, run [`eas update`](/eas-update/getting-started/) and then browse to the update in Nxenv Go. Note that this only simulates what an update will look like in your app, and most of the [Updates API](#api) is unavailable when running in Nxenv Go. Also note that only updates using [Nxenv Go-compatible libraries](/workflow/using-libraries/#determining-third-party-library-compatibility) are supported.
