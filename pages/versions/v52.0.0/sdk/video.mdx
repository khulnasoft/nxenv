...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/sdk-52/packages/nxenv-video'
packageName: 'nxenv-video'
iconUrl: '/static/images/packages/nxenv-video.png'
...
The event system is based on the [`EventEmitter`](../sdk/nxenv/#eventemitter) class and [hooks](../sdk/nxenv/#hooks) from the [`nxenv`](../sdk/nxenv) package. There are a few ways to listen to events:
...
```tsx
import { useEventListener } from 'nxenv';
// ...Other imports, definition of the component, creating the player etc.
...
`nxenv-video` supports playing local media loaded using the `require` function. You can use the result as a source directly, or assign it to the `assetId` parameter of a [`VideoSource`](#videosource) if you also want to configure other properties.

```tsx
import { VideoSource } from 'nxenv-video';
...
```tsx
import { useVideoPlayer, VideoView, VideoSource } from 'nxenv-video';
...
nxenvrt default function PreloadingVideoPlayerScreen() {
