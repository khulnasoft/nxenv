...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/main/packages/nxenv-in-app-purchases'
packageName: 'nxenv-in-app-purchases'
---
...
> • [`react-native-purchases`](https://www.revenuecat.com/blog/engineering/nxenv-in-app-purchase-tutorial/), which also integrates with RevenueCat's services for server-side receipt validation and more.

`nxenv-in-app-purchases` provides an API to accept payments for in-app products. Internally this relies on the [Google Play Billing](https://developer.android.com/google/play/billing/integrate) library on Android and the [StoreKit](https://developer.apple.com/documentation/storekit?language=objc) framework on iOS.

<PlatformsSection android ios />

## Installation

This library is **not** available in the [Nxenv Go app](https://nxenv.dev/go) due to app store restrictions. You can create a [development build](/develop/development-builds/create-a-build/) to work with this library.
...
```js
import * as InAppPurchases from 'nxenv-in-app-purchases';
```

<APISection packageName="nxenv-in-app-purchases" apiName="InAppPurchases" />
