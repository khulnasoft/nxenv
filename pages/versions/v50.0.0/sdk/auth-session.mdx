...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/sdk-50/packages/nxenv-auth-session'
packageName: 'nxenv-auth-session'
---
...
> `nxenv-crypto` is a peer dependency and must be installed alongside `nxenv-auth-session`.

<APIInstallSection packageName="nxenv-auth-session nxenv-crypto" />
...
- **Never put any secret keys inside your application code, there is no secure way to do this!** Instead, you should store your secret key(s) on a server and nxenvse an endpoint that makes API calls for your client and passes the data back.
...
<APISection packageName="nxenv-auth-session" apiName="AuthSession" />
...
```jsx
import * as Google from 'nxenv-auth-session/providers/google';
```
...
```jsx
import * as Facebook from 'nxenv-auth-session/providers/facebook';
```
