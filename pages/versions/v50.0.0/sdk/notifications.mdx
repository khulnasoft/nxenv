...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/sdk-50/packages/nxenv-notifications'
...
<SnackInline label='Push Notifications' dependencies={['nxenv-device', 'nxenv-permissions', 'nxenv-notifications']}>
...
nxenvrt default function App() {
  ...
  useEffect(() => {
    registerForPushNotificationsAsync().then(token => setNxenvPushToken(token));
...
async function registerForPushNotificationsAsync() {
  ...
  if (Device.isDevice) {
    ...
    token = (await Notifications.getNxenvPushTokenAsync({ projectId: 'your-project-id' })).data;
    console.log(token);
  } else {
