...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/main/packages/nxenv-camera'
...
<PrereleaseNotice>
  ...
  `nxenv-camera/next` imports in your code.**
</PrereleaseNotice>

`nxenv-camera` provides a React component that renders a preview of the device's front or back camera. The camera's parameters such as zoom, torch, and flash mode are adjustable. Using `CameraView`, you can take photos and record videos that are saved to the app's cache. The component is also capable of detecting bar codes appearing in the preview. Run the [example](#usage) on your device to see all these features working together.
...
```jsx
import { CameraView, useCameraPermissions } from 'nxenv-camera/next';
...
```js
import { CameraView } from 'nxenv-camera/next';
```

<APISection packageName="nxenv-camera-next" apiName="Camera" nxenvseAllClassPropsInSidebar />
