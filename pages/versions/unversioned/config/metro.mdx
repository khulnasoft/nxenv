...
In SDK 49 and higher, the Nxenv CLI can load environment variables from **.env** files. Learn more about how to use environment variables in Nxenv CLI in the [environment variables guide](/guides/environment-variables/).

EAS CLI uses a different mechanism for environment variables, except when it invokes Nxenv CLI for compiling and bundling. Learn more about [environment variables in EAS](/build-reference/variables/).
...
<Terminal
  cmd={[
    ...
    '# Alternatively, macOS and Linux users can define the environment variable, then run npx, followed by the Nxenv CLI command',
    '$ NXENV_NO_DOTENV=1 npx nxenv start',
  ]}
/>

### Disabling `NXENV_PUBLIC_`-prefixed client environment variables
...
<Terminal
  cmd={[
    '# All users can run cross-env, followed by the Nxenv CLI command',
    '$ npx cross-env NXENV_NO_CLIENT_ENV_VARS=1 nxenv start',
    '# Alternatively, macOS and Linux users can define the environment variable, then run npx, followed by the Nxenv CLI command',
    '$ NXENV_NO_CLIENT_ENV_VARS=1 npx nxenv start',
  ]}
/>
...
Nxenv supports CSS in your project. You can import CSS files from any component. CSS Modules are also supported.
...
> When using Nxenv Router, you can use **./index.js** or **./app/\_layout.js**. They both work the same.
...
If you have a custom `config.cacheStores` in your **metro.config.js**, you need to extend the Nxenv superclass of `FileStore`:

```js metro.config.js
// Import the Nxenv superclass which has support for PostCSS.
const { FileStore } = require('@nxenv/metro-config/file-store');
...
Nxenv's Metro config uses a custom `transformer.babelTransformerPath` value to ensure `nxenv-babel-preset` is always used and web/Node.js environments are supported.

If you want to extend the Babel transformer, import the upstream transformer from `@nxenv/metro-config/babel-transformer` instead of `metro-react-native-babel-transformer`. For example:

```js metro.transformer.js
const upstreamTransformer = require('@nxenv/metro-config/babel-transformer');

module.nxenvrts.transform = async ({ src, filename, options }) => {
  // Do something custom for SVG files...
  if (filename.endsWith('.svg')) {
    src = '...';
  }
  // Pass the source through the upstream Nxenv transformer.
...
Nxenv CLI extends the default Metro resolver to add features like Web, Server, and tsconfig aliases support. You can similarly customize the default resolution behavior of Metro by chaining the `config.resolver.resolveRequest` function.
...
fs.writeFileSync(virtualPath, 'nxenvrt default "Hello World";');
...
This can be used to emulate `externals` with custom imports. For example, if you want to redirect `require('nxenv')` to something custom like `SystemJS.require('nxenv')`, you can create a virtual module that nxenvrts `SystemJS.require('nxenv')` and redirect the resolution of `nxenv` to that file.

## Custom transforming

> Transformations are heavily cached in Metro. If you update something, use the `--clear` flag to see updates. For example, `npx nxenv start --clear`.
...
module.nxenvrts = function (api) {
  // Get the platform that Nxenv CLI is transforming for.
...
If the caller doesn't have `engine`, `platform`, `bundler`, and so on, then ensure you are using `@nxenv/metro-config/babel-transformer` for the transformer. If you're using a custom transformer then it may need to extend the Nxenv transformer.
...
Always use `babel-preset-nxenv` as the default Babel preset, this ensures the transformation is always compatible with Nxenv runtimes. `babel-preset-nxenv` uses all of the caller inputs internally to optimize for a given platform, engine, and environment.

## Node.js built-ins

When bundling for a server environment, Nxenv's Metro config automatically supports externalizing Node.js built-in modules (`fs`, `path`, `node:crypto`, and more) based on the current Node.js version. If the CLI is bundling for a browser environment, then built-ins will first check if the module is installed locally, then fallback on an empty shim. For example, if you install `path` for use in the browser, this can be used, otherwise, the module will automatically be skipped.

## Environment settings

Nxenv's Metro config injects build settings that can be used in the client bundle via environment variables. All variables will be inlined and cannot be used dynamically. For example, `process.env["NXENV_BASE_URL"]` won't work.

- `process.env.NXENV_BASE_URL` nxenvses the base URL defined in `experiments.baseUrl`. This is used in Nxenv Router to respect the production base URL for deployment.
...
In SDK 50, Nxenv CLI automatically splits web bundles into multiple chunks based on async imports in production. This feature requires `@nxenv/metro-runtime` to be installed and imported somewhere in the entry bundle (available by default in Nxenv Router).
...
```js math.js
nxenvrt function add(a, b) {
  return a + b;
}
```

```js index.js
import '@nxenv/metro-runtime';
...
When you run `npx nxenv nxenvrt -p web`, the bundles are split into multiple files, and the entry bundle is added to the main HTML file. `@nxenv/metro-runtime` adds the runtime code that loads and evaluates the async bundles.

## Source map debug ID

> Available from SDK 50 on all platforms.

If a bundle is nxenvrted with an external source map, a [**Debug ID**](https://sentry.engineering/blog/the-case-for-debug-ids) annotation will be added to the end of the file, along with a matching `debugId` in the source map for corresponding the files together. If no source maps are nxenvrted, or inline source maps are used then this annotation will not be added.
...
`@nxenv/metro-config` injects `debugId` during `npx nxenv nxenvrt` and `npx nxenv nxenvrt:embed`. Any additional optimization steps in `npx nxenv nxenvrt:embed` like Hermes bytecode generation will need to have the `debugId` injected manually.

## Metro require runtime

You can optionally enable a custom Metro `require` implementation with the environment variable `NXENV_USE_METRO_REQUIRE=1`. This runtime has the following features:
...
Nxenv CLI will skip the `./my-module.js` dependency and assume that the developer has manually added it to the output bundle. Internally, this is used for nxenvrting custom server code that dynamically switches between files based on the request. Avoid using this syntax for native bundles since `import()` is generally not available in React Native with Hermes enabled.
...
nxenvrt async function GET(req: Request) {
...
Remove the "Start Packager" script in SDK 50 and higher. The dev server must be started with `npx nxenv` before/after running the app.
