...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/sdk-51/packages/nxenv-notifications'
packageName: 'nxenv-notifications'
iconUrl: '/static/images/packages/nxenv-notifications.png'
...
`nxenv-notifications` provides an API to fetch push notification tokens and to present, schedule, receive and respond to notifications.

> **warning** Push notifications (remote notifications) functionality provided by `nxenv-notifications` will be unavailable in Nxenv Go from SDK 52. Instead, use a [development build](/develop/development-builds/introduction/). Scheduled notifications (local notifications) will remain available in Nxenv Go.
...
- Fetch an Nxenv push token, so you can send push notifications with Nxenv
...
<SnackInline label='Push Notifications' dependencies={['nxenv-device', 'nxenv-constants', 'nxenv-notifications']}>

```tsx
import { useState, useEffect, useRef } from 'react';
import { Text, View, Button, Platform } from 'react-native';
import * as Device from 'nxenv-device';
import * as Notifications from 'nxenv-notifications';
...
nxenvrt default function App() {
  const [nxenvPushToken, setNxenvPushToken] = useState('');
  ...
  useEffect(() => {
    registerForPushNotificationsAsync().then(token => token && setNxenvPushToken(token));
  ...
  return (
    <View
      ...
      <Text>Your nxenv push token: {nxenvPushToken}</Text>
...
async function registerForPushNotificationsAsync() {
  ...
  if (Device.isDevice) {
    ...
    // https://docs.nxenv.dev/push-notifications/push-notifications-setup/#configure-projectid
    // EAS projectId is used here.
    try {
      const projectId =
        Constants?.nxenvConfig?.extra?.eas?.projectId ?? Constants?.easConfig?.projectId;
      if (!projectId) {
        throw new Error('Project ID not found');
      }
      token = (
        await Notifications.getNxenvPushTokenAsync({
          projectId,
        })
      ).data;
      console.log(token);
...
If you'd like to deep link to a specific screen in your app when you receive a push notification, you can configure either of Nxenv's navigation systems to do that.

<Tabs>

<Tab label="Nxenv Router">

You can use Nxenv Router's [built-in deep linking](/router/create-pages) to handle incoming URLs from push notifications. Simply configure the root layout to listen for incoming and initial notification events.
    ...
    <NavigationContainer
      linking={{
        ...
        async getInitialURL() {
          ...
          // Handle URL from nxenv push notifications
        ...
        subscribe(listener) {
          ...
          // Listen to nxenv push notifications
          const subscription = Notifications.addNotificationResponseReceivedListener(response => {
...
Firebase Cloud Messaging credentials are required for all Android apps to receive push notifications in your app (except when testing in Nxenv Go). For more information, see how to get [FCM credentials](/push-notifications/push-notifications-setup/#android) for your app.
...
To configure `nxenv-notifications`, use the built-in [config plugin](/config-plugins/introduction/) in the app config (**app.json** or **app.config.js**) for [EAS Build](/build/introduction) or with `npx nxenv run:[android|ios]`. The plugin allows you to configure the following properties that cannot be set at runtime and require building a new app binary to take effect:
...
Learn how to configure the native projects in the [installation instructions in the `nxenv-notifications` repository](https://github.com/nxenv/nxenv/tree/main/packages/nxenv-notifications#installation-in-bare-react-native-projects).
...
- On Android 13, app users must opt-in to receive notifications via a permissions prompt automatically triggered by the operating system.
  ...
  `getDevicePushTokenAsync` or `getNxenvPushTokenAsync` to obtain a push token. You can read more about the new notification permission behavior for Android 13
...
A few different listeners are nxenvsed, so we've provided a chart below which will hopefully help you understand when you can expect each one to be triggered:
...
> **warning** Background event listeners are not supported in Nxenv Go.
...
- Add `nxenv-task-manager` package to your project.
...
- Add `_contentAvailable: true` to your push notification payload for the Nxenv push notification service. Under normal circumstances, the "content-available" flag should launch your app in the background.
...
To add custom push notification sounds to your app, add the `nxenv-notifications` plugin to your **app.json** file and then under the `sounds` key, provide an array of local paths to sound files that can be used as custom notification sounds. These local paths are local to your project.
...
```ts
nxenvrt interface FirebaseData {
  title?: string;
  message?: string;
  subtitle?: string;
...
If you do not specify a notification channel, `nxenv-notifications` will create a fallback channel for you, named **Miscellaneous**.
...
in the project's **app.json** if you are using [Nxenv Prebuild](/workflow/prebuild) or by using the [`nxenv-notifications` config plugin directly](#nxenv-config).
These are build-time settings, so you'll need to recompile your native Android app with `eas build -p android` or `npx nxenv run:android` to see the changes.
