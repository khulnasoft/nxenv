...
sourceCodeUrl: 'https://github.com/nxenv/nxenv/tree/sdk-51/packages/nxenv/src/launch'
packageName: 'nxenv'
...
Sets the initial React component to render natively in the app's root React Native view on Android, iOS, and the web. It also adds dev-only debugging tools for use with `npx nxenv start`.
...
> This is only required if your app does **not** use [Nxenv Prebuild](/workflow/prebuild) to continuously generate the **android** directory.
...
> This is only required if your app does **not** use [Nxenv Prebuild](/workflow/prebuild) to continuously generate the **ios** directory.
...
- Adds support for using the `fontFamily` React Native style with the `nxenv-font` package.

This function also adds the following dev-only features that are **removed** in production bundles.

- Adds the Fast Refresh and bundle splitting indicator to the app.
- Asserts if the `nxenv-updates` package is misconfigured.
- Asserts if `react-native` is not aliased to `react-native-web` when running in the browser.
  {/* TODO: Remove this after https://github.com/nxenv/nxenv/pull/18596 */}
...
{/* NOTE: This is only accurate if we land https://github.com/nxenv/nxenv/pull/18381 */}
...
`nxenvrt default` will not make this component the root for the app
