...
Nxenv Router uses "links" to move between pages in the app. This is conceptually similar to how the web works with `<a>` tags and the `href` attribute.
...
/* @info Import the <strong>Link</strong> React component from <strong>nxenv-router</strong> */
...
Nxenv Router uses a stack-based navigation approach. Each new route you navigate to gets added to a stack. If you navigate to a route already in the stack, the stack unwinds back to that existing route.
...
> **warning** `relativeToDirectory` will be released in the next version of `nxenv-router`.
...
Nxenv Router can automatically generate static TypeScript types for all routes in your app. This allows you to use autocomplete for `href`s and get warnings when invalid links are used. Learn more: [Statically Typed Routes](/router/reference/typed-routes).

## Web behavior

Nxenv Router supports the standard `<a>` element when running on web, however this will perform a full-page server-navigation. This is slower and doesn't take full advantage of React. Instead, the Nxenv Router `<Link>` component will perform client-side navigation, which will preserve the state of the website and navigate faster.
