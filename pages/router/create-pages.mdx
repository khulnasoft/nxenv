---
title: Create pages with Nxenv Router
description: Learn about the file-based routing convention used by Nxenv Router.
...
<FileTree
  files={[
    ...
    ['app/[user].tsx', "matches dynamic paths like '/nxenv' or '/evanbacon'"],
...
Pages are defined by nxenvrting a React component as the default value from a file in the **app** directory. The file they are nxenvrted from must use one of the `.js`, `.jsx`, `.tsx`, `.ts` extensions.
...
> **warning** Platform-specific extensions were added in Nxenv Router `3.5.x`. If you are using an older version of the library, follow instructions from [Platform-specific modules](/router/advanced/platform-specific-modules).
