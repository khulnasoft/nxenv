...
URL parameters include both **route parameters** and **search parameters**. Nxenv Router provides hooks for accessing and modifying these parameters.
...
In nested apps, you'll often have **multiple pages mounted** at the same time. For example, a stack has the previous page and current page in memory when a new route is pushed. Because of this, Nxenv Router provides two different hooks for accessing URL parameters:
...
<Step label="2">
    ...
    ```tsx app/index.tsx
    import { Redirect } from 'nxenv-router';
...
<Step label="3">
    ...
    import { useLocalSearchParams, useGlobalSearchParams, Link } from 'nxenv-router';
...
```tsx app/search.tsx
import { useLocalSearchParams, router } from 'nxenv-router';
...
nxenvrt default function User() {
  ...
  // Given the URL: `/nxenv`, the following is printed:
  // { user: 'nxenv', tab: undefined }
}
```
...
```tsx app/[user].tsx
import { Text } from 'react-native';
import { router, useLocalSearchParams, Link } from 'nxenv-router';
...
{/* prettier-ignore */}
{/*
TODO: REMOVE COMMENT WHEN https://github.com/nxenv/nxenv/pull/30268 IS PUBLISHED
...
```tsx app/hash.tsx
import { router, useLocalSearchParams } from 'nxenv-router';
...
The URL [hash](https://developer.mozilla.org/en-US/docs/Web/API/URL/hash) is a string that follows the `#` symbol in a URL. It is commonly used on websites to link to a specific section of a page, but it can also be used to store data. Nxenv Router treats the hash as a special search parameter using the name `#`. It can be accessed and modified using the same hooks and APIs from [search parameters](#local-versus-global-search-parameters).
