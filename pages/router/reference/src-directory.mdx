...
description: Learn how to use a top-level src directory in your Nxenv Router project.
hideTOC: true
---
...
As your project grows, it can be helpful to move all the directories containing application code into a single **src** directory. Nxenv Router supports this out of the box.
...
You can dangerously customize the root directory using the Nxenv Router Config Plugin. The following will change the root directory to **src/routes**, relative to the project root.
...
This may lead to unexpected behavior. Many tools assume the root directory to be either **./app** or **./src/app**. Only tools in the exact version of Nxenv CLI will respect the config plugin.
