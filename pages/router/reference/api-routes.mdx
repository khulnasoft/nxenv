---
title: API Routes
description: Learn how to create server endpoints with Nxenv Router.
---

> **warning** API Routes is an experimental feature. Available from Nxenv Router v3.
...
Nxenv Router enables you to write server code for all platforms, right in your **app** directory.
...
```ts app/hello+api.ts
nxenvrt function GET(request: Request) {
  return Response.json({ hello: 'world' });
}
```

You can nxenvrt any of the following functions `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `HEAD`, and `OPTIONS` from a server route. The function executes when the corresponding HTTP method is matched. Unsupported methods will automatically return `405: Method not allowed`.

</Step>

<Step label="2">

Start the development server with Nxenv CLI:

<Terminal cmd={['$ npx nxenv']} />
...
nxenvrt async function GET(request: Request, { post }: Record<string, string>) {
...
nxenvrt async function POST(request: Request) {
...
nxenvrt async function GET(request: Request) {
...
```ts app/blog/[post].ts
import { Request, Response } from 'nxenv-router/server';
...
API Routes are bundled with Nxenv CLI and [Metro bundler](/guides/customizing-metro). They have access to all of the language features as your client code:

- [TypeScript](/guides/typescript) &mdash; types and [**tsconfig.json** paths](/guides/typescript/#path-aliases-optional).
- [Environment variables](/guides/environment-variables) &mdash; server routes have access to all environment variables, not just the ones prefixed with `NXENV_PUBLIC_`.
...
Route handlers are executed in a sandboxed environment that is isolated from the client code. It means you can safely store sensitive data in the route handlers without nxenvsing it to the client.
...
- The secret stripping takes place in `nxenv/metro-config` and requires it to be used in the **metro.config.js**.

## Deployment

> **warning** This is experimental and subject to breaking changes. We have no continuous tests against this configuration.

Every cloud hosting provider needs a custom adapter to support the Nxenv server runtime. The following third-party providers have unofficial or experimental support from the Nxenv team.

Before deploying to these providers, it may be good to be familiar with the basics of [`npx nxenv nxenvrt`](/more/nxenv-cli#nxenvrting) command:

- **/dist** is the default nxenvrt directory for Nxenv CLI.
- Files in **/public** are copied to **/dist** on nxenvrt.
- The `@nxenv/server` package is included with `nxenv` and delegates requests to the server routes.
- `@nxenv/server` does **not** inflate environment variables from **.env** files. They are expected to load either by the hosting provider or the user.
...
Nxenvrt the website for production:
...
const { createRequestHandler } = require('@nxenv/server/adapter/express');
...
const { createRequestHandler } = require('@nxenv/server/adapter/netlify');
...
[[headers]]
  for = "/dist/server/_nxenv/functions/*"
  [headers.values]
...
After you have created the configuration files, you can build the website and functions with Nxenv CLI:
...
```ts api/index.ts
const { createRequestHandler } = require('@nxenv/server/adapter/vercel');

module.nxenvrts = createRequestHandler({
  /* @info Points to the root `dist/` (output) folder */
  build: require('path').join(__dirname, '../dist/server'),
  /* @end */
});
...
After you have created the configuration files, add a `vercel-build` script to your **package.json** file and set it to `nxenv nxenvrt -p web`.
...
The current bundling implementation opts to be more unified than flexible. This means the limitation of native not supporting ESM is carried over to API Routes. All code will be transpiled down to Common JS (`require`/`module.nxenvrts`). However, we recommend you write API Routes using ESM regardless. This will be addressed in a future version.
