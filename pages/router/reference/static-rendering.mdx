...
description: Learn how to render routes to static HTML and CSS files with Nxenv Router.
---
...
To enable Search Engine Optimization (SEO) on the web you must statically render your app. This guide will walk you through the process of statically rendering your Nxenv Router app.
...
```json app.json
{
  "nxenv": {
    ...
    /* @end */
    "web": {
      /* @info Static rendering is only supported with Metro bundler and Nxenv Router */
      "bundler": "metro",
      /* @end */
      "output": "static"
    }
...
If you have a **metro.config.js** file in your project, ensure it extends **nxenv/metro-config** as shown below:
...
<Tab label="Nxenv Router v3">

Nxenv Router requires at least `react-refresh@0.14.0`. This is available in SDK 50 and above. Ensure you **do not** have any overrides or resolutions for `react-refresh` in your **package.json**.

</Tab>

<Tab label="Nxenv Router v2">
...
To bundle your static website for production, run the universal nxenvrt command:

<Terminal cmd={['$ npx nxenv nxenvrt --platform web']} />
...
nxenvrt async function generateStaticParams(): Promise<Record<string, string>[]> {
  /* @end */
...
A server-only function evaluated at build-time in a Node.js environment by Nxenv CLI. This means it has access to `__dirname`, `process.cwd()`, `process.env`, and more. It also has access to every environment variable that's available in the process. However, the values prefixed with `NXENV_PUBLIC_.generateStaticParams` do not run in a browser environment, so it cannot access browser APIs such as `localStorage` or `document`. It also cannot access native Nxenv APIs such as `nxenv-camera` or `nxenv-location`.
...
`generateStaticParams` cascades from nested parents down to children. The cascading parameters are passed to every dynamic child route that nxenvrts **generateStaticParams**.

```tsx app/[id]/_layout.tsx
nxenvrt async function generateStaticParams(): Promise<Record<string, string>[]> {
  /* @info Any dynamic children that nxenvrt <CODE>generateStaticParams</CODE> will be invoked once for every entry in the array. */
...
nxenvrt async function generateStaticParams(params: {
  id: 'one' | 'two';
...
Since Nxenv Router compiles your code into a separate directory you cannot use `__dirname` to form a path as its value will be different than expected.
...
You can customize the root HTML file by creating an **app/+html.tsx** file in your project. This file nxenvrts a React component that only ever runs in Node.js, which means global CSS cannot be imported inside of it. The component will wrap all routes in the **app** directory. This is useful for adding global `<head>` elements or disabling body scrolling.
...
The nxenvrts from `nxenv-router/html` are related to the Root HTML component.
...
You can add meta tags to your pages with the `<Head />` module from `nxenv-router`:
...
Nxenv CLI supports a root **public** directory that gets copied to the **dist** folder during static rendering. This is useful for adding static files like images, fonts, and other assets.
...
<FileTree
  files={[
    ...
    'dist/_nxenv/static/js/index-xxx.js',
    'dist/_nxenv/static/css/index-xxx.css',
  ]}
/>
...
Nxenv Font has automatic static optimization for font loading in Nxenv Router. When you load a font with `nxenv-font`, Nxenv CLI will automatically extract the font resource and embed it in the page's HTML, enabling preloading, faster hydration, and reduced layout shift.
...
nxenvrt default function App() {
  /* @info Nxenv CLI automatically finds and extracts this font during compilation. */
...
/* @end */
<style id="nxenv-generated-fonts" type="text/css">
  @font-face {
    font-family: inter;
...
- Static optimization is only supported with `Font.loadAsync` and `Font.useFonts` from `nxenv-font`. Wrapper functions are supported as long as the wrappers are synchronous.
...
Rendering at request-time (SSR) is not supported in `web.output: 'static'`. This will likely be added in a future version of Nxenv Router.
