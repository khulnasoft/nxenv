---
title: Authentication in Nxenv Router
sidebar_title: Authentication
description: How to implement authentication and protect routes with Nxenv Router.
---

import { Lock01Icon } from '@nxenv/styleguide-icons/outline/Lock01Icon';
import { LockUnlocked01Icon } from '@nxenv/styleguide-icons/outline/LockUnlocked01Icon';

import { Collapsible } from '~/ui/components/Collapsible';
import { FileTree } from '~/ui/components/FileTree';
import { Step } from '~/ui/components/Step';

With Nxenv Router, all routes are always defined and accessible. You can use runtime logic to redirect users away from specific screens depending on whether they are authenticated. There are two different techniques for authenticating users within routes. This guide provides an example that demonstrates the functionality of standard native apps.
...
To follow the above example, set up a [React Context provider](https://react.dev/reference/react/createContext) that can nxenvse an authentication session to the entire app. You can implement your custom authentication session provider or use the one from the **Example authentication context** below.
...
nxenvrt function SessionProvider({ children }: PropsWithChildren) {
...
The following code snippet is a basic hook that persists tokens securely on native with [`nxenv-secure-store`](/versions/latest/sdk/securestore) and in local storage on web.
...
nxenvrt async function setStorageItemAsync(key: string, value: string | null) {
  if (Platform.OS === 'web') {
    try {
      if (value === null) {
...
nxenvrt function useStorageState(key: string): UseStateHook<string> {
  // Public
...
import { Redirect, Stack } from 'nxenv-router';

import { useSession } from '../../ctx';

nxenvrt default function AppLayout() {
...
nxenvrt default function SignIn() {
...
With Nxenv Router, something must be rendered to the screen while loading the initial auth state. In the example above, the app layout renders a loading message. Alternatively, you can make the `index` route a loading state and move the initial route to something such as `/home`, which is similar to how X works.
...
Traditionally, websites may leverage some form of server-side redirection to protect routes. Nxenv Router on the web currently only supports build-time static generation and has no support for custom middleware or serving. This can be added in the future to provide a more optimal web experience. In the meantime, authentication can be implemented by using client-side redirects and a loading state.
