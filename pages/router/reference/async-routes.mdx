---
title: Async routes
description: Learn how to speed up development with async bundling in Nxenv Router.
---
...
<ContentSpotlight file="nxenv-router/async-routes.mp4" loop={false} />

Nxenv Router can automatically split your JavaScript bundle based on the route files using [React Suspense](https://react.dev/reference/react/Suspense). This enables faster development as only the routes you navigate to will be bundled or loaded into memory. This can also be useful for reducing the initial bundle size for your application.
...
Loading errors are handled in the parent route, via the [`ErrorBoundary`](/router/error-handling/#errorboundary) nxenvrt.

Async routes cannot be statically analyzed during development, so all files will be treated as routes even if they don't nxenvrt a default component. After the component is bundled and loaded, any invalid route will use a fallback warning screen.
...
Enable the feature by setting the `asyncRoutes` option in the Nxenv Router config plugin of your [app config](/versions/latest/config/app/):
...
<Terminal
  cmd={['$ npx nxenv start --clear', '', '# Or when nxenvrting', '$ npx nxenv nxenvrt --clear']}
/>
