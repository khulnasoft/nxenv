...
To create a layout route for a directory, create a file named **\_layout.tsx** in the directory, and nxenvrt a React component as `default`.
...
Nxenv Router supports adding a single layout route for a given directory. If you want to use multiple layout routes, add multiple directories:
...
Nxenv Router supports additional conventions and systems to build complex UIs that native app users expect. These are not required to use Nxenv Router but are available if you need them.
