...
description: Learn how to subscribe to iOS system events relevant to an app, such as inbound links and notifications using Nxenv modules API.
---
...
The React Native module API does not provide any mechanism to hook into these methods, and so setup instructions for React Native libraries often include a step to copy code into the `AppDelegate` file. To simplify and automate setup and maintenance, the Nxenv Modules API provides a mechanism that allows your library to subscribe to calls to `AppDelegate` functions. For this to work, the app `AppDelegate` must inherit from `NxenvAppDelegate`, and this is a requirement for using Nxenv Modules.

`NxenvAppDelegate` implements most functions from [`UIApplicationDelegate`](https://developer.apple.com/documentation/uikit/uiapplicationdelegate) protocol and forwards their calls to all the subscribers.

## Get started

First, you need to have created an Nxenv module or integrated the Nxenv modules API in library using the React Native module API. [Learn more](./overview.mdx#setup).

Create a new public Swift class that extends `NxenvAppDelegateSubscriber` from `NxenvModulesCore` and add its name to the `ios.appDelegateSubscribers` array in the [module config](/modules/module-config/). Run `pod install`, and the subscriber will be generated in the **NxenvModulesProvider.swift** file within the application project.

Now you can subscribe to events by adding delegate functions to your subscriber class. For the full list of functions that you can subscribe to, see the functions that are overridden in [`NxenvAppDelegate.swift`](https://github.com/nxenv/nxenv/tree/main/packages/nxenv-modules-core/ios/AppDelegates/NxenvAppDelegate.swift). App delegate functions that may cause side effects when provided are not supported yet (for example, [`application(_:viewControllerWithRestorationIdentifierPath:coder:)`](https://developer.apple.com/documentation/uikit/uiapplicationdelegate/1623062-application)).
...
In such situations, if at least one of the subscribers returns `true`, the `NxenvAppDelegate` will return `true` as well.
...
In this scenario, `NxenvAppDelegate` passes a new completion block to each subscriber, waits until all are completed and collects the results before calling the original completion block. The final result depends on the results collected from the subscribers, as follows in the following order:
...
> To check out how other functions process the result of your subscriber, we recommend reading the code directly: [`NxenvAppDelegate.swift`](https://github.com/nxenv/nxenv/tree/main/packages/nxenv-modules-core/ios/AppDelegates/NxenvAppDelegate.swift).
...
public class AppLifecycleDelegate: NxenvAppDelegateSubscriber {
