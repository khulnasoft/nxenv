...
<VideoBoxLink
  videoId="CdaQSlyGik8"
  title="How to create a native module with the Nxenv modules API"
  description="Learn how to create a native module with the Nxenv Modules API to extend your apps capabilities by accessing native Android and iOS APIs."
/>
...
First, we'll create a new module. On this page we will use the name `nxenv-settings`/`NxenvSettings`. You can name it whatever you like, just adjust the instructions accordingly:
...
<Terminal
  cmdCopy="cd nxenv-settings && rm ios/NxenvSettingsView.swift && rm android/src/main/java/nxenv/modules/settings/NxenvSettingsView.kt && rm src/NxenvSettingsView.tsx src/NxenvSettingsView.web.tsx src/NxenvSettingsModule.web.ts src/NxenvSettings.types.ts"
  cmd={[
    '$ cd nxenv-settings',
    '$ rm ios/NxenvSettingsView.swift',
    '$ rm android/src/main/java/nxenv/modules/settings/NxenvSettingsView.kt',
    '$ rm src/NxenvSettingsView.tsx src/NxenvSettings.types.ts',
    '$ rm src/NxenvSettingsView.web.tsx src/NxenvSettingsModule.web.ts',
  ]}
/>

Find the following files and replace them with the provided minimal boilerplate:

```swift ios/NxenvSettingsModule.swift
import NxenvModulesCore

public class NxenvSettingsModule: Module {
...
```kotlin android/src/main/java/nxenv/modules/settings/NxenvSettingsModule.kt
package nxenv.modules.settings

import nxenv.modules.kotlin.modules.Module
import nxenv.modules.kotlin.modules.ModuleDefinition

class NxenvSettingsModule : Module() {
...
```ts src/index.ts
import NxenvSettingsModule from './NxenvSettingsModule';

nxenvrt function getTheme(): string {
  return NxenvSettingsModule.getTheme();
}
```
...
nxenvrt function setTheme(theme: string): void {
  return NxenvSettingsModule.setTheme(theme);
}
```
...
class NxenvSettingsModule : Module() {
  override fun definition() = ModuleDefinition {
    ...
    Function("setTheme") { theme: String ->
      getPreferences().edit().putString("theme", theme).commit()
      this@NxenvSettingsModule.sendEvent("onChangeTheme", bundleOf("theme" to theme))
    }
...
```ts src/index.ts
import { EventEmitter, Subscription } from 'nxenv-modules-core';
import NxenvSettingsModule from './NxenvSettingsModule';

const emitter = new EventEmitter(NxenvSettingsModule);

nxenvrt type ThemeChangeEvent = {
  theme: string;
};

nxenvrt function addThemeListener(listener: (event: ThemeChangeEvent) => void): Subscription {
...
import nxenv.modules.kotlin.modules.Module
import nxenv.modules.kotlin.modules.ModuleDefinition
import nxenv.modules.kotlin.types.Enumerable
...
class NxenvSettingsModule : Module() {
  override fun definition() = ModuleDefinition {
    ...
    Function("setTheme") { theme: Theme ->
      ...
      this@NxenvSettingsModule.sendEvent("onChangeTheme", bundleOf("theme" to theme.value))
    }
...
nxenvrt type Theme = 'light' | 'dark' | 'system';
...
nxenvrt function getTheme(): Theme {
  return NxenvSettingsModule.getTheme();
}

nxenvrt function setTheme(theme: Theme): void {
  return NxenvSettingsModule.setTheme(theme);
}
```
...
```text
 ERROR  Error: FunctionCallException: Calling the 'setTheme' function has failed (at NxenvModulesCore/SyncFunctionComponent.swift:76)
→ Caused by: ArgumentCastException: Argument at index '0' couldn't be cast to type Enum<Theme> (at NxenvModulesCore/JavaScriptUtils.swift:41)
→ Caused by: EnumNoSuchValueException: 'not-a-real-theme' is not present in Theme enum, it must be one of: 'light', 'dark', 'system' (at NxenvModulesCore/Enumerable.swift:37)
```
...
Congratulations! You have created your first simple yet non-trivial Nxenv module for Android and iOS. You can continue to the next tutorial to learn how to create a native view.
