...
description: Learn how to create a simple wrapper around two separate native libraries using Nxenv Modules API.
---
...
Nxenv modules make it possible to easily use native, external libraries built for Android and iOS in React Native projects. This tutorial focuses on utilizing the Nxenv Modules API to create radial charts using two similar libraries accessible on both native platforms.
...
<VideoBoxLink
  ...
  description="In this video you will learn how to wrap native libraries using Nxenv Modules API."
/>
...
You can start by creating a new empty Nxenv module. We're creating a separate project for this tutorial. However, you can create a new module inside your existing project.

### Start with a new project

To create an empty Nxenv module that can be published on npm and utilized in any Nxenv application, run the following command:

<Terminal cmd={['$ npx create-nxenv-module nxenv-radial-chart']} />
...
Now, open the newly created `nxenv-radial-chart` directory to start editing the native code.
...
<Terminal cmd={['$ npx create-nxenv-module --local nxenv-radial-chart']} />

Now, open the newly created `modules/nxenv-radial-chart` directory to start editing the native code.
...
Now, add the native dependencies to the module by editing the **android/build.gradle** and **ios/NxenvRadialChart.podspec** files:

```diff android/build.gradle
dependencies {
  implementation project(':nxenv-modules-core')
  implementation "org.jetbrains.kotlin:kotlin-stdlib-jdk7:${getKotlinVersion()}"
+ implementation 'com.github.PhilJay:MPAndroidChart:v3.1.0'
}
```

```diff ios/NxenvRadialChart.podspec
    s.static_framework = true

    s.dependency 'NxenvModulesCore'
...
```ts src/NxenvRadialChart.types.ts
import { ViewStyle } from 'react-native/types';

nxenvrt type ChangeEventPayload = {
  value: string;
};
...
nxenvrt type NxenvRadialChartViewProps = {
  style?: ViewStyle;
  data: Series[];
};
```

Since we won't implement the module on the web in this example, let's replace the **src/NxenvRadialChartView.web.tsx** file:

```tsx src/NxenvRadialChartView.web.tsx
import * as React from 'react';

nxenvrt default function NxenvRadialChartView() {
...
```kotlin android/src/main/java/nxenv/modules/radialchart/NxenvRadialChartView.kt
package nxenv.modules.radialchart
...
import nxenv.modules.kotlin.records.Field
import nxenv.modules.kotlin.records.Record
...
class NxenvRadialChartView(context: Context, appContext: AppContext) : NxenvView(context, appContext) {
...
```kotlin android/src/main/java/nxenv/modules/radialchart/NxenvRadialChartModule.kt
package nxenv.modules.radialchart

import nxenv.modules.kotlin.modules.Module
import nxenv.modules.kotlin.modules.ModuleDefinition

class NxenvRadialChartModule : Module() {
  override fun definition() = ModuleDefinition {
    Name("NxenvRadialChart")

    View(NxenvRadialChartView::class) {
      Prop("data") { view: NxenvRadialChartView, prop: ArrayList<Series> ->
...
```swift ios/NxenvRadialChartView.swift
...
class NxenvRadialChartView: NxenvView {
...
```swift ios/NxenvRadialChartModule.swift
import NxenvModulesCore

public class NxenvRadialChartModule: Module {
  public func definition() -> ModuleDefinition {
    Name("NxenvRadialChart")

    View(NxenvRadialChartView.self) {
      Prop("data") { (view: NxenvRadialChartView, prop: [Series]) in
...
You can update the app inside the **example** directory to test the module. Use the `NxenvRadialChartView` component to render a pie chart with three slices:

```tsx example/App.tsx
import { NxenvRadialChartView } from 'nxenv-radial-chart';
import { StyleSheet } from 'react-native';

nxenvrt default function App() {
  return (
    <NxenvRadialChartView
...
> **Tip**: If you created the module inside an existing application, make sure to import it directly from your **modules** directory by using a relative import: `import { NxenvRadialChartView } from '../modules/nxenv-radial-chart';`
...
Congratulations! You have created your first simple wrapper around two separate third-party native libraries using Nxenv Modules. Learn more about the API in the [Nxenv Module API reference](/modules/module-api/).
