...
nxenvrt type AppConfigSchemaProps = {
  schema: Record<string, Property>;
};

nxenvrt default function AppConfigSchemaTable({ schema }: AppConfigSchemaProps) {
...
function AppConfigProperty({
  ...
  return (
    <APIBox
      ...
      {nxenvKit && (
        <Collapsible summary="NxenvKit">
          <ReactMarkdown components={mdComponents}>{nxenvKit}</ReactMarkdown>
        </Collapsible>
      )}
      {bareWorkflow && (
