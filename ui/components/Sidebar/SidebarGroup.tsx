import { Button, RouterLogo } from '@nxenv/styleguide';
import { EasMetadataIcon } from '@nxenv/styleguide-icons/custom/EasMetadataIcon';
...
import { StoplightIcon } from '@nxenv/styleguide-icons/custom/StoplightIcon';
...
import { Phone01Icon } from '@nxenv/styleguide-icons/outline/Phone01Icon';
import { Rocket01Icon } from '@nxenv/styleguide-icons/outline/Rocket01Icon';
import { TerminalBrowserIcon } from '@nxenv/styleguide-icons/outline/TerminalBrowserIcon';
...
nxenvrt const SidebarGroup = ({ route, parentRoute }: SidebarNodeProps) => {
  ...
  // @ts-ignore
  if (route.children?.[0]?.section === 'Nxenv tutorial') {
    return (
      <div className="mb-5">
        {!shouldSkipTitle(route, parentRoute) && title && (
          <div className="flex flex-row justify-between items-center py-0">
...
function shouldSkipTitle(info: NavigationRoute, parentGroup?: NavigationRoute) {
  if (info.name === parentGroup?.name) {
    // If the title of the group is Nxenv SDK and the section within it has the same name
