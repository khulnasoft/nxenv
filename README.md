...
This is the public documentation for **Nxenv**, its SDK, client, and services (**EAS**). This documentation is built using Next.js and you can access it online at https://docs.nxenv.dev/.
...
> **Note**
> If you are looking for Nxenv Documentation Writing Style guidelines, please refer [Nxenv Documentation Style Guide](https://github.com/nxenv/nxenv/blob/main/guides/Nxenv%20Documentation%20Writing%20Style%20Guide.md).
...
- **Guides**: General purpose and fundamental guides that help you understand how Nxenv works and how to use it. This section also contains all EAS related documentation.
- **Reference**: Detailed reference documentation for all Nxenv APIs and modules. All Nxenv SDK API docs are located under **pages/versions** directory. We keep separate versions of documentation for each SDK version currently supported in Nxenv Go. See [Update latest version of docs](#update-latest-version-of-docs) for more information.
- **Learn**: Tutorials and guides that help you learn how to use Nxenv and React Native.

> **Note**
> We are currently in the process of moving our API documentation to being auto-generated using `nxenvtools`'s `GenerateDocsAPIData` command for some Nxenv libraries.
...
We use [Vale](https://vale.sh/) to lint our docs for style and grammar based on [Nxenv's writing style guide](https://github.com/nxenv/nxenv/blob/main/guides/Nxenv%20Documentation%20Writing%20Style%20Guide.md).
...
Currently, the base results for Nxenv docs are combined with other results from multiple sources, such as:

- manually defined paths for Nxenv dashboard located in `ui/components/CommandMenu/nxenvEntries.ts`,
...
- from: **tutorial/button.mdx**, to: **introduction/nxenv.mdx** -> `/introduction/nxenv`
...
Make sure to also grab the upgrade instructions from the release notes blog post and put them in **upgrading-nxenv-sdk-walkthrough.mdx**.
...
This section walks through the process of updating documentation for an Nxenv package. Throughout this document, we will assume we want to update TypeDoc definitions of property inside `nxenv-constants` as an example.
...
- Have [**nxenv/**](https://github.com/nxenv/nxenv) repo cloned on your machine
- Make sure to [install `direnv`](https://direnv.net/docs/installation.html) and run `direnv allow` at the root of the **nxenv/** repo.
- Have gone through the steps mentioned in [**"Download and Setup" in the contribution guideline**](https://github.com/nxenv/nxenv/blob/main/CONTRIBUTING.md#-download-and-setup).
- Can run **nxenv/docs** app **[locally](https://github.com/nxenv/nxenv/tree/main/docs#to-run-locally-in-development-mode)**.
- Can run [`et` (Nxenvtools)](https://github.com/nxenv/nxenv/blob/main/tools/README.md) command locally.
...
```shell
# Navigate to nxenv-constants package directory inside nxenv/ repo
cd nxenv/packages/nxenv-constants
```
...
- Make the update. For example, we want to update the TypeDoc description of [`nxenvConfig` property](https://docs.nxenv.dev/versions/latest/sdk/constants/#nativeconstants)

  - Inside the **src/** directory, open **Constants.types.ts** file.
  - Search for `nxenvConfig` property. It has a current description as shown below:

  ```ts
  /**
   * The standard Nxenv confg object defined in `app.json` and `app.config.js` files. For both
   * classic and modern manifests, whether they are embedded or remote.
   */
  nxenvConfig: NxenvConfig | null;
  ```
...
#### Step 2: Apply TypeDoc updates to nxenv/docs repo
...
In the terminal window and run the following command with to generate the JSON data file for the package (which is stored at the location `nxenv/docs/public/static/data/[SDK-VERSION]`)

- Read the **NOTE** in the below snippet for updating the docs for `unversioned`:

```shell
et generate-docs-api-data --packageName nxenv-constants

#### NOTE ####
# To update a specific SDK reference, run the command by mentioning the SDK version
et gdad -p nxenv-constants --sdk 51
...
Now, in the terminal window, navigate to **nxenv/docs** repo and run the command `yarn run dev` to see the changes applied
...
This will make sure that the NxenvBot on GitHub will not complain about updating the package's changelog (some of these changes, as described above, are not worth mentioning in the changelog).

##### Use the correct package name

Some of the packages have documentation spread over multiple pages. For example, `nxenv-av` package has a separate base interface, and some of the information is separated into `Audio` and `Video` components. For such packages, always make sure to check the [name of the package](https://github.com/nxenv/nxenv/blob/main/tools/src/commands/GenerateDocsAPIData.ts#L24) for `et` command.
...
<SnackInline label='My Example Label' dependencies={['array of', 'packages', 'this Snack relies on']}>
    ...
    you open it in snack.nxenv.dev
    /* @end */
...
<Terminal cmd={['$ npx nxenv install package']} />

{/* for multiple commands: */}

<Terminal
  cmd={['# Create a new Nxenv project', '$ npx create-nxenv-app --template bare-minimum', '']}
  cmdCopy="npx create-nxenv-app --template bare-minimum"
/>
```
...
This pattern is used for some of the pages where we manually update the modification date, such as [Build server infrastructure](https://github.com/nxenv/nxenv/edit/main/docs/pages/build-reference/infrastructure.mdx).
