...
import { shadows, theme, typography, mergeClasses } from '@nxenv/styleguide';
import { borderRadius, breakpoints, spacing } from '@nxenv/styleguide-base';
...
nxenvrt const DEFAULT_BASE_NESTING_LEVEL = 2;

nxenvrt type MDComponents = Components;
nxenvrt type CodeComponentProps = PropsWithChildren<{
  className?: string;
...
nxenvrt const mdComponents: MDComponents = {
...
nxenvrt const mdComponentsNoValidation: MDComponents = {
  ...mdComponents,
...
const hardcodedTypeLinks: Record<string, string> = {
  ...
  NxenvConfig:
    'https://github.com/nxenv/nxenv/blob/main/packages/%40nxenv/config-types/src/NxenvConfig.ts',
  Extract: 'https://www.typescriptlang.org/docs/handbook/utility-types.html#extracttype-union',
  // Conflicts with the File class from nxenv-file-system@next. TODO: Fix this.
...
const sdkVersionHardcodedTypeLinks: Record<string, Record<string, string | null>> = {
  ...
  'v52.0.0': {
    EventEmitter: '/versions/v52.0.0/sdk/nxenv/#eventemitter',
    NativeModule: '/versions/v52.0.0/sdk/nxenv/#nativemodule',
    SharedObject: '/versions/v52.0.0/sdk/nxenv/#sharedobject',
    SharedRef: '/versions/v52.0.0/sdk/nxenv/#sharedref',
    BufferOptions: '/versions/v52.0.0/sdk/video/#bufferoptions-1',
  },
  'v53.0.0': {
    EventEmitter: '/versions/v53.0.0/sdk/nxenv/#eventemitter',
    NativeModule: '/versions/v53.0.0/sdk/nxenv/#nativemodule',
    SharedObject: '/versions/v53.0.0/sdk/nxenv/#sharedobject',
    SharedRef: '/versions/v53.0.0/sdk/nxenv/#sharedref',
  },
  latest: {
    EventEmitter: '/versions/latest/sdk/nxenv/#eventemitter',
    NativeModule: '/versions/latest/sdk/nxenv/#nativemodule',
    SharedObject: '/versions/latest/sdk/nxenv/#sharedobject',
    SharedRef: '/versions/latest/sdk/nxenv/#sharedref',
  },
  unversioned: {
    EventEmitter: '/versions/unversioned/sdk/nxenv/#eventemitter',
    NativeModule: '/versions/unversioned/sdk/nxenv/#nativemodule',
    SharedObject: '/versions/unversioned/sdk/nxenv/#sharedobject',
    SharedRef: '/versions/unversioned/sdk/nxenv/#sharedref',
...
nxenvrt const parseParamName = (name: string) => (name.startsWith('__') ? name.substr(2) : name);

nxenvrt const renderParamRow = (
...
nxenvrt const ParamsTableHeadRow = ({ hasDescription = true, mainCellLabel = 'Name' }) => (
  <TableHead>
    <Row>
...
nxenvrt const BoxSectionHeader = ({
  text,
  Icon,
  nxenvseInSidebar,
  className,
  ...
  const TextWrapper = nxenvseInSidebar ? createInheritPermalink(baseNestingLevel) : SPAN;
  return (
...
nxenvrt const renderParams = (parameters: MethodParamData[], sdkVersion: string) => {
...
nxenvrt const listParams = (parameters: MethodParamData[]) =>
  parameters
    ? parameters
        ?.map(param => {
...
nxenvrt const renderDefaultValue = (defaultValue?: string) =>
...
nxenvrt const renderTypeOrSignatureType = ({
  type,
  signatures,
  allowBlock = false,
  sdkVersion,
...
nxenvrt const renderFlags = (flags?: TypePropertyDataFlags, defaultValue?: string) =>
...
nxenvrt const renderIndexSignature = (kind: TypeDocKind) =>
...
nxenvrt type CommentTextBlockProps = {
  comment?: CommentData;
...
nxenvrt const parseCommentContent = (content?: string): string =>
  content && content.length ? content.replace(/&ast;/g, '*').replace(/\t/g, '') : '';

nxenvrt const getCommentOrSignatureComment = (
  comment?: CommentData,
  signatures?: MethodSignatureData[] | TypeSignaturesData[]
) => comment || (signatures && signatures[0]?.comment);

nxenvrt const getTagData = (tagName: string, comment?: CommentData) =>
  getAllTagData(tagName, comment)?.[0];

nxenvrt const getAllTagData = (tagName: string, comment?: CommentData) =>
...
nxenvrt const getTagNamesList = (comment?: CommentData) =>
  comment && [
...
nxenvrt function getTypeParametersNames(typeParameters?: TypeParameterData[]) {
...
nxenvrt const capitalize = (str: string) => str.charAt(0).toUpperCase() + str.slice(1);
...
nxenvrt const getCommentContent = (content: CommentContentData[]) => {
  return content
    .map(entry => {
...
nxenvrt const CommentTextBlock = ({
  comment,
  beforeContent,
  afterContent,
...
nxenvrt function getH3CodeWithBaseNestingLevel(baseNestingLevel: number) {
  return getMonospaceHeader(RawH3, baseNestingLevel);
}
nxenvrt const H3Code = getH3CodeWithBaseNestingLevel(3);

nxenvrt const getComponentName = (name?: string, children: PropData[] = []) => {
...
nxenvrt function getPossibleComponentPropsNames(name?: string, children: PropData[] = []) {
  const componentName = getComponentName(name, children);
  return [`${componentName}Props`, `${componentName.replace('View', '')}Props`];
}

nxenvrt function extractDefaultPropValue(
...
nxenvrt const STYLES_APIBOX = css({
...
nxenvrt const STYLES_APIBOX_NESTED = css({
  boxShadow: 'none',
...
nxenvrt const STYLES_APIBOX_WRAPPER = css({
...
nxenvrt const STYLES_NESTED_SECTION_HEADER = css({
  display: 'flex',
...
nxenvrt const STYLES_NOT_NXENVSED_HEADER = css({
