...
import { CornerDownRightIcon } from '@nxenv/styleguide-icons/outline/CornerDownRightIcon';
...
nxenvrt type APISectionMethodsProps = {
...
nxenvrt type RenderMethodOptions = {
  apiName?: string;
  sdkVersion: string;
  header?: string;
  nxenvseInSidebar?: boolean;
...
nxenvrt const renderMethod = (
  method: MethodDefinitionData | AccessorDefinitionData | PropData,
  { apiName, nxenvseInSidebar = true, sdkVersion, ...options }: RenderMethodOptions
) => {
...
const APISectionMethods = ({
  ...
  apiName,
  header = 'Methods',
  nxenvseInSidebar = true,
}: APISectionMethodsProps) =>
  data?.length ? (
    <>
      <H2 key={`${header}-header`}>{header}</H2>
      {data.map((method: MethodDefinitionData | PropData) =>
        renderMethod(method, { apiName, sdkVersion, header, nxenvseInSidebar })
      )}
    </>
  ) : null;

nxenvrt default APISectionMethods;
