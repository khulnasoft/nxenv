...
import remarkNxenvrtHeadings from './mdx-plugins/remark-nxenvrt-headings.js';
...
NextLog.info(`Copied latest Nxenv SDK version from v${version}`);
...
nxenvrt default {
  ...
  // Create a map of all pages to nxenvrt
  // https://nextjs.org/docs/api-reference/next.config.js/nxenvrtPathMap
  async nxenvrtPathMap(defaultPathMap, { dev, outDir }) {
    if (dev) {
    ...
    const pathMap = Object.assign(
      ...Object.entries(defaultPathMap).map(([pathname, page]) => {
        if (pathname.match(/unversioned/)) {
          // Remove unversioned pages from the nxenvrted site
          return {};
        } else {
          // Remove newer unreleased versions from the nxenvrted side
