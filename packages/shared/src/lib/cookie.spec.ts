...
describe('cookie', () => {
  ...
  it('should set with options', () => {
    ...
    expect(document.cookie).toBe(
      'foo=bar; domain=nxenv.khulnasoft.com; max-age=315360000; partitioned; path=/; samesite=strict',
    );
  });
  ...
  it('should expire cookie with options', () => {
    ...
    expect(document.cookie).toBe(
      'foo=expired; domain=nxenv.khulnasoft.com; max-age=0; path=/',
    );
  });
});
