...
it('should show a message that no rss found', async () => {
  ...
  nock('http://localhost:3000')
    .get('/scrape/source')
    .query({ url: 'https://nxenv.khulnasoft.com' })
    .reply(200, { type: 'website', rss: [] });
  const input = await screen.findByRole('textbox');
  userEvent.type(input, 'https://nxenv.khulnasoft.com');
...
it('should show if the source already exists in the system', async () => {
  ...
  nock('http://localhost:3000')
    ...
    .reply(200, {
      type: 'website',
      rss: [
        { url: 'https://nxenv.khulnasoft.com/blog/rss.xml', title: 'RSS' },
  ...
  const source = {
    id: 'daily',
    name: 'nxenv.khulnasoft.com',
    handle: 'daily',
    image: 'https://nxenv.khulnasoft.com',
  };
  mockGraphQL({
    request: {
      query: SOURCE_BY_FEED_QUERY,
      variables: { feed: 'https://nxenv.khulnasoft.com/blog/rss.xml' },
    },
    result: {
      data: {
        source,
...
it('should send source request', async () => {
  ...
  mockGraphQL({
    request: {
      ...
      variables: { data: { sourceUrl: 'https://nxenv.khulnasoft.com/blog/rss.xml' } },
    },
    result: {
      data: {
        source: {
