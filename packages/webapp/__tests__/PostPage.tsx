...
} from '@nxenv/shared/src/graphql/actions';
...
import { SourceType } from '@nxenv/shared/src/graphql/sources';
import { createTestSettings } from '@nxenv/shared/__tests__/fixture/settings';
import {
...
import * as hooks from '@nxenv/shared/src/hooks/useViewSize';
import { UserVoteEntity } from '@nxenv/shared/src/hooks';
import { VOTE_MUTATION } from '@nxenv/shared/src/graphql/users';
...
//
// jest.mock('@nxenv/shared/src/hooks/useSubscription', () => ({
//   __esModule: true,
//   default: jest
//     .fn()
//     .mockImplementation(
...
it('should check meta tag with no description and no summary', async () => {
  ...
  expect(seo).toEqual(
    'Join us to the discussion about "Learn SQL" on nxenv.khulnasoft.com ✌️',
  );
});
...
it('should check meta tag with no description, summary of shared post', async () => {
  const seo = getSeoDescription(
    createPostMock({
      ...
      sharedPost: {
        ...
        commentsPermalink: 'https://nxenv.khulnasoft.com',
        type: PostType.Article,
  ...
  expect(seo).toEqual(
    'Join us to the discussion about "GitHub is down" on nxenv.khulnasoft.com ✌️',
  );
});
...
it('should check meta tag with no description, summary or title', async () => {
  ...
  expect(seo).toEqual('Join the discussion on nxenv.khulnasoft.com ✌️');
});
  ...
  renderPost({}, [
    createPostMock({
      author: {
        id: 'u1',
        name: 'Ido Shamun',
        image: 'https://nxenv.khulnasoft.com/ido.jpg',
        permalink: 'https://nxenv.khulnasoft.com/idoshamun',
        username: 'idoshamun',
      },
    }),
    createCommentsMock(),
