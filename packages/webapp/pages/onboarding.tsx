...
import AuthOptions, {
  ...
  AuthOptionsProps,
  AuthProps,
} from '@nxenv/shared/src/components/auth/AuthOptions';
import { AuthTriggers } from '@nxenv/shared/src/lib/auth';
import { OnboardingHeader } from '@nxenv/shared/src/components/onboarding';
import {
  ButtonSize,
  ButtonVariant,
} from '@nxenv/shared/src/components/buttons/Button';
import { ExperimentWinner } from '@nxenv/shared/src/lib/featureValues';
import { storageWrapper as storage } from '@nxenv/shared/src/lib/storageWrapper';
...
import {
  OnboardingStep,
  wrapperMaxWidth,
} from '@nxenv/shared/src/components/onboarding/common';
import { useAuthContext } from '@nxenv/shared/src/contexts/AuthContext';
import { NextSeo, NextSeoProps } from 'next-seo';
import { SIGNIN_METHOD_KEY } from '@nxenv/shared/src/hooks/auth/useSignBack';
import {
  useFeature,
  useFeaturesReadyContext,
  useGrowthBookContext,
} from '@nxenv/shared/src/components/GrowthBookProvider';
import SignupDisclaimer from '@nxenv/shared/src/components/auth/SignupDisclaimer';
import {
  FooterLinks,
  withFeaturesBoundary,
} from '@nxenv/shared/src/components';
import useFeedSettings from '@nxenv/shared/src/hooks/useFeedSettings';
import {
  EXPERIENCE_TO_SENIORITY,
  logSignUp,
  OnboardingLogs,
} from '@nxenv/shared/src/components/auth/OnboardingLogs';
import { feature } from '@nxenv/shared/src/lib/featureManagement';
import { OnboardingHeadline } from '@nxenv/shared/src/components/auth';
import { useViewSize, ViewSize } from '@nxenv/shared/src/hooks';
import { GenericLoader } from '@nxenv/shared/src/components/utilities/loaders';
import { LoggedUser } from '@nxenv/shared/src/lib/user';
import { useSettingsContext } from '@nxenv/shared/src/contexts/SettingsContext';
import { ChecklistViewState } from '@nxenv/shared/src/lib/checklist';
import { getPathnameWithQuery } from '@nxenv/shared/src/lib';
import { webappUrl } from '@nxenv/shared/src/lib/constants';
import useMutateFilters from '@nxenv/shared/src/hooks/useMutateFilters';
import dynamic from 'next/dynamic';
import { defaultOpenGraph, defaultSeo } from '../next-seo';

const ContentTypes = dynamic(() =>
  import(
    /* webpackChunkName: "contentTypes" */ '@nxenv/shared/src/components/onboarding/ContentTypes/ContentTypes'
  ).then((mod) => mod.ContentTypes),
);
const EditTag = dynamic(() =>
  import(
    /* webpackChunkName: "editTag" */ '@nxenv/shared/src/components/onboarding/EditTag'
  ).then((mod) => mod.EditTag),
);
const ReadingReminder = dynamic(() =>
  import(
    /* webpackChunkName: "readingReminder" */ '@nxenv/shared/src/components/onboarding/ReadingReminder'
  ).then((mod) => mod.ReadingReminder),
);
const OnboardingFooter = dynamic(() =>
  import(
    /* webpackChunkName: "onboardingFooter" */ '@nxenv/shared/src/components/onboarding/OnboardingFooter'
...
export function OnboardPage(): ReactElement {
  ...
  return (
    <div className="z-3 flex h-full max-h-screen min-h-screen w-full flex-1 flex-col items-center overflow-x-hidden">
      ...
      <NextSeo {...seo} titleTemplate="%s | nxenv.khulnasoft.com" />
