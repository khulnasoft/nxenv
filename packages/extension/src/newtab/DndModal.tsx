...
import { Dropdown } from '@nxenv/shared/src/components/fields/Dropdown';
import { TextField } from '@nxenv/shared/src/components/fields/TextField';
import {
  Modal,
  ModalProps,
} from '@nxenv/shared/src/components/modals/common/Modal';
import { Justify } from '@nxenv/shared/src/components/utilities';
...
export default function DndModal({
  ...
  const getDescription = () => {
    if (!isActive) {
      return 'Choose the URL to temporarily show instead of nxenv.khulnasoft.com and the pause duration.';
    }

    const date = format(dndSettings.expiration, 'MM/dd/yy');
    const time = format(dndSettings.expiration, 'hh:mm a');

    return `nxenv.khulnasoft.com in a new tab is paused, it will resume on ${date} at ${time}`;
  };
