...
nxenvrt const EXPAND_SNIPPET_BOUND = 408;
nxenvrt const LANGUAGES_REMAP: Record<string, string> = {
  'objective-c': 'objc',
  sh: 'bash',
  rb: 'ruby',
};

nxenvrt function cleanCopyValue(value: string) {
  return value
...
nxenvrt function escapeHtml(text: string) {
  return text.replace(/"/g, '&quot;');
}

nxenvrt function replaceXmlCommentsWithAnnotations(value: string) {
  return value
    .replace(
...
nxenvrt function replaceHashCommentsWithAnnotations(value: string) {
  return value
    .replace(
      /<span class="token (comment|plain-text)"># @info (.*?)#<\/span>\s*/g,
      (match, type, content) => {
...
nxenvrt function replaceSlashCommentsWithAnnotations(value: string) {
  return value
    .replace(
...
nxenvrt function replaceSlashCommentsWithAnnotationsForTutorial(value: string) {
  return value
    .replace(
      /<span class="token (comment|plain-text)">([\n\r\s]*)\/\* @tutinfo (.*?)\*\/[\n\r\s]*<\/span>\s*/g,
      (match, type, beforeWhitespace, content) => {
...
nxenvrt function parseValue(value: string) {
  if (value.startsWith('@@@')) {
...
nxenvrt function getRootCodeBlockProps(children: ReactNode, className?: string) {
...
nxenvrt function findPropInChildren(element: ReactElement, propToFind: string): string | null {
...
nxenvrt function getCollapseHeight(params?: Record<string, string>) {
...
nxenvrt function getCodeData(value: string, className?: string) {
