 ...
 */
nxenvrt const toString = (node: React.ReactNode): string => {
...
nxenvrt const generateSlug = (slugger: GithubSlugger, node: React.ReactNode, length = 7): string => {
 ...
 */
nxenvrt const replaceVersionInUrl = (url: string, replaceWith: string) => {
 ...
 */
nxenvrt const getUserFacingVersionString = (
  version: string,
...
nxenvrt const stripVersionFromPath = (path?: string) => {
  if (!path) {
    return path;
  }
...
nxenvrt const pathStartsWith = (name: string, path: string) => {
  return path.startsWith(`/${name}`);
};

nxenvrt const chunkArray = (array: any[], chunkSize: number) => {
...
nxenvrt function listMissingHashLinkTargets(apiName?: string) {
