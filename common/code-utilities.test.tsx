...
describe(cleanCopyValue, () => {
  it('SlashComments - preserves the fully annotated line', () => {
    expect(
      cleanCopyValue(
        `/* @info Import FontAwesome. */import FontAwesome from "@nxenv/vector-icons/FontAwesome";/* @end */`
      )
    ).toBe(`import FontAwesome from "@nxenv/vector-icons/FontAwesome";`);
  });

  it('SlashComments - removes the annotation mid-line', () => {
    expect(
      cleanCopyValue(
        `nxenvrt default function Button({ label,/* @info The prop theme to detect the button variant. */ theme/* @end */ }) {`
      )
    ).toBe(`nxenvrt default function Button({ label, theme }) {`);
  });
